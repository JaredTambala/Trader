...
| `coding_write_candidate_file` | Strategy Engineering; local workspace mutation; active exact attempt and file/total byte budgets required. | Complete replacement with identical content is stable; runtime fingerprints each accepted write before any retry. | Candidate path, hash, bytes, and workspace use; traversal/suffix/size/inactive-workspace failure; `new_authorship_and_repair`, `crash_and_lost_response`. |
...
| `coding_destroy_workspace` | Strategy Engineering runtime cleanup; irreversible local mutation after package acceptance or terminal failure. | A second call currently returns a cleanup error; the runtime must checkpoint accepted cleanup and never use blind retry. Making exact cleanup replay-safe is a remaining capability gap. | Destroyed identity and non-recoverable status; missing/path/OS failure; `crash_and_lost_response`, `irreparable_admission`. |

...
under the bounded-context authority declared in [agent roles and authority](../../trader_agents/docs/roles_and_authority.md). The MCP `agent_owner` envelope field identifies
...
The runtime/deployment subset is implemented; feature, training, evaluation, registry, and drift capabilities remain
planned. All current and future ML requests and
...
Full walk-forward optimisation depends on reproducible backtesting, ML deployment where applicable, and robustness
prerequisites. Chronological validation folds remain an earlier ML correctness contract.
...
| `coding_run_check` | Strategy Engineering; isolated local execution; active workspace, allowlisted check, pinned container, and resource budget required. | Repeatable only for the same workspace snapshot/check identity; every attempt remains public lineage. The runner owns an exact container ID and verifies forced cleanup after every exit or cutoff. | Exit/timeout/bounded output evidence; missing OCI runtime, non-zero, timeout, output cutoff, cleanup failure, or policy denial; `new_authorship_and_repair`, `irreparable_admission`. |
| `coding_package_candidate` | Strategy Engineering; read-only inert packaging; exact active workspace/source required. | Content-addressed package ID is stable for identical files. Complete source is retained in the service-owned immutable package after workspace cleanup but is not returned through the model-facing envelope. | Package and attempt identity, source hash, file manifest, and package ID; syntax/path/size failure; all adaptation/authorship scenarios. |
...
| `research_register_strategy_implementation` | Strategy Engineering caller, Experiments authority; canonical local mutation; accepted package/build lineage and mutation budget required. General callers select exactly one of complete `source_code` or `candidate_package_id`; Strategy Engineering policy permits only its exact package ID. | The MCP adapter resolves retained package source inside the deterministic service boundary, injects immutable attempt/build/repository metadata, and performs content-addressed registration. Exact replay is stable; changed source creates a new version. | Source-free Strategy-facing implementation result and canonical ref; selector, package, schema, source, or store conflict; adaptation/authorship scenarios. |
...
Budget use counts physical work rather than only the latest checkpoint. Each provider request has one public call
identity and must terminate in a result or validation event. Fresh-process recovery merges those observations with
checkpoint usage and canonical mutation journals without counting the same accepted operation twice. A delegation
reservation is an upper bound for the worst allowed path—including a bounded structured-output or candidate repair—so
the scheduler rejects work that the parent session cannot fund before dispatch.
...
Implementation registration accepts `name`, `version`, `factory_name`, exactly one of complete `source_code` or an
immutable Coding Workspace `candidate_package_id`, optional `class_name`, a bounded parameter schema, dependency
declarations, authoring origin, capabilities, resource/runtime requirements, optional generic provenance refs, and
metadata. Package-backed registration resolves source inside the MCP adapter and injects package, candidate-attempt,
build-contract, repository-revision, and source-hash lineage. Strategy Engineering cannot submit direct source. IDs
are content-addressed over normalized identity and source hash.
...
[knowledge extraction](../../trader_research/docs/knowledge.md). This appendix defines transport and artifact contracts.
...
| ML Agent | Registered `ml_create_deployment_manifest` and `ml_validate_deployment`; the remaining lifecycle tools are planned. |

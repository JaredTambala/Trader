...
from trader_mcp.protocol.contracts import SideEffect, error_envelope
...
def register_quant_methods_tools(
    ...
    *,
    ...
    resolved_embedding_provider = (
        embedding_provider
        or RuntimeConfiguredEmbeddingProvider(env=environment.embeddings_env())
    )

    def _knowledge_store() -> KnowledgeStore | None:
        return (
            knowledge_store_provider() if knowledge_store_provider is not None else None
        )
    ...
    def _artifact_store_error(
        command: str, error: ResearchArtifactStoreError
    ) -> CallToolResult:
    ...
    def knowledge_ingest_documents(
    ...
    @server.tool(
        ...
        description=KNOWLEDGE_TOOL_DESCRIPTIONS[
            KNOWLEDGE_DISCOVER_METHODOLOGY_CANDIDATES_TOOL
        ],
    )

...
...         values: list[float] = []
...         for index in range(0, len(bars) - self.period + 1):
...             window = bars[index : index + self.period]
...             closes = [bar.close for bar in window]
...             values.append(max(closes) - min(closes))
...         return values
...
>>> indicator = CloseRangeIndicator(period=2)
>>> (indicator.name, indicator.window)
('close_range', 2)
>>> now = datetime(2026, 1, 1, tzinfo=UTC)
>>> bars = tuple(
...     Bar(ts=now - timedelta(minutes=index), open=value, high=value, low=value, close=value, volume=1.0, vwap=None, trade_count=None)
...     for index, value in enumerate((12.0, 10.0, 11.0))
... )
>>> indicator.compute_series(bars)
[2.0, 1.0]
```

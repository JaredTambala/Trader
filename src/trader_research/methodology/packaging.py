...
from trader_research.methodology.implementation.io import file_sha256, load_manifest, validation_report_path
...
def package_method_artifact(
    *,
    ...
    Returns:
        Local-mutating package result. Invalid Python implementation or validation
...
def _implementation_blockers(implementation: MethodImplementationManifest) -> list[str]:
    ...
    entry = get_method(implementation.method_id)
    requires_evidence = implementation.implementation_kind == "generated" or bool(entry and entry.requires_evidence)
    if requires_evidence and not implementation.method_card_ids:

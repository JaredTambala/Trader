...
from trader_research.method_implementations.io import file_sha256, load_manifest
...
def compile_cpp_kernel(
    *,
    ...
    if blockers:
        return _failed_compile_envelope(artifact_root, manifest, blockers=blockers)
    ...
    try:
        ...
        return _failed_compile_envelope(
            artifact_root,
            manifest,
...
def _python_manifest_blockers(manifest: MethodImplementationManifest) -> list[str]:
    ...
    if not manifest.method_card_ids:
...
def _blocked_generation(blockers: list[str]) -> ToolEnvelope:
...
def _local_error(command: str, code: str, message: str) -> ToolEnvelope:
    return error_envelope(
        command=command,
        side_effect=SideEffect.LOCAL_MUTATING,
        code=code,

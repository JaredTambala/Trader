...
def run_multiple_testing_report(
    *,
    ...
    Args:
        ...
        knowledge_store: Optional injected store used for citation validation.

    Returns:
        A Quantitative Methods `ToolEnvelope` and a persisted
        `multiple_testing_report.json` artifact. Validation failures are
        returned as a failed envelope with blockers embedded in the artifact.
    """
    issues = _Issues()
...
def _validate_bh_contract(
    *,
    ...
    try:
        entry = get_method("benjamini_hochberg", knowledge_store=knowledge_store)
    ...
    if entry.approved_method_card_ids and not set(method_card_ids).intersection(entry.approved_method_card_ids):
        issues.warnings.append("method-card evidence for benjamini_hochberg is not in the seeded registry allowlist")
    ...
    if not citation_result.ok:
        issues.blockers.append("knowledge citation validation failed for benjamini_hochberg")

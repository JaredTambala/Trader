"""Adapt Postgres knowledge records to the research knowledge port.

The adapter publishes complete ingestion generations atomically and exposes
canonical source, chunk, index, report, and method-card reads. It normalizes SQL
...
from trader_research.knowledge.store import (
...
class PostgresKnowledgeStore:
    """KnowledgeStore adapter that translates Postgres records into research types.

    The adapter delegates persistence and search to `PostgresKnowledgeRecordStore`,
    converts JSON payloads back into the research-domain dataclasses, and maps
    persistence exceptions onto the knowledge-store error hierarchy. This keeps
    ...
    def runtime_summary(self) -> Mapping[str, Any]:
        """Return runtime metadata translated from the SQL record repository."""
    ...
    def save_source(self, manifest: KnowledgeSourceManifest) -> None:
        """Persist a source manifest by converting it to a record payload."""
    ...
    def load_source(self, source_id: str) -> KnowledgeSourceManifest | None:
        ...
        return (
            KnowledgeSourceManifest.from_dict(payload) if payload is not None else None
        )
    ...
    def list_sources(
        ...
        *,
        ...
        Topic, family, status, and limit are delegated to the SQL record
        repository; every returned payload is parsed into a typed source manifest.
        Persistence failures are translated to ``KnowledgeStoreError``.
        """
    ...
    def find_sources_by_file_hash(
        self, file_hash: str
    ) -> tuple[KnowledgeSourceManifest, ...]:
        """Return Postgres-backed sources with a matching file hash digest for duplicates."""
        payloads = _translate_errors(
            lambda: self._records.find_sources_by_file_hash(file_hash)
        )
        return tuple(KnowledgeSourceManifest.from_dict(payload) for payload in payloads)
    ...
    def replace_chunks(self, source_id: str, chunks: Sequence[KnowledgeChunk]) -> None:
        ...
        _translate_errors(
            lambda: self._records.replace_chunks(
                source_id, [chunk.to_dict() for chunk in chunks]
            )
        )
    ...
    def list_chunks(
        ...
        payloads = _translate_errors(
            lambda: self._records.list_chunks(source_ids=source_ids)
        )
        return tuple(KnowledgeChunk.from_dict(payload) for payload in payloads)
    ...
    def load_chunks_by_ids(
        ...
        payloads = _translate_errors(
            lambda: self._records.load_chunks_by_ids(chunk_ids)
        )
        return tuple(KnowledgeChunk.from_dict(payload) for payload in payloads)
    ...
    def index_embeddings(
        ...
        """Persist embedding vectors through the SQL record repository.

        The adapter discards `chunks` because the record repository already receives
        ...
        _translate_errors(
            lambda: self._records.index_embeddings(
                manifest.to_dict(),
                [
                    {"chunk_id": embedding.chunk_id, "vector": list(embedding.vector)}
    ...
    def save_ingestion_report(self, report: KnowledgeIngestionReport) -> None:
        """Persist an ingestion report by converting it to a record payload."""
    ...
    def publish_ingestion(
        ...
        Replacement chunks and typed vector values are converted to record JSON and
    ...
    def list_ingestion_reports(
        ...
        *,
        ...
        payloads = _translate_errors(
            lambda: self._records.list_ingestion_reports(
                source_ids=source_ids, run_id=run_id
            )
        )
        return tuple(
            KnowledgeIngestionReport.from_dict(payload) for payload in payloads
        )
    ...
    def search_lexical(
        ...
        *,
        ...
        to the SQL record repository. Ranked plain-data rows are returned unchanged
        in store order, with persistence failures translated to ``KnowledgeStoreError``.
        """
    ...
    def search_vector(
        ...
        *,
        ...
        in repository order; dimension and storage failures are translated through
    ...
    def save_method_card(self, method_card: MethodCard) -> None:
        """Persist a method-card payload through the SQL record repository."""
    ...
    def save_method_card_set(self, method_card_set: MethodCardSet) -> None:
        """Persist a stable method-card set through the SQL record repository."""
        _translate_errors(
            lambda: self._records.save_method_card_set(method_card_set.to_dict())
        )
    ...
    def close(self) -> None:
        """Close the wrapped Postgres record repository after use."""
        self._records.close()

    def connection(self) -> Any:
        """Expose the wrapped Postgres connection for integration utilities and tests."""

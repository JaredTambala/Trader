"""Persist research knowledge records through a Postgres connection.
...
research infrastructure separates database-specific retrieval mechanics from
the knowledge domain and application services.
"""
...
try:  # pragma: no cover - exercised by postgres integration tests
    ...
    from psycopg.types.json import Jsonb as _jsonb_type
...
class PostgresKnowledgeRecordStore:
    ...
    def ensure_schema(self) -> None:
        ...
        try:
            ...
            raise PostgresKnowledgeStoreError(
                f"failed to initialize knowledge schema: {exc}"
            ) from exc
    ...
    def load_source(self, source_id: str) -> Mapping[str, Any] | None:
        ...
        row = self._connection.execute(
            "SELECT payload FROM knowledge_sources WHERE source_id = %s", [source_id]
        ).fetchone()
    ...
    def replace_chunks(
        self, source_id: str, chunks: Sequence[Mapping[str, Any]]
    ) -> None:
    ...
    def load_chunks_by_ids(
        ...
        by_chunk_id = {
            str(row["payload"].get("chunk_id") or ""): _mapping(row["payload"])
            for row in rows
        }
        return tuple(
            by_chunk_id[chunk_id] for chunk_id in requested if chunk_id in by_chunk_id
        )
    ...
    def search_vector(
        ...
        *,
        ...
        self._validate_vector_dimension(
            vector, provider=provider, model=model, version=version
        )
        where, params = self._search_filters(
            source_ids=source_ids,
            topic=topic,
    ...
    def save_method_card(self, payload: Mapping[str, Any]) -> None:
        ...
        validation_refs_payload = (
            ...
            if isinstance(validation_refs, Sequence)
            and not isinstance(validation_refs, (str, bytes))
            else []
        )
        self._connection.execute(
            """
    ...
    def list_persisted_method_cards(self) -> tuple[Mapping[str, Any], ...]:
        ...
        rows = self._connection.execute(
            "SELECT payload FROM knowledge_method_cards ORDER BY method_card_id"
        ).fetchall()
        return tuple(_mapping(row["payload"]) for row in rows)

"""Tests for event-store factory configuration normalization."""
...
def test_build_event_store_returns_noop_without_postgres_dependency() -> None:
    """Ensure dry-run backends bypass Postgres construction."""
    assert isinstance(build_event_store(SimpleNamespace(event_store="noop")), NoOpEventStore)
    assert isinstance(build_event_store(SimpleNamespace(event_store="none")), NoOpEventStore)

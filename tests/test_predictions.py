"""Tests for provider-neutral runtime prediction contracts."""
...
def _binding(*, predictor: FixturePredictor | None = None, failure_action: str = "fail_closed") -> RuntimePredictionBinding:
...
def test_feature_batch_hash_covers_ordered_point_in_time_values() -> None:
    ...
    assert first.to_dict(include_values=False)["rows"] == [
        {"symbol": "AAA", "as_of_ts": NOW.isoformat(), "availability_ts": NOW.isoformat()},
        {"symbol": "BBB", "as_of_ts": NOW.isoformat(), "availability_ts": NOW.isoformat()},
    ]
...
def test_universe_binding_rejects_partial_symbol_callbacks() -> None:
    with pytest.raises(PredictionRuntimeError, match="complete ordered symbol universe"):
        _binding().evaluate(
            run_id="run_1",
            cycle_id="cycle_1",
            decision_ts=NOW,

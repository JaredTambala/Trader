...
def test_mcp_quant_methods_registration_and_maintained_implementation_flow(tmp_path: Path) -> None:
    ...
    async def _run() -> None:
        ...
        manifest = registered.structuredContent["data"]["method_implementation_manifest"]
        ...
        package = await server.call_tool(
            MATH_PACKAGE_METHOD_ARTIFACT_TOOL,
            {
                "implementation_manifest": validated.structuredContent["data"]["method_implementation_manifest"],
                "validation_report": validated.structuredContent["data"]["indicator_validation_report"],
            },
        )
        ...
        assert "sma" in {item["method_id"] for item in methods.structuredContent["data"]["methods"]}
        ...
        assert package.structuredContent["data"]["method_package_manifest"]["method_card_ids"] == []

    anyio.run(_run)


def test_mcp_method_card_draft_has_only_validation_backed_contract(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    environment = replace(load_local_environment("env.template"), artifact_root=artifact_root)
    server = create_server(
        environment,
    ...
    async def _run() -> None:
        tool = next(item for item in await server.list_tools() if item.name == KNOWLEDGE_CREATE_METHOD_CARD_DRAFT_TOOL)
        ...
        assert "exactly one of methodology_candidate_validation_id" in result.structuredContent["errors"][0]["message"]

    anyio.run(_run)


def test_mcp_methodology_writes_fail_closed_without_research_artifact_store(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    environment = replace(load_local_environment("env.template"), artifact_root=artifact_root)
    server = create_server(
        environment,
    ...
    async def _run() -> None:
        ...
        assert drafted.structuredContent["errors"][0]["message"] == "research artifact store is required"

"""Optimization determinism, canonical integrity, and holdout-isolation qualification.

Subject: Repeatable evidence graphs, tamper rejection, and staged holdout access.
Level: Cross-package controlled qualification.
Collaborators: Core Postgres data, research artifacts, stdio MCP, and verification-control tables.
Guarantees: Repeated runs agree, corrupted evidence fails closed, and holdout access follows policy.
Non-goals: Statistical power analysis, optimizer quality comparison, or live data access.
"""
...
def test_postgres_57n_determinism_integrity_and_holdout_access(
    ...
    """Prove repeatability, tamper rejection, and bounded holdout access end to end."""
...
def _execute_staged_holdout_access_graph(
    ...
    async def _setup() -> Mapping[str, Any]:
        async with stdio_client(
            _server_parameters(access_stage="plan_setup")
        ) as streams:
            async with ClientSession(
                ...
                plan = _value(
                    await _call(
                        ...
                        RESEARCH_CREATE_PARAMETER_OPTIMIZATION_PLAN_TOOL,
                        {
                            ...
                            "objective_validation_ref": objective_validation[
                                "validation_id"
                            ],
                            "search_space": [
                                {
                ...
                *streams, read_timeout_seconds=timedelta(seconds=180)
                ...
                run = _value(
                    await _call(
                        ...
                        RESEARCH_RUN_PARAMETER_OPTIMIZATION_TOOL,
                        {
                            "optimization_plan_ref": setup["plan"][
    ...
    trials = [
        ...
        if record.payload.get("optimization_run_id")
        == optimization["optimization_run_id"]
    ]
    assert trials
    for trial in trials:
        assert trial["observation"]["lineage"]["fold"] == "selection"
        child = store.load_artifact(
            BACKTEST_RUN, trial["child_refs"]["backtest_run_id"]
        )

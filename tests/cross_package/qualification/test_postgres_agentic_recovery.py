...
Subject: Agent recovery and idempotency across mutation, admission, response, and cancellation boundaries.
Level: Cross-package controlled qualification.
Collaborators: Real fresh processes, local model, MCP, MLflow traces, guarded Postgres, and injected faults.
Guarantees: Accepted work is not replayed, canonical receipts survive, and cancellation remains terminal.
Non-goals: General process supervision, performance ceilings, or live trading recovery.
"""
...
def test_agentic_session_recovers_across_four_fresh_processes() -> None:
    """Prove pre-mutation, lost-response, and return-reconciliation recovery semantics across processes."""

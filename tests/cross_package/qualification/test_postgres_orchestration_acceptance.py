"""Canonical acceptance record for the frozen deterministic orchestration surface.

Subject: Final reconciliation of retained runtime, core, workflow, recovery, policy, and scale evidence.
Level: Cross-package controlled acceptance.
Collaborators: Guarded Postgres verification tables, immutable revision identity, and historical command manifests.
Guarantees: A credential-free verdict is written only when every retained phase agrees.
Non-goals: Re-executing retired orchestration behavior or qualifying the current model-backed agent architecture.
"""
...
_COMMANDS = {
    "ORCHESTRATION_RUNTIME": [
        "uv run pytest tests/cross_package/qualification/test_postgres_orchestration_runtime.py -m postgres -q -W error",
    ],
    "ORCHESTRATION_CORE": [
    ...
    "ORCHESTRATION_POLICY": [
        "uv run pytest tests/test_orchestration_policy_security.py tests/test_research_composition.py tests/test_experiment_design_specialist.py tests/cross_package/boundaries -q -W error",
    ],
    "ORCHESTRATION_SCALE": [
...
def test_controlled_orchestration_acceptance_record(
    ...
    with psycopg.connect(
        ...
        lock_hashes = {str(row["manifest"]["dependency_lock_sha256"]) for row in phases}
...
def _call_expectations(
    ...
    *,
    ...
    assert (
        counts[f"ORCHESTRATION_E2E:{RESEARCH_RECORD_WORKFLOW_OUTCOME_TOOL}:accepted"]
        == 1
    )
    assert (
        counts[f"ORCHESTRATION_E2E:{DATA_CREATE_RESEARCH_SNAPSHOT_TOOL}:accepted"] == 4
    )

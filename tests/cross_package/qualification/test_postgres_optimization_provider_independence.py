"""Provider-independent optimization qualification through the public MCP process.

Subject: Built-in optimizers and canonical result reads when optional providers are unavailable.
Level: Cross-package controlled qualification.
Collaborators: Core Postgres data, research services, stdio MCP, and a stripped subprocess environment.
Guarantees: Built-in execution and canonical reads require neither Optuna nor MLflow.
Non-goals: Optional-provider parity, model behavior, or third-party service availability.
"""
...
def test_builtin_optimizers_and_canonical_reads_need_no_optional_providers(
    ...
    """Prove built-in optimization remains usable without optional provider packages."""
...
def _server_parameters() -> StdioServerParameters:
    ...
    return StdioServerParameters(
        ...
        args=["-m", "tests.cross_package.qualification.support.mcp_postgres_no_optional_server"],
        cwd=repo_root,
        env=environment,
    )

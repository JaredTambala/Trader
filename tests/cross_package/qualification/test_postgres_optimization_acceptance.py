"""Canonical acceptance record for controlled optimization qualification.

Subject: Final reconciliation and persistence of every mandatory optimization qualification phase.
Level: Cross-package controlled acceptance.
Collaborators: Guarded Postgres evidence, immutable revision identity, and exact phase command manifests.
Guarantees: One reviewed verdict binds fixture, graph, integrity, recovery, provider, and scale evidence.
Non-goals: Executing preceding phases, repairing failed evidence, or selecting an optimization policy.
"""
...
_COMMANDS = {
    "57J": [
        "uv run pytest tests/cross_package/qualification/test_postgres_verification_runtime.py -m postgres -q -W error",
    ],
    "57K": [
        ...
        "uv run pytest tests/trader_mcp/tools/data/test_inventory_and_discovery.py tests/trader_mcp/tools/data/test_quality_and_loading.py tests/trader_mcp/tools/methodology/test_method_contracts.py tests/trader_mcp/tools/experiments/test_optimization_policy.py tests/trader_agents/contracts_state tests/cross_package/documentation/test_agent_orchestration_docs.py tests/cross_package/documentation/test_controlled_qualification_docs.py tests/cross_package/documentation/test_research_capability_docs.py tests/cross_package/documentation/test_research_roadmap_docs.py tests/trader_research/governance/test_artifact_authority.py tests/trader_research/governance/test_specialist_handoff_contracts.py tests/cross_package/boundaries -q -W error",
        "git diff --check verification-57i-freeze-v6^ verification-57i-freeze-v6",
    ],
    "57L": [
        "uv run pytest tests/cross_package/qualification/test_postgres_realistic_optimization_fixture.py -m postgres -q -W error",
    ],
    "57M": [
        "uv run pytest tests/cross_package/qualification/test_postgres_optimization_evidence_graph.py -m postgres -q -W error -s",
    ],
    "57N": [
        "uv run pytest tests/cross_package/qualification/test_postgres_optimization_determinism_integrity.py -m postgres -q -W error -s",
    ],
    "57O": [
        "uv run pytest tests/cross_package/qualification/test_postgres_optimization_recovery.py -m postgres -q -W error",
    ],
    "57P": [
        "uv run pytest tests/cross_package/qualification/test_postgres_optimization_provider_independence.py -m postgres -q -W error",
    ],
    "57Q": [
        "uv run pytest tests/trader_research/experiments/test_parameter_optimization_workflow.py tests/trader_research/experiments/test_optimization_isolation.py tests/trader_research/experiments/test_optuna_engine_contracts.py tests/trader_mcp/tools/experiments/test_optimization_policy.py tests/trader_research/experiments/test_maintained_implementation_templates.py tests/cross_package/boundaries -q -W error",
    ],
    "57R": [
        "uv run pytest tests/cross_package/qualification/test_postgres_optimization_bounded_scale.py tests/cross_package/qualification/test_postgres_optimization_evidence_graph.py -m postgres -q -W error -s",
    ],
}
...
def test_controlled_optimization_acceptance_record() -> None:
    """Persist acceptance only when every required phase matches one frozen revision."""

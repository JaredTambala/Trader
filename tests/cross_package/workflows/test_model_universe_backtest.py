"""Synchronized-universe workflow for a model-backed strategy backtest.

Subject: Composition of universe feature readiness, prediction mapping, strategy decisions, and backtesting.
Level: Cross-package local workflow.
Collaborators: Real core/standard components with DuckDB and a deterministic counting predictor.
Guarantees: Inference runs once per complete universe timestamp and records aligned evidence.
Non-goals: Research deployment validation, MCP transport, real model providers, or Postgres.
"""
...
def test_model_backtest_runs_once_per_complete_universe_timestamp(
    ...
    """Run inference only when the synchronized multi-asset feature universe is complete."""

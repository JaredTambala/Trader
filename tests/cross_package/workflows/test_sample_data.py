"""Executable workflow tests for the maintained sample backtest.

Subject: Composition of packaged sample data, core backtesting, and maintained strategy/risk implementations.
Level: Cross-package local workflow.
Collaborators: Real config, CSV loader, standard implementations, backtest runner, and local DuckDB.
Guarantees: Sample ingestion is idempotent and its documented backtest payload is reproducible.
Non-goals: Postgres persistence, provider data, optimization, or live trading.
"""
...
def test_sample_data_loader_is_idempotent(tmp_path: Path) -> None:
    """Keep repeated sample ingestion idempotent in the local event store."""
...
def test_reproducible_sample_backtest_emits_stable_payload(tmp_path: Path) -> None:
    """Require the documented sample backtest to emit reproducible evidence payloads."""

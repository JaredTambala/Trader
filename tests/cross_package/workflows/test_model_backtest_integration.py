"""Direct evidence workflow for one model-backed strategy backtest.

Subject: Composition of research deployment evidence, maintained prediction mapping, and core backtesting.
Level: Cross-package local workflow.
Collaborators: Real research/core/standard services with DuckDB and a deterministic inference adapter.
Guarantees: Immutable deployment identity reaches predictions, signals, orders, and backtest evidence.
Non-goals: MCP transport, real MLflow, Postgres recovery, training, or live trading.
"""
...
def _record_bar(
    store: DuckDBEventStore, symbol: str, ts: datetime, close: float
) -> None:
    store.record_event(
        "stock_bar_events",
        {
...
def test_model_deployment_binding_executes_backtest_with_prediction_lineage(
    tmp_path: Path,
) -> None:
    """Preserve immutable deployment lineage through direct model-backed backtest execution."""
    ...
    strategy_id = strategy_created.data["strategy_specification"][
    ...
    backtest_id = backtest_created.data["backtest_specification"][
    ...
    assert all(
        row[0] == "alpha" and row[1] == "identity_numeric:v1" for row in signal_rows
    )

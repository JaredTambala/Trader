"""Postgres-native stdio MCP evidence graph for task 57M."""
...
_PROJECTIONS = {
    IMPLEMENTATION_VERSION: ("research_implementation_versions", "implementation_version_id"),
    IMPLEMENTATION_VALIDATION_REPORT: ("research_implementation_validations", "validation_id"),
    STRATEGY_SPECIFICATION: ("research_strategy_specifications", "strategy_specification_id"),
    STRATEGY_SPECIFICATION_VALIDATION_REPORT: (
        "research_strategy_specification_validations",
        "validation_id",
    ),
    RISK_STACK_SPECIFICATION: ("research_risk_stack_specifications", "risk_stack_specification_id"),
    RISK_STACK_SPECIFICATION_VALIDATION_REPORT: (
        "research_risk_stack_specification_validations",
        "validation_id",
    ),
    BACKTEST_SPECIFICATION: ("research_backtest_specifications", "backtest_specification_id"),
    BACKTEST_SPECIFICATION_VALIDATION_REPORT: (
    ...
    PARAMETER_OPTIMIZATION_TRIAL: ("research_parameter_optimization_trials", "trial_id"),
    PARAMETER_OPTIMIZATION_EVALUATION_REPORT: (
    ...
    async def _run() -> Mapping[str, Any]:
        ...
        async with stdio_client(parameters) as (read_stream, write_stream):
            async with ClientSession(
                ...
                assert {tool.name for tool in listed.tools} == set(REGISTERED_TOOL_NAMES)
                return await _run_graph(session, responses, postgres_research_artifact_store)
...
async def _run_graph(
    ...
    selection_manifest, selection_quality = await _mcp_data_evidence(
        session, fixture.selection.start.isoformat(), fixture.selection.end.isoformat(), responses
    )
    holdout_manifest, holdout_quality = await _mcp_data_evidence(
        session, fixture.holdout.start.isoformat(), fixture.holdout.end.isoformat(), responses
    )
    ...
    selection_run = _value(
        await _call(
            session,
            RESEARCH_RUN_BACKTEST_SPECIFICATION_TOOL,
            {"backtest_specification_validation_ref": selection_validation["validation_id"]},
            responses,
        ),
        "backtest_run",
    )
    ...
    plan = _value(
        await _call(
            ...
            RESEARCH_CREATE_PARAMETER_OPTIMIZATION_PLAN_TOOL,
            {
                "base_backtest_specification_validation_ref": selection_validation["validation_id"],
    ...
    holdout_run = _value(
        await _call(
            session,
            RESEARCH_RUN_BACKTEST_SPECIFICATION_TOOL,
            {"backtest_specification_validation_ref": holdout_validation["validation_id"]},
            responses,
        ),
        "backtest_run",
    )
    ...
    reloaded = store.load_artifact(PARAMETER_OPTIMIZATION_RUN, optimization["optimization_run_id"])
    ...
    return {
        ...
        "risk_implementation_uri": _uri(IMPLEMENTATION_VERSION, risk["implementation_version_id"]),
        ...
        "evaluation_uri": _uri(PARAMETER_OPTIMIZATION_EVALUATION_REPORT, evaluation["report_id"]),
        "audit_plan_uri": _uri(PARAMETER_OPTIMIZATION_AUDIT_PLAN, audit_plan["audit_plan_id"]),
...
def _assert_parameter_sensitive_optimization(
    ...
    returns = [float(trial["observation"]["metrics"]["total_return"]) for trial in trials]
...
def _assert_artifact_references(value: Any) -> None:
    if isinstance(value, Mapping):
        if "artifact_type" in value and "uri" in value:
            assert value["uri"].startswith(f"research://postgres/{value['artifact_type']}/")

"""Adapter tests for MCP research-session and canonical-evidence tools.

Subject: Coordinator-facing session persistence, resolution, and bounded artifact reads.
Level: Adapter integration.
Collaborators: Real MCP adapters with unavailable or in-memory canonical artifact stores.
Guarantees: Missing authority fails closed and exact sessions round-trip with transport metadata.
Non-goals: Model execution, coordinator routing, Postgres recovery, or research artifact interpretation.
"""
...
def test_agentic_tools_fail_closed_without_canonical_store() -> None:
    """Fail session resolution closed when no canonical artifact store exists."""
...
def test_agentic_tools_persist_and_resolve_exact_session() -> None:
    """Persist and resolve one exact session through Coordinator-owned MCP operations."""
    ...
    async def _run() -> None:
        ...
        assert (
            resolved.structuredContent["data"]["research_session"]["session_digest"]

"""Workflow tests for MCP Data quality and bounded loading tools.

Subject: Quality reporting, loading policy, backfill planning, and evidence envelopes.
Level: Adapter integration and local workflow.
Collaborators: Real MCP adapters, DuckDB, injected loaders, and one real stdio subprocess.
Guarantees: Data mutation is gated, attributable, validated, and visible before and after loading.
Non-goals: Live provider access, Postgres recovery, agent decisions, or production-scale ingestion.
"""
...
def test_mcp_server_lists_full_data_workflow_tools(tmp_path: Path) -> None:
    """Register inventory, quality, and loading as one visible Data capability set."""
...
def test_mcp_quality_tool_returns_sample_report(tmp_path: Path) -> None:
    """Return complete quality evidence for a deterministic local sample dataset."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
...
def test_mcp_quality_tool_returns_validation_errors(tmp_path: Path) -> None:
    """Expose invalid timestamps and timeframes as structured quality errors."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    server = create_server(
        load_local_environment("env.template"), event_store_provider=lambda: store
    )
    ...
    async def _run() -> None:
        bad_datetime = await server.call_tool(
            DATA_SUMMARIZE_QUALITY_TOOL, _data_args(start="not-a-date")
        )
        bad_timeframe = await server.call_tool(
            DATA_SUMMARIZE_QUALITY_TOOL, _data_args(timeframe="bad")
        )
        ...
        assert (
            ...
            in bad_datetime.structuredContent["errors"][0]["message"]
        )
        assert bad_timeframe.isError is True
        assert bad_timeframe.structuredContent is not None
        assert (
            bad_timeframe.structuredContent["errors"][0]["code"] == "validation_error"
        )
        assert (
            "Invalid timeframe"
            in bad_timeframe.structuredContent["errors"][0]["message"]
        )
...
def test_mcp_default_environment_rejects_sample_loading(tmp_path: Path) -> None:
    """Reject mutating sample loading when the environment gate remains closed."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    server = create_server(
        load_local_environment("env.template"), event_store_provider=lambda: store
    )
    ...
    async def _run() -> None:
        ...
        assert (
            result.structuredContent["errors"][0]["code"] == "data_loading_not_allowed"
        )
...
def test_mcp_explicit_policy_allows_sample_loading(tmp_path: Path) -> None:
    """Load sample bars and return post-load evidence under explicit policy."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    journal = InMemoryResearchArtifactStore()
    environment = replace(
        load_local_environment("env.template"), allow_data_loading=True
    )
    server = create_server(
        environment,
...
def test_mcp_backfill_dry_run_plans_without_writing_rows(tmp_path: Path) -> None:
    """Plan backfill without network calls or event-store writes during dry-run."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    server = create_server(
        load_local_environment("env.template"), event_store_provider=lambda: store
    )
    ...
    async def _run() -> None:
        ...
        assert (
            inventory.structuredContent["data"]["dataset_manifest"]["total_rows"] == 0
        )

    anyio.run(_run)


def test_mcp_backfill_non_dry_run_runs_through_injected_tool_policy(
    tmp_path: Path,
) -> None:
    """Execute an approved backfill only through the injected bounded runner."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    journal = InMemoryResearchArtifactStore()
    environment = replace(
        load_local_environment("env.template"), allow_data_loading=True
    ...
    server = create_server(
        ...
        data_loading_policy=DataEnsureLoadedPolicy(
            allow_data_loading=True, backfill_runner=_runner
        ),
    )
    ...
    async def _run() -> None:
        ...
        plan_id = planned.structuredContent["data"]["load_result"]["backfill_plan"][
            "plan_id"
        ]
        result = await server.call_tool(
            DATA_ENSURE_LOADED_TOOL,
...
def test_mcp_ensure_loaded_returns_validation_errors(tmp_path: Path) -> None:
    """Reject an empty loading scope through the public validation envelope."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    server = create_server(
        load_local_environment("env.template"), event_store_provider=lambda: store
    )
    ...
    async def _run() -> None:
        result = await server.call_tool(
            DATA_ENSURE_LOADED_TOOL, _data_args(mode="existing", symbols=[])
        )
...
def test_mcp_config_safety_distinguishes_registration_and_runtime_permission() -> None:
    """Distinguish tool visibility from the independently closed mutation gate."""
    ...
    async def _run() -> None:
        ...
        assert (
            data["safety"]["data_loading_mutation_allowed"]
            is environment.allow_data_loading
        )
        ...
        assert (
            data["safety"]["backtest_execution_allowed"] is environment.allow_backtests
        )
...
def test_stdio_mcp_complete_data_workflow_evidence() -> None:
    """Observe inventory, loading, and improved quality through real stdio transport."""
    async def _run() -> None:
        ...
        server_params = StdioServerParameters(
            ...
            args=[
                "-m",
                "tests.trader_mcp.tools.data.support.mcp_sample_loading_server",
            ],
            cwd=repo_root,
            env=env,
        )

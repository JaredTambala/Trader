"""Package-local MCP stdio server seeded with sample inventory data.

The subprocess fixture belongs to the MCP Data tool tests and exposes no
general repository-level test API.
"""
...
SAMPLE_CSV = Path(__file__).resolve().parents[5] / "examples/data/demo_stock_1min.csv"
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="trader-mcp-evidence-") as tmp_dir:
        ...
        try:
            create_server(local_env, event_store_provider=lambda: store).run(

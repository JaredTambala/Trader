"""Policy tests for MCP parameter-optimization and tracking tools.

Subject: Registration metadata and independent execution, provider, and external-write gates.
Level: Adapter integration.
Collaborators: Real MCP adapters with in-memory research artifacts; no optimizer execution.
Guarantees: Optimization, variants, and tracking projections remain separately gated and attributed.
Non-goals: Trial selection, backtest execution, Postgres persistence, or provider correctness.
"""
...
def test_mcp_exposes_decoupled_tools_and_independent_write_gates() -> None:
    """Expose decoupled optimization capabilities with independent conservative write gates."""
    ...
    async def _run() -> None:
        ...
        assert metadata[RESEARCH_REGISTER_OPTIMIZATION_OBJECTIVE_TOOL][
            "agent_owner"
        ] == ("Quantitative Methods Agent")
        assert (
            metadata[RESEARCH_RUN_PARAMETER_OPTIMIZATION_TOOL]["side_effect"]
            == "local_mutating"
        )
        ...
        assert (
            config.structuredContent["data"]["safety"]["optimization_execution_allowed"]
            is False
        )
        assert config.structuredContent["data"]["experiment_tracking_runtime"][
            "authority"
        ...
        assert (
            backtest.structuredContent["errors"][0]["code"] == "backtests_not_allowed"
        )
        ...
        assert (
            optimization.structuredContent["errors"][0]["code"]
        ...
        projection = await server.call_tool(
            RESEARCH_PROJECT_EXPERIMENT_TRACKING_TOOL,
            {
                "canonical_run_ref": "missing",
                "tracking_profile": "mlflow_backtest_optimization",
            },
        )
        ...
        profiles = {
            ...
            for item in runtime.structuredContent["data"]["profiles"]
        }
...
def test_optimization_requires_its_gate_after_backtests_are_enabled() -> None:
    """Require optimization permission independently after backtest execution is enabled."""
    environment = replace(
        load_local_environment("env.template"),
    ...
    async def _run() -> None:
        ...
        assert (
            result.structuredContent["errors"][0]["code"] == "optimization_not_allowed"
        )
...
def test_optuna_variant_execution_uses_the_same_external_write_gates() -> None:
    """Apply identical external-provider gates to baseline and variant Optuna execution."""
    store = InMemoryResearchArtifactStore()
    store.save_artifact(
    ...
    async def _run() -> None:
        ...
        assert (
            result.structuredContent["errors"][0]["code"] == "optuna_writes_not_allowed"
        )

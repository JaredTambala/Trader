...
def test_package_validated_indicator_method_artifact(tmp_path: Path) -> None:
    ...
    assert repeated.data["method_package_manifest"]["package_id"] == package["package_id"]
...
def test_package_validated_signal_method_artifact(tmp_path: Path) -> None:
    ...
    assert package["validation_report_ref"]["artifact_type"] == "signal_implementation_validation_report"


def test_package_resolves_persisted_implementation_and_report_ids(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    manifest, report = _validated_sma(artifact_root)
    ...
    package = result.data["method_package_manifest"]
    assert package["implementation_id"] == manifest["implementation_id"]
    assert package["validation_report_ref"]["path"].endswith(f"{report['validation_id']}.json")
...
def test_package_rejects_missing_validation_report(tmp_path: Path) -> None:
    ...
    assert result.ok is False
    assert "validation_report_id or validation_report is required" in result.data["blockers"]
...
def test_package_handles_optional_cxx_metadata_as_non_gating(tmp_path: Path) -> None:
    ...
    generated_cxx = {**valid_cxx, "status": "generated", "build": {"status": "not_compiled"}}
    ...
    assert with_cxx.data["method_package_manifest"]["cxx_kernel_refs"][0]["kernel_id"] == "cxx_kernel_test"
    assert invalid_cxx.ok is True, invalid_cxx.to_dict()
    assert invalid_cxx.data["method_package_manifest"]["cxx_kernel_refs"] == []
    assert any("optional C++ kernel excluded: status must be compiled" in warning for warning in invalid_cxx.warnings)
...
def _validated_signal(artifact_root: Path) -> tuple[dict[str, object], dict[str, object]]:
...
def _compiled_cxx_manifest(python_manifest: dict[str, object]) -> dict[str, object]:
    return {
        ...
        "generated_source": {"path": "artifacts/research/cpp_kernels/sources/cxx_kernel_test.cpp", "sha256": "abc"},
        "build": {"status": "compiled", "binary_path": "artifacts/research/cpp_kernels/build/cxx_kernel_test.so"},

"""Contracts for the long-lived trader service orchestration shell.

Subject: Loop iteration, required injection, broker-portfolio mismatch, startup reset, and periodic reconciliation.
Level: In-process runtime integration contracts with bounded fakes and temporary DuckDB where needed.
Collaborators: Real TraderService, standard no-op policies, recording cycle and broker fakes, and event stores.
Guarantees: Service startup and scheduling preserve injected boundaries and fail closed on ambiguous broker state.
Non-goals: Realtime Postgres listeners, provider networking, actual sleeps, order submission, or sustained operation.
"""
...
def test_trader_service_loop_runs_expected_iterations(monkeypatch) -> None:
    """Ensure bounded loop mode reuses injected dependencies for the configured iterations."""
    recorder = CycleRecorder()
    broker = object()
    monkeypatch.setattr("trader.runtime.service.run_cycle", recorder)
    monkeypatch.setattr(
        "trader.runtime.service.build_runtime_broker",
        lambda config, event_store: broker,
    )
...
def test_trader_service_requires_injected_risk_manager() -> None:
    """Ensure service construction rejects missing risk authority before execution starts."""
...
def test_trader_service_fails_closed_on_broker_position_mismatch(monkeypatch) -> None:
    """Ensure startup aborts when broker positions exceed the configured trading universe."""
...
def test_trader_service_resets_local_portfolio_from_alpaca_before_mismatch_failure(
    monkeypatch, tmp_path
) -> None:
    """Ensure broker truth is recorded locally before a mismatch stops service startup."""
    broker = FakeAlpacaBroker()
...
def test_trader_service_periodic_reconcile_uses_broker_capability(monkeypatch) -> None:
    """Ensure due periodic reconciliation invokes the persistent broker's explicit capability."""

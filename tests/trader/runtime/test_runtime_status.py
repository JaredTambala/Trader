"""Contracts for event-store-first runtime status and health assessment.

Subject: Health reasons, run and cycle state, market-data freshness, halts, portfolios, and stale orders.
Level: Deterministic unit and temporary-DuckDB status-integration contracts.
Collaborators: Real status services and health logic with fixed configuration, timestamps, and DuckDB events.
Guarantees: Persisted runtime evidence yields stable operator status, severity, reasons, and halt behavior.
Non-goals: Broker queries, Postgres semantics, operator CLI formatting, service control, or live remediation.
"""
...
def test_assess_runtime_health_returns_typed_healthy_assessment() -> None:
    """Ensure complete successful evidence yields a typed healthy assessment and zero exit."""
...
def test_assess_runtime_health_accumulates_degraded_and_unhealthy_reasons() -> None:
    """Ensure independent failure evidence accumulates into one deterministic unhealthy assessment."""
    assessment = assess_runtime_health(
...
def test_runtime_status_empty_store_is_degraded(tmp_path) -> None:
    """Ensure an empty event store reports missing evidence as degraded, not healthy."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))

    status = runtime_status(
        store, _config(), now=datetime(2026, 1, 1, tzinfo=timezone.utc)
    )
...
def test_runtime_status_healthy_store(tmp_path) -> None:
    """Ensure recent successful runtime evidence produces complete healthy operator status."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
...
def test_runtime_status_stale_market_data_is_unhealthy(tmp_path) -> None:
    """Ensure expired bar timestamps surface as unhealthy market-data evidence."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    now = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
    _seed_successful_runtime(store, now=now)
    store.record_event(
...
def test_halt_state_degrades_health(tmp_path) -> None:
    """Ensure an operator halt round-trips and prevents a healthy classification."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    now = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
...
def test_stale_open_orders_are_reported(tmp_path) -> None:
    """Ensure old nonterminal orders are counted and exposed as stale status evidence."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    now = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
    _seed_successful_runtime(store, now=now)
    store.record_event(
...
def _seed_successful_runtime(store: DuckDBEventStore, *, now: datetime) -> None:
    store.record_run_session_start("run_1", "trading", now, strategy_id="noop")
    store.record_run_session_finish(
        "run_1", "trading", now, now, "success", None, strategy_id="noop"
    )
    store.record_cycle_start("run_1", "cycle_1", "noop", "loop", now, now)
    store.record_cycle_finish(
        "run_1", "cycle_1", "noop", "loop", now, now, now, "success", None
    )

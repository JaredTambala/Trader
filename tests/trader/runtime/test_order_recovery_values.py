"""Contracts for pure runtime order-recovery values and decisions.

Subject: Scope partitioning, latest-event projection, timestamps, local repair, broker adoption, and cleanup plans.
Level: Pure domain and query-planning unit contracts.
Collaborators: Real recovery value helpers supplied with fixed local and broker-shaped mappings.
Guarantees: Equivalent order evidence yields deterministic, normalized, auditable recovery actions or explicit skips.
Non-goals: Broker calls, database execution, event persistence, clocks, service startup, or order cancellation.
"""
...
def test_partition_broker_orders_normalizes_asset_class_and_crypto_symbols() -> None:
    """Ensure broker orders are normalized before configured-universe scope partitioning."""
...
def test_partition_local_orders_uses_configured_asset_class_for_symbol_normalization() -> (
    None
):
    """Ensure local order symbols use the configured asset class during scope checks."""
...
def test_latest_order_events_from_rows_keeps_newest_row_per_client_order() -> None:
    """Ensure ordered event rows yield one newest record per identified order."""
...
def test_build_latest_order_events_query_names_expected_projection() -> None:
    """Ensure the recovery query selects complete order evidence in deterministic newest-first order."""
...
def test_parse_timestamp_accepts_z_suffix_and_naive_values() -> None:
    """Ensure supported timestamps become UTC-aware while malformed values remain absent."""
    assert parse_timestamp("2026-01-01T12:00:00Z") == datetime(
        2026, 1, 1, 12, tzinfo=timezone.utc
    )
    assert parse_timestamp(datetime(2026, 1, 1, 12)) == datetime(
        2026, 1, 1, 12, tzinfo=timezone.utc
    )
...
def test_plan_local_open_order_recovery_closes_missing_broker_order() -> None:
    """Ensure missing broker truth plans an explicit terminal local-order repair event."""
...
def test_plan_local_open_order_recovery_updates_changed_broker_order() -> None:
    """Ensure changed broker status plans a matching local transition and fill evidence."""
    fallback_ts = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
    ...
    plan = plan_local_open_order_recovery(
        local_event, broker_order, fallback_ts=fallback_ts
    )
...
def test_plan_local_open_order_recovery_ignores_matching_broker_order() -> None:
    """Ensure matching local and broker status produces no redundant recovery action."""
    fallback_ts = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
    local_event = {
...
def test_plan_broker_open_adoption_prepares_missing_local_order() -> None:
    """Ensure unidentified local history yields a complete broker-order adoption plan."""
    fallback_ts = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
...
def test_plan_broker_open_adoption_skips_known_or_unidentified_orders() -> None:
    """Ensure known orders and broker rows without identity are not adopted twice."""
    fallback_ts = datetime(2026, 1, 1, 12, tzinfo=timezone.utc)
...
def test_plan_local_clean_start_close_prepares_local_cancellation() -> None:
    """Ensure clean-start plans a local cancellation without implying a broker action."""
    ...
    plan = plan_local_clean_start_close(
        local_event, run_id="run_clean", event_ts=event_ts
    )

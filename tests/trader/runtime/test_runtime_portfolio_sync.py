"""Contracts for normalizing broker portfolio truth and initial local seeds.

Subject: Account cash, provider positions, configured-universe mismatches, log records, and seed decisions.
Level: Pure runtime-policy unit contracts.
Collaborators: Real portfolio-sync helpers with fixed account, position, configuration, and stored-state values.
Guarantees: Broker state remains complete and normalized while mismatches and seed reasons stay inspectable.
Non-goals: Provider calls, snapshot persistence, service startup, order recovery, or concurrent portfolio changes.
"""
...
def test_broker_portfolio_sync_snapshot_preserves_all_positions_and_flags_mismatches() -> (
    None
):
    """Ensure synchronization retains broker truth while identifying configured-universe mismatches."""
...
def test_broker_portfolio_sync_snapshot_treats_missing_cash_as_zero() -> None:
    """Ensure absent broker cash normalizes explicitly to the runtime zero balance."""
    snapshot = build_broker_portfolio_sync_snapshot(
        account={"cash": None},
        positions_raw=[],
        configured_symbols=(),
...
def test_initial_portfolio_seed_config_is_resolved_before_reading_existing_state() -> (
    None
):
    """Ensure seed eligibility is determined from configuration before persistent-state inspection."""
    assert (
        resolve_initial_portfolio_seed_config(
            portfolio_source="alpaca",
            config_snapshot={"trader_service": {"initial_cash": 1000}},
        ).reason
        == "portfolio_source_alpaca"
    )
    assert (
        resolve_initial_portfolio_seed_config(
            portfolio_source="db",
...
def test_initial_portfolio_seed_decision_respects_existing_state() -> None:
    """Ensure existing portfolio evidence prevents an initial configuration seed from overwriting it."""
    seed_config = resolve_initial_portfolio_seed_config(
        portfolio_source="db",
...
def test_initial_portfolio_seed_decision_builds_seed_positions_and_cash() -> None:
    """Ensure an empty eligible store receives normalized configured positions and cash."""
    seed_config = resolve_initial_portfolio_seed_config(
        portfolio_source="db",
        config_snapshot={
            "trader_service": {

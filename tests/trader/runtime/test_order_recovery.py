"""Contracts for startup order recovery and operator-requested local cleanup.

Subject: Recovery payloads, missing-local closure, broker-order adoption, fail-closed scope, and clean start.
Level: Deterministic unit and temporary-DuckDB broker-integration contracts.
Collaborators: Real recovery shell and Alpaca adapter with shared broker fakes and a DuckDB event store.
Guarantees: Broker truth yields append-only local evidence while out-of-scope ambiguity remains visible and closed.
Non-goals: Provider networking, cancelling broker orders, concurrent reconciliation, Postgres, or service scheduling.
"""
...
from tests.trader.broker.test_alpaca_broker import FakeOrder, FakeTradingClient


def _broker(store, client):
    return AlpacaPaperBroker(
        api_key="key", secret_key="secret", event_store=store, client=client
    )


def test_build_order_event_payload_is_deterministic_without_mutating_input() -> None:
    """Ensure local recovery records are deterministic and leave broker mappings unchanged."""
    order = {
...
def test_build_fill_event_payload_requires_fill_fields_and_uses_explicit_fallback_time() -> (
    None
):
    """Ensure fill evidence needs quantity and price and uses the supplied fallback clock."""
    ...
    assert (
        build_fill_event_payload({"fill_qty": 1.0}, fallback_fill_ts=fallback_ts)
        is None
    )
    assert (
        build_fill_event_payload({"fill_price": 100.0}, fallback_fill_ts=fallback_ts)
        is None
    )


def test_startup_recovery_closes_missing_local_open(tmp_path) -> None:
    """Ensure local-open orders absent at the broker receive an appended terminal event."""
...
def test_startup_recovery_adopts_broker_open_order_in_scope(tmp_path) -> None:
    """Ensure an in-scope broker-open order missing locally is adopted into audit history."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
...
def test_startup_recovery_fail_closed_on_broker_open_in_scope(tmp_path) -> None:
    """Ensure fail-closed startup rejects unresolved in-scope broker-open order state."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    client.listed.append(
        FakeOrder(
    ...
    with pytest.raises(
        ValueError, match="Broker open orders exist in configured universe"
    ):
        run_startup_recovery(
            event_store=store,
            broker=broker,
...
def test_clean_start_closes_local_open_orders_without_touching_broker(tmp_path) -> None:
    """Ensure operator clean-start appends local closures without requesting broker mutation."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    base_ts = datetime(2026, 1, 20, tzinfo=timezone.utc)
    store.record_event(
...
def test_report_identifies_out_of_scope_broker_orders(tmp_path) -> None:
    """Ensure recovery reports broker positions outside the configured trading universe."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    client.listed.append(
        FakeOrder(

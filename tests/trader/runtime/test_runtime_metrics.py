"""Contracts for computing and shaping runtime portfolio metrics.

Subject: Valuation, exposure, returns, drawdown, snapshot records, broker normalization, and price query plans.
Level: Deterministic numerical and persistence-planning unit contracts.
Collaborators: Real metrics helpers with fixed positions, provider mappings, portfolio state, and database-shaped rows.
Guarantees: Available values produce stable metrics and lineage while absent state or prices remain explicit.
Non-goals: Sampling workers, database execution, broker calls, time-series analysis, or promotion policy.
"""
...
from trader.runtime.metrics import (
    _latest_price_lookup_from_rows,
    _latest_price_query_plan,
)
...
def test_compute_metrics_sample_values_positions_without_mutation() -> None:
    """Ensure priced positions produce exposure and equity without mutating caller values."""
...
def test_compute_metrics_sample_uses_existing_baseline_and_peak() -> None:
    """Ensure retained baseline and peak values determine return and drawdown consistently."""
    ts = datetime(2026, 1, 20, 12, 0, tzinfo=timezone.utc)
...
def test_compute_metrics_sample_skips_empty_cashless_state() -> None:
    """Ensure absent positions and cash yield no misleading metrics sample."""
    ts = datetime(2026, 1, 20, 12, 0, tzinfo=timezone.utc)
...
def test_build_runtime_metrics_snapshot_record_serializes_payload() -> None:
    """Ensure computed metrics serialize with complete runtime lineage and universe context."""
    ts = datetime(2026, 1, 20, 12, 0, tzinfo=timezone.utc)
...
def test_positions_and_cash_from_portfolio_state_preserves_loaded_values() -> None:
    """Ensure local portfolio state passes typed positions and cash through unchanged."""
...
def test_positions_and_cash_from_broker_payload_normalizes_provider_values() -> None:
    """Ensure broker account and position mappings become canonical metric inputs."""
...
def test_latest_price_query_plan_uses_bounded_asset_class_table() -> None:
    """Ensure price lookup queries select the asset-specific table and bounded symbols."""
    stock_plan = _latest_price_query_plan(
        asset_class="stocks", symbols=(" aapl ", "MSFT")
    )
    crypto_plan = _latest_price_query_plan(
        asset_class="cryptocurrency", symbols=("BTC/USD",)
    )
...
def test_latest_price_query_plan_skips_empty_symbol_universe() -> None:
    """Ensure an empty symbol universe produces no unnecessary price query plan."""
...
def test_latest_price_lookup_from_rows_normalizes_prices() -> None:
    """Ensure database price rows become a normalized symbol-to-price mapping."""

"""Contracts for market-data query normalization, SQL parameters, and row mapping.

Subject: Canonical query values, predicate construction, positional parameters, and typed result records.
Level: Pure persistence-boundary unit contracts.
Collaborators: Real query-domain and SQL helpers supplied with fixed values and database-shaped rows.
Guarantees: Equivalent requests normalize identically and SQL or row ordering matches the declared contracts.
Non-goals: Executing queries, validating database schemas, provider access, ingestion, or quality analysis.
"""
...
def test_normalize_bar_query_canonicalizes_symbols_and_defaults_limit() -> None:
    """Ensure equivalent symbols, aliases, times, and empty filters normalize canonically."""
...
def test_bar_query_sql_predicate_and_params_include_optional_source() -> None:
    """Ensure optional source filtering extends SQL and parameters in matching order."""
    query = normalize_bar_query(
        BarQuery(
            symbols=("DEMO", "OTHER"),
            asset_class="stocks",
...
def test_symbol_discovery_normalization_and_sql_params_are_deterministic() -> None:
    """Ensure discovery inputs normalize into reproducible predicates and positional values."""
...
def test_row_mappers_return_typed_market_data_records() -> None:
    """Ensure database-shaped rows become typed bars and coverage-aware symbol records."""

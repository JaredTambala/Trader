"""Contracts for ingesting provider bars and making them available to execution.

Subject: Bar ingestion ordering, staleness, provider normalization, and stock or crypto persistence.
Level: Deterministic unit and DuckDB-backed in-process integration contracts.
Collaborators: Real market-data and cycle code, standard no-op policies, injected provider fakes, and DuckDB.
Guarantees: Normalized bars persist before strategy use while stale inputs suppress execution explicitly.
Non-goals: Provider networking, Postgres notifications, historical backfill, quality analysis, or live trading.
"""
...
class ProbeStrategy(Strategy):
    ...
    def generate_orders(
        ...
        *,
        ...
        self.calls += 1
        assert any(
            event_type == "stock_bar_events"
            for event_type, _ in self._event_store.events
        )
        return []

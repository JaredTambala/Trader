"""Contracts for querying bounded market-data inventory and stored bars.

Subject: Counts, ranges, sources, symbol discovery, ordered bar retrieval, validation, and adapter errors.
Level: DuckDB-backed adapter integration and deterministic boundary-validation contracts.
Collaborators: Real query services, the checked-in sample dataset, shared DuckDB support, and a no-op store.
Guarantees: Normalized bounds select stable inventory evidence and invalid or unqueryable inputs fail explicitly.
Non-goals: Postgres query planning, live providers, ingestion, quality classification, or backtest replay.
"""
...
def test_market_data_queries_count_rows_ranges_sources_and_symbols(
    tmp_path: Path,
) -> None:
    """Ensure one bounded query returns aligned row, range, source, and symbol evidence."""
    ...
    assert [
        (item.symbol, item.first_ts, item.last_ts)
        for item in fetch_bar_ranges(store, query)
    ] == [
        (
            "DEMO",
    ...
    assert [
        (item.symbol, item.source, item.row_count)
        for item in count_bar_sources(store, query)
    ] == [
        ("DEMO", "sample", 12),
    ]


def test_market_data_queries_discover_local_symbols_with_coverage(
    tmp_path: Path,
) -> None:
    """Ensure local symbol discovery returns matching coverage and provenance metadata."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    load_sample_market_data_csv(store, SAMPLE_CSV)
...
def test_fetch_bars_is_bounded_and_ordered(tmp_path: Path) -> None:
    """Ensure fetched bars respect the limit and remain ordered by event time."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    load_sample_market_data_csv(store, SAMPLE_CSV)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _query(symbols=tuple(f"SYM{index}" for index in range(21))),
            "at most 20 symbols",
        ),
...
def test_normalize_bar_query_validates_inputs(query: BarQuery, message: str) -> None:
    """Ensure unsafe query dimensions fail with an actionable boundary-specific reason."""
...
def test_market_data_queries_require_queryable_connection() -> None:
    """Ensure stores without a query connection fail before attempting SQL execution."""

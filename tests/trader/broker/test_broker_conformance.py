"""Protect the contracts shared by internal and Alpaca paper brokers.

Subject: Canonical responses and optional broker capabilities exposed through the core broker protocols.
Level: In-process contract tests with deterministic broker adapters.
Collaborators: Real broker implementations, protocol runtime checks, and a fake Alpaca trading client.
Guarantees: Implementations expose compatible response fields, capabilities, and normalized failures.
Non-goals: Provider networking, event-store persistence, reconciliation history, or execution quality.
"""
...
def test_internal_broker_response_shape_includes_canonical_fields() -> None:
    """Return the canonical filled-order shape with configured slippage and fees."""
    broker = InternalPaperBroker(
        slippage_bps=10.0, fee_bps=5.0, fee_minimum=1.0, sleep_on_fill_delay=False
    )
...
def test_alpaca_broker_exposes_optional_capabilities() -> None:
    """Expose account, lookup, cancellation, and reconciliation through runtime protocols."""
...
def test_alpaca_fake_client_conformance_for_accepted_lookup_cancel_and_partial_fill() -> (
    None
):
    """Preserve canonical semantics across submission, lookup, partial fill, and cancellation."""
    client = FakeTradingClient()
    broker = AlpacaPaperBroker(
        api_key="key",
        secret_key="secret",
...
def test_alpaca_submit_error_is_normalized() -> None:
    """Convert a provider submission exception into a canonical error response."""
...
def test_build_alpaca_submission_error_response_is_deterministic() -> None:
    """Build stable plain-data failure evidence from an Alpaca submission exception."""

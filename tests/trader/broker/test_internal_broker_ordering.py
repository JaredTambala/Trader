"""Verify deterministic execution records produced by the internal paper broker.

Subject: Internal order normalization, fills, rejections, and lifecycle-event ordering.
Level: Deterministic unit and in-process lifecycle integration contracts.
Collaborators: Real internal broker builders, cycle recorders, and an in-memory event collector.
Guarantees: Canonical requests produce correctly ordered, priced, and shaped execution evidence.
Non-goals: External brokers, database persistence, concurrent cycles, or market-liquidity simulation.
"""
...
def test_internal_broker_fill_becomes_terminal_order_state() -> None:
    """Append a fill after submission so the lifecycle ends in terminal state."""
    ...
    _record_order_events(
        store, [order], status="validated", event_ts=base_ts + timedelta(seconds=1)
    )
    _record_order_events(
        store, [order], status="submitted", event_ts=base_ts + timedelta(seconds=2)
    )
...
def test_normalize_internal_order_returns_typed_request_without_mutating_input() -> (
    None
):
    """Normalize valid input into a typed request while rejecting invalid sides safely."""
    order = {
...
def test_build_internal_fill_response_calculates_partial_sell_fill_deterministically() -> (
    None
):
    """Calculate a partial sell fill with deterministic latency, slippage, and fees."""
...
def test_build_internal_rejection_response_uses_canonical_mapping_shape() -> None:
    """Represent an internal rejection with the canonical order-response mapping shape."""

"""Exercise the Alpaca paper-broker boundary without contacting Alpaca.

Subject: Request, response, idempotency, and reconciliation behavior for the Alpaca paper broker.
Level: Deterministic unit and DuckDB-backed adapter-integration contracts.
Collaborators: Real broker/domain code, a fake trading client, and temporary DuckDB event stores.
Guarantees: Provider data is normalized, caller inputs stay intact, and reconciliation persists canonical events.
Non-goals: Alpaca authentication, provider-network behavior, rate limits, or live and paper order placement.
Cohesion rationale: These layers jointly define one adapter boundary and share the same provider-shaped fakes.
"""
...
class FakeTradingClient:
    ...
    def submit_order(self, order_data: object) -> FakeOrder:
        ...
        order_type = getattr(order_data, "order_type", None) or getattr(
            order_data, "type", None
        )
...
def test_build_alpaca_reconciliation_order_event_is_deterministic_without_mutation() -> (
    None
):
    """Build the same canonical order event without rewriting caller-owned evidence."""
...
def test_build_alpaca_reconciliation_fill_event_requires_fill_evidence() -> None:
    """Require quantity and price before emitting a normalized reconciliation fill."""
    ...
    payload = build_alpaca_reconciliation_fill_event(
        local_event, broker_order, fill_ts=fill_ts
    )
    ...
    assert (
        build_alpaca_reconciliation_fill_event(
            local_event, {"fill_qty": 1.0}, fill_ts=fill_ts
        )
        is None
    )
    assert (
        build_alpaca_reconciliation_fill_event(
            local_event, {"fill_price": 100.0}, fill_ts=fill_ts
        )
        is None
    )
...
def test_plan_alpaca_reconciliation_event_builds_order_and_fill_updates() -> None:
    """Translate a filled broker order into synchronized order and fill updates."""
...
def test_plan_alpaca_reconciliation_event_handles_missing_and_unchanged_orders() -> (
    None
):
    """Cancel vanished orders while suppressing updates for unchanged broker state."""
    fallback_ts = datetime(2026, 1, 20, 12, 0, tzinfo=timezone.utc)
    local_event = {
    ...
    assert (
        missing_plan.order_event.to_record()["rejection_reason"] == "reconciled_missing"
    )
...
def test_normalize_alpaca_order_request_fields_converts_crypto_order_for_trading_api() -> (
    None
):
    """Convert crypto symbols and time-in-force values to Alpaca request semantics."""
    order = {
        "symbol": "btc/usd",
        "side": " BUY ",
        "qty": "0.25",
...
def test_normalize_alpaca_order_request_fields_preserves_stock_day_order() -> None:
    """Preserve equity day orders while normalizing casing and numeric fields."""
...
def test_ensure_alpaca_client_order_id_preserves_explicit_id_without_mutation() -> None:
    """Keep an explicit client order identifier and the original mapping untouched."""
    order = {
...
def test_ensure_alpaca_client_order_id_derives_stable_id_without_mutation() -> None:
    """Derive a stable identifier on a copy without normalizing caller fields."""
    order = {
        "cycle_id": "cycle_1",
        "symbol": " aapl ",
        "side": " BUY ",
    ...
    assert enriched["client_order_id"] == deterministic_client_order_id(
        "cycle_1", "AAPL", "buy", 1.0
    )
    assert enriched["symbol"] == " aapl "


def test_normalize_alpaca_order_response_maps_provider_payload_without_mutation() -> (
    None
):
    """Map a provider order into Trader conventions without mutating its payload."""
    response = {
        "id": "alpaca_1",
        "status": "filled",
        "symbol": "BTCUSD",
...
def test_normalize_alpaca_position_maps_provider_object_and_mapping() -> None:
    """Normalize object and mapping positions consistently across supported asset classes."""
...
def test_normalize_alpaca_account_maps_object_and_mapping() -> None:
    """Project object and mapping accounts onto the same canonical account fields."""
    account = type(
        "Account", (), {"cash": "1000", "buying_power": "2000", "equity": "3000"}
    )()
    ...
    assert normalize_alpaca_account(
        {"cash": "1", "buying_power": "2", "equity": "3"}
    ) == {
        "cash": "1",
        "buying_power": "2",
        "equity": "3",
...
def test_latest_alpaca_order_events_from_rows_deduplicates_newest_first_rows() -> None:
    """Keep the newest complete order event for each client order identifier."""
    rows = [
        (
            "cid_1",
            "run",
...
def test_alpaca_order_lookup_query_and_row_normalization_are_pure() -> None:
    """Build dialect-correct lookups and normalize optional rows without side effects."""
...
def test_latest_alpaca_order_events_query_names_expected_projection() -> None:
    """Pin the projection and newest-first ordering consumed by reconciliation logic."""
...
def test_alpaca_broker_idempotent_submission(tmp_path) -> None:
    """Resubmit when local history exists but no broker-side order can be reconciled."""
...
def test_alpaca_broker_reconcile_updates_events(tmp_path) -> None:
    """Persist a reconciled fill as a terminal order and one fill event."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    broker = AlpacaPaperBroker(
        api_key="key",
    ...
    statuses = {
        ...
        .execute("SELECT status FROM order_events WHERE client_order_id = 'cid_2'")
        .fetchall()
    }
    assert "filled" in statuses
    fill_count = (
        store.connection()
        .execute("SELECT COUNT(*) FROM fill_events WHERE client_order_id = 'cid_2'")
        .fetchone()[0]
    )
    assert fill_count == 1
...
def test_alpaca_broker_reconcile_missing_closes_open_order(tmp_path) -> None:
    """Close a locally open order when Alpaca has no matching order evidence."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    broker = AlpacaPaperBroker(
        api_key="key",
...
def test_alpaca_submit_does_not_list_orders_without_broker_order_id(tmp_path) -> None:
    """Avoid provider lookups for local-only events and submit a replacement order."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    client = FakeTradingClient()
    broker = AlpacaPaperBroker(
        api_key="key",
...
def test_alpaca_status_mapping() -> None:
    """Map Alpaca lifecycle values onto Trader's canonical broker status vocabulary."""
    broker = AlpacaPaperBroker(
        api_key="key",
        secret_key="secret",
        client=FakeTradingClient(),

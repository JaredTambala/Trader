"""Contracts for normalizing records at the Postgres event-store boundary.

Subject: Insert plans, lifecycle parameters, JSON and array values, queries, and row-to-record mapping.
Level: Pure persistence-boundary unit contracts.
Collaborators: Real record-normalization helpers with fixed mappings, rows, timestamps, and identifiers.
Guarantees: Domain values map to ordered SQL parameters and stored rows return canonical plain-data records.
Non-goals: Executing SQL, managing transactions, creating schemas, or evaluating lifecycle policy.
"""
...
def test_postgres_event_insert_plan_rejects_unknown_event_types() -> None:
    """Ensure unsupported event tables fail explicitly before any SQL is constructed."""
...
def test_json_payload_helpers_serialize_optional_payloads() -> None:
    ...
    assert (
        json_payload_or_none({"ts": timestamp}) == '{"ts": "2026-01-21 12:00:00+00:00"}'
    )
    assert json_payload_or_empty(None) == "{}"
    assert (
        json_payload_or_empty({"ts": timestamp})
        == '{"ts": "2026-01-21 12:00:00+00:00"}'
    )
...
def test_run_session_sql_parameters_normalize_config_and_symbols() -> None:
    """Ensure run-session configuration and symbols normalize into the declared statement order."""
...
def test_trading_session_sql_parameters_include_strategy_id() -> None:
    """Ensure trading-session parameters add strategy identity without disturbing shared ordering."""
    started_at = datetime(2026, 1, 21, 12, 0, tzinfo=timezone.utc)
    finished_at = datetime(2026, 1, 21, 12, 5, tzinfo=timezone.utc)

"""Contracts for buffering and dispatching event-store writes.

Subject: Buffered write planning, lifecycle payloads, dispatch, queue limits, and flush behavior.
Level: Deterministic unit and in-process adapter contracts.
Collaborators: Real buffering helpers, an in-memory recording store, fixed clocks, and controlled workers.
Guarantees: Buffered writes retain canonical payloads, ordering, backpressure policy, and surfaced failures.
Non-goals: Database persistence, process recovery, concurrent producers, or throughput qualification.
"""
...
class RecordingStore(EventStore):
    ...
    def record_event(self, event_type: str, payload: Mapping[str, object]) -> None:
        ...
        self.calls.append(
            ("record_event", {"event_type": event_type, "payload": dict(payload)})
        )
...
def test_plan_buffered_event_write_preserves_ordinary_events() -> None:
    """Ensure non-reserved events retain their type and payload as ordinary writes."""

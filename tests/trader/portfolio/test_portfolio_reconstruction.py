"""Contracts for reconstructing typed portfolio state from persisted rows.

Subject: Position normalization, invalid rows, ordering, optional cash, and complete state reconstruction.
Level: Pure persistence-boundary unit contracts.
Collaborators: Real reconstruction helpers with fixed database-shaped rows and immutable position values.
Guarantees: Stored numeric values become ordered typed state and malformed identity fails with context.
Non-goals: Running queries, recording snapshots, choosing latest rows, state mutation, or broker synchronization.
"""
...
def test_position_from_snapshot_row_normalizes_numeric_fields() -> None:
    ...
    assert position_from_snapshot_row(("AAPL", "2", "100.5")) == Position(
        "AAPL", 2.0, 100.5
    )
...
def test_positions_from_snapshot_rows_preserves_row_order() -> None:
    """Ensure multi-row reconstruction retains the ordering selected by persistence queries."""
...
def test_cash_balance_from_snapshot_row_handles_empty_rows() -> None:
    """Ensure missing and null latest-cash rows remain explicit optional values."""

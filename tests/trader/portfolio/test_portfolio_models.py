"""Contracts for immutable portfolio values and their supported import surface.

Subject: Position and snapshot identity, transition exports, order validation, and signed quantity deltas.
Level: Pure domain and public-surface unit contracts.
Collaborators: Real portfolio model, snapshot, transition, facade, and core-surface objects only.
Guarantees: Public identities remain singular and invalid normalized orders fail before state transitions.
Non-goals: Raw input normalization, arithmetic implementation, persistence, broker payloads, or runtime recovery.
"""
...
def test_portfolio_order_signed_qty_delta() -> None:
    """Ensure order side determines the canonical signed quantity delta."""

"""Contracts for normalizing raw portfolio order mappings.

Subject: Symbol, side, quantity, price, and fee normalization plus explicit invalid-input behavior.
Level: Deterministic boundary unit contracts.
Collaborators: The real order-input normalizer with fixed mappings and price lookups.
Guarantees: Accepted mappings become typed canonical inputs while ignored or invalid values remain explicit.
Non-goals: Applying transitions, portfolio mutation, broker validation, persistence, or market-price discovery.
"""
...
from trader.portfolio.order_inputs import (
    PortfolioOrderInput,
...
def test_normalize_portfolio_order_inputs_skips_blank_symbols_and_non_positive_qty() -> (
    None
):

"""Contracts for recording portfolio snapshots through the event-store boundary.

Subject: Position-event payloads, snapshot-method delegation, and supported persistence-loader imports.
Level: Deterministic persistence-shell and public-surface unit contracts.
Collaborators: Real persistence helpers, immutable snapshots, and an in-memory recording event store.
Guarantees: Snapshot writes retain lineage and cash evidence while public loaders resolve to one implementation.
Non-goals: Executing database queries, reconstructing rows, transaction failures, or runtime reconciliation.
"""
...
from trader.portfolio.core import (
    load_latest_portfolio_state as core_load_latest_portfolio_state,
)
from trader.portfolio.core import load_latest_positions as core_load_latest_positions
from trader.portfolio.persistence import (
    load_latest_cash,
...
def test_portfolio_snapshot_persist_delegates_for_existing_callers() -> None:
    """Ensure the snapshot method delegates to the single explicit persistence helper."""
    store = RecordingEventStore(events=[])
    snapshot = PortfolioSnapshot(
        asof_ts=datetime(2025, 1, 1, tzinfo=timezone.utc),
        positions=(),

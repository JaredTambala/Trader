"""Protect the HTTP compatibility surface for UI-triggered backtests.

Subject: Starting a backtest and reading its in-process progress and result resources.
Level: FastAPI adapter-integration contracts.
Collaborators: Real routes and TestClient with a synchronous thread stub and fake worker.
Guarantees: Requests receive stable identifiers, progress payloads, cached results, and missing-result errors.
Non-goals: Real backtest execution, durable job state, authentication, concurrency, or process recovery.
"""
...
def test_post_backtest_returns_run_id(api_client: TestClient) -> None:
    """Start the stubbed worker and return its generated run identifier."""
    payload = {
        "symbols": ["AAPL"],
        "timeframe": "1Min",
...
def test_get_backtest_progress(api_client: TestClient) -> None:
    """Return the current status and progress metadata for a known run."""
...
def test_get_backtest_result_returns_cached_payload(api_client: TestClient) -> None:
    """Return a completed run's cached result through the result endpoint."""
...
def test_get_backtest_result_404_when_missing(api_client: TestClient) -> None:
    """Return HTTP 404 when neither memory nor persistence contains the run."""

"""Contracts for composing and executing the maintained SMA strategy.

Subject: Sufficient-history entry, cycle signal persistence, and negative-signal exit behavior.
Level: Temporary-DuckDB strategy and cycle integration contracts.
Collaborators: Real SMA indicator, signal generator, strategy, core cycle, static bars, and no-op risk.
Guarantees: Bar history produces directionally correct orders and leaves inspectable signal evidence.
Non-goals: Provider data, Postgres, parameter optimization, profitability, broker fills, or live operation.
"""
...
def test_sma_strategy_emits_signal_when_sufficient_bars(tmp_path: Path) -> None:
    """Ensure sufficient rising history produces one maintained SMA buy decision."""
    ...
    strategy = SimpleStrategy(
        signal_generator=generator, primary_signal="sma_crossover"
    )
    orders = list(
...
def test_cycle_persists_sma_signals(tmp_path: Path) -> None:
    """Ensure core cycle composition persists the SMA signal used for its decision."""
...
def test_simple_strategy_emits_sell_order_on_negative_signal(tmp_path: Path) -> None:
    """Ensure a negative SMA signal emits a sell for the held quantity."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    now = datetime.now(timezone.utc)
    bars = [

"""Contracts for the maintained indicator, signal, policy, and strategy library.

Subject: Indicator math, signal semantics, protective state, compositions, cycle evidence, and backtest support.
Level: Deterministic unit and temporary-DuckDB workflow integration contracts.
Collaborators: Real standard implementations with core cycle/backtest APIs, fixed bars, and shared DuckDB support.
Guarantees: Maintained compositions produce distinct reproducible decisions and preserve signal/indicator evidence.
Non-goals: Statistical profitability, live providers, Postgres, broker realism, research admission, or optimization.
"""
...
from trader_standard.indicators import (
...
def _seed_stock_bars(
    store: DuckDBEventStore, symbol: str, closes: list[float]
) -> list[StockBarEvent]:
...
def test_indicator_helpers_compute_expected_values() -> None:
    """Ensure maintained EMA, RSI, MACD, and Bollinger calculations match fixed series."""
    ...
    rsi_series = RsiIndicator(period=5).compute_series(
        _bars_from_closes([1, 2, 3, 4, 5, 6])
    )
    ...
    assert macd_observation.to_payload()["value"]["histogram"] == pytest.approx(
        macd_series[0].histogram
    )
...
def test_signals_cover_crossovers_thresholds_and_band_reentry() -> None:
    """Ensure maintained signals translate indicator states into their documented directional values."""
    ema_signal = EmaCrossoverSignal(
        fast=EmaIndicator(period=2), slow=EmaIndicator(period=4)
    )
    assert (
        ema_signal.compute(
            _bars_from_closes([8.43, 10.27, 9.22, 10.62, 10.75, 7.39, 7.08, 12.02])
        )
        == 1.0
    )
    assert (
        ema_signal.compute(
            _bars_from_closes([8.56, 8.41, 12.97, 9.82, 12.02, 9.86, 10.83, 7.9])
        )
        == -1.0
    )

    rsi_signal = RsiThresholdSignal(
        indicator=RsiIndicator(period=5), oversold=30.0, overbought=70.0
    )
    ...
    macd_signal = MacdCrossoverSignal(
        indicator=MacdIndicator(fast_period=3, slow_period=6, signal_period=3)
    )
    assert (
        macd_signal.compute(
            _bars_from_closes(
    ...
    bollinger_signal = BollingerBandSignal(
        indicator=BollingerBandsIndicator(period=5, stddev_multiplier=1.0)
    )
    assert (
        bollinger_signal.compute(_bars_from_closes([10, 10, 10, 10, 10, 7, 8])) == 1.0
    )


def test_trailing_stop_policy_tracks_high_water_and_resets() -> None:
    """Ensure trailing protection retains per-symbol highs and resets after a flat state."""
...
def test_strategy_stop_precedes_entry_and_flattens_existing_long(
    tmp_path: Path,
) -> None:
    """Ensure protective exits outrank entry signals and flatten the existing long position."""
    ...
    portfolio = Portfolio(
        positions={"AAPL": Position(symbol="AAPL", qty=1.0, avg_price=10.0)},
        cash_balance=0.0,
    )
...
def test_strategy_does_not_pyramid_existing_long(tmp_path: Path) -> None:
    """Ensure a repeated positive entry signal cannot increase an existing long position."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    _seed_stock_bars(store, "AAPL", [10, 10, 10, 10, 10, 8])
    strategy = build_mean_reversion_strategy(
        symbols=["AAPL"],
    ...
    portfolio = Portfolio(
        positions={"AAPL": Position(symbol="AAPL", qty=1.0, avg_price=8.0)},
        cash_balance=0.0,
    )
...
def test_built_in_compositions_behave_differently_on_same_bars(tmp_path: Path) -> None:
    """Ensure trend and mean-reversion compositions remain behaviorally distinct on identical bars."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))
    _seed_stock_bars(
        store,
        "AAPL",
        [9.77, 10.18, 9.94, 12.55, 10.01, 11.99, 9.12, 12.3, 12.4, 9.77, 10.41, 12.52],
    )
...
def test_bollinger_band_strategy_enters_and_exits_on_reversion(tmp_path: Path) -> None:
    """Ensure the Bollinger composition enters outside and exits after band reversion."""
...
def test_run_cycle_persists_signal_and_indicator_events_for_library_strategy(
    tmp_path: Path,
) -> None:
    """Ensure cycle execution records both maintained signal and indicator decision evidence."""
    config = _config(tmp_path)
    store = DuckDBEventStore(config.db_path)
    events = _seed_stock_bars(
        store,
        "AAPL",
        [9.77, 10.18, 9.94, 12.55, 10.01, 11.99, 9.12, 12.3, 12.4, 9.77, 10.41, 12.52],
    )
    ...
    assert {"macd_line", "signal_line", "histogram"} <= set(
        macd_payloads[0]["metadata"]
    )


def test_backtest_runner_supports_policy_driven_strategy(tmp_path: Path) -> None:
    """Ensure a maintained policy-driven strategy composes with the core backtest runner."""
    store = DuckDBEventStore(str(tmp_path / "events.duckdb"))

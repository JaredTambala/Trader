"""Deterministic MCP evidence graph for canonical parameter optimization."""
...
def test_mcp_optimization_holdout_and_adversarial_evidence_graph(tmp_path: Path) -> None:
    ...
    async def _run() -> None:
            ...
            await server.call_tool(
                RESEARCH_REGISTER_STRATEGY_IMPLEMENTATION_TOOL,
                {
                    ...
                    "parameter_schema": {
                        "type": "object",
                        "properties": {
                            "period": {"type": "integer", "minimum": 2, "maximum": 3, "default": 2}
                        },
                    },
        ...
        risk_stack_validation = _data(
            await server.call_tool(
                RESEARCH_VALIDATE_RISK_STACK_SPECIFICATION_TOOL,
                {"risk_stack_specification_id": risk_stack["risk_stack_specification_id"]},
            ),
        ...
        plan = _data(
            await server.call_tool(
                RESEARCH_CREATE_PARAMETER_OPTIMIZATION_PLAN_TOOL,
                {
                    "base_backtest_specification_validation_ref": selection_backtest_validation["validation_id"],
        ...
        optimization_result = await server.call_tool(
            RESEARCH_RUN_PARAMETER_OPTIMIZATION_TOOL,
            {"optimization_plan_ref": plan["optimization_plan_id"], "optimizer_profile": "builtin_grid"},
        )
        ...
        holdout_validation = await _create_validated_backtest(
            server,
            strategy_validation_id=optimization["selected_child_refs"]["strategy_specification_validation_id"],
            risk_validation_id=optimization["selected_child_refs"]["risk_stack_specification_validation_id"],
        ...
        assert holdout_run["risk_lineage"][0]["implementation_version_id"] == risk[
        ...
        audit = _data(
            await server.call_tool(
                ...
                {
                    "audit_plan_ref": audit_plan["audit_plan_id"],
                    "variant_optimization_run_refs": [item["optimization_run_id"] for item in variants],
                },
            ),
            "parameter_optimization_robustness_report",
        )
        assert audit["status"] == "passed"
        assert artifact_store.load_artifact(
            "parameter_optimization_run", optimization["optimization_run_id"]
        )["selected_trial_id"] == optimization["selected_trial_id"]
...
def _scope(
    ...
    return _jsonable(inventory.data["dataset_manifest"]), _jsonable(quality.data["data_quality_report"])

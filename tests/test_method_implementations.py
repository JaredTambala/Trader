...
def test_maintained_indicator_registration_is_knowledge_provenance_neutral(tmp_path: Path) -> None:
...
def test_maintained_method_contract_catalog_is_immutable_and_store_independent(tmp_path: Path) -> None:
    store = JsonKnowledgeStore(tmp_path / "artifacts")
    listed = math_list_method_contracts()
    valid = math_validate_method_contract(
        artifact_root=tmp_path / "artifacts",
        method_contract={"method_id": "sma", "parameters": {"period": 5}, "no_lookahead": True},
    )
    invalid = math_validate_method_contract(
        artifact_root=tmp_path / "artifacts",
        method_contract={"method_id": "sma", "parameters": {"period": 1}, "no_lookahead": True},
    )

    assert listed.ok is True
    assert "sma" in {method["method_id"] for method in listed.data["methods"]}
    assert valid.ok is True
    assert invalid.data["method_validation_report"]["blockers"] == ["parameter period is below minimum 2.0"]
    assert not hasattr(store, "save_method_contract")


def test_evidence_required_indicator_uses_approved_card_read_port(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    method_id = "bollinger_wma_band_rule"
    method_card_id = "method_card_bollinger_wma_band_rule_algorithmic_trading_v1"
    reader = _approved_reader(artifact_root, method_id, method_card_id, family="indicator")
    ...
    assert validated.ok is True, validated.to_dict()
    assert validated.data["indicator_validation_report"]["fixture_results"][0]["actual"][2]["middle"] == 2.0
...
def test_evidence_required_signal_registers_and_validates(tmp_path: Path) -> None:
    ...
    assert isinstance(BollingerBwmaActionSignal(period=20, stddev_multiplier=2.0), Signal)
    ...
    assert registered.ok is True, registered.to_dict()
    assert validated.ok is True, validated.to_dict()
    assert [row["actual"] for row in validated.data["signal_implementation_validation_report"]["fixture_results"]] == [
        1.0,
        -1.0,
        0.0,
    ]
...
def test_signal_fixture_failure_remains_inspectable(tmp_path: Path) -> None:
    ...
    result = math_run_signal_fixtures(
        artifact_root=artifact_root,
        implementation_manifest=registered.data["method_implementation_manifest"],
        fixtures=[{"fixture_id": "insufficient_bars", "closes": [10.0] * 19, "expected": 0.0}],
        approved_card_reader=reader,
    )
...
def test_registration_rejects_unknown_card_and_source_hash_drift(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    reader = _approved_reader(artifact_root, "sma", "method_card_sma_validated_v1", family="indicator")

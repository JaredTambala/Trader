"""Pure tests for runtime service configuration helpers."""
...
def test_parse_market_data_notify_returns_none_for_generic_or_invalid_payloads(payload: str) -> None:
...
def test_validate_startup_recovery_mode_preserves_exact_supported_values() -> None:
    ...
    with pytest.raises(ValueError, match="startup recovery mode must be 'resume' or 'fail_closed'"):
        validate_startup_recovery_mode("RESUME")


def test_build_notify_cycle_config_scopes_symbol_timeframe_and_asset_class() -> None:
    config = _config(market_data_symbols=("MSFT",), market_data_asset_class="stocks", strategy_timeframe="5Min")
...
def test_build_notify_cycle_config_requires_symbol_and_defaults_missing_fields() -> None:
    config = _config(market_data_symbols=("MSFT",), market_data_asset_class="stocks", strategy_timeframe="5Min")
...
def test_resolve_metrics_worker_settings_returns_none_when_disabled() -> None:
    assert resolve_metrics_worker_settings(_config(metrics_interval_seconds=0), run_id="run") is None
    assert resolve_metrics_worker_settings(_config(metrics_interval_seconds=None), run_id="run") is None
...
def test_parse_initial_positions_and_cash() -> None:
    ...
    assert [(position.symbol, position.qty, position.avg_price) for position in positions] == [
        ("AAPL", 2.0, 100.5),
        ("MSFT", 3.0, None),
    ]
...
def test_resolve_portfolio_source_precedence() -> None:
    assert resolve_portfolio_source(_config(trader_service_portfolio_source="db"), {"trader_service": {"portfolio_source": "alpaca"}}) == "db"
    assert resolve_portfolio_source(_config(), {"trader_service": {"portfolio_source": "alpaca"}}) == "alpaca"
...
def test_deduplicate_market_data_notify_returns_next_state_without_mutating_input() -> None:
    ...
    first = deduplicate_market_data_notify(
        {"symbol": "AAPL", "timeframe": "1Min", "asset_class": "stocks", "ts": first_ts},
        original_seen,
    )
    ...
    newer = deduplicate_market_data_notify(
        {"symbol": "AAPL", "timeframe": "1Min", "asset_class": "stocks", "ts": second_ts},
        first.last_seen,
    )
...
def test_deduplicate_market_data_notify_runs_unkeyed_payload_without_state_change() -> None:
    last_seen = {("AAPL", "1Min", "stocks"): datetime(2026, 1, 1, 12, tzinfo=timezone.utc)}
...
def test_decide_order_reconciliation_preserves_disabled_and_unavailable_states() -> None:

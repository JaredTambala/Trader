"""Tests for pure portfolio snapshot record and query helpers."""
...
def test_build_position_snapshot_events_emits_one_record_per_position() -> None:
    """Ensure position snapshots preserve sorted/caller-provided position values."""
    timestamp = datetime(2026, 1, 21, 12, 0, tzinfo=timezone.utc)

    events = build_position_snapshot_events(
        asof_ts=timestamp,
        positions=(PositionInput("AAPL", 2.0, 100.0), PositionInput("MSFT", -1.0, 250.0)),
        cash_balance=500.0,
        run_id="run_1",
        cycle_id="cycle_1",
    ...
    assert [event.event_type for event in events] == ["position_snapshots", "position_snapshots"]
...
def test_latest_cash_query_plan_omits_params_when_unbounded() -> None:
    """Ensure unbounded cash queries need no parameters."""

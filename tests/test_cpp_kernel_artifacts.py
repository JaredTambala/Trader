...
def test_generate_cpp_kernel_requires_validated_indicator_manifest(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
...
def test_compile_cpp_kernel_reports_missing_compiler_and_tampered_source(tmp_path: Path) -> None:
    artifact_root = tmp_path / "artifacts"
    generated = math_generate_cpp_kernel(
        artifact_root=artifact_root,
        implementation_manifest=_validated_sma_manifest(artifact_root),
    ...
    source_path = Path(generated.data["cxx_kernel_manifest"]["generated_source"]["path"])
...
def test_compile_cpp_kernel_persists_build_log_on_compiler_error(tmp_path: Path) -> None:
    if shutil.which("c++") is None:
        pytest.skip("local C++ compiler not available")
    artifact_root = tmp_path / "artifacts"
    generated = math_generate_cpp_kernel(

...
from tests.support.agentic_scale import (
...
_COMMANDS = {
    "AGENTIC_RUNTIME_ISOLATION": [
        "uv run pytest tests/test_postgres_agentic_runtime_isolation.py -m postgres -q -W error",
    ],
    "AGENTIC_CORE_CHECKS": [
        "uv run ruff check src tests",
        "uv run mypy src/trader_agents src/trader_mcp src/trader_research tests/support/agentic_*.py tests/support/mcp_agentic_qualification_server.py tests/test_agent_runtime_foundation.py tests/test_agentic_campaign.py tests/test_agentic_faults.py tests/test_agentic_governance.py tests/test_agentic_observation.py tests/test_agentic_qualification.py tests/test_agentic_sandbox_qualification.py tests/test_agentic_scenario_inputs.py tests/test_postgres_agentic_acceptance.py tests/test_postgres_agentic_bounded_scale.py tests/test_postgres_agentic_end_to_end.py tests/test_postgres_agentic_recovery.py tests/test_postgres_agentic_runtime_isolation.py tests/test_postgres_agentic_security.py",
        "uv run pytest -m 'not postgres' -q -W error",
    ],
    "AGENTIC_POSTGRES_E2E": [
        "uv run pytest tests/test_postgres_agentic_end_to_end.py -m postgres -q -W error -s",
    ],
    "AGENTIC_RECOVERY": [
        "uv run pytest tests/test_postgres_agentic_recovery.py -m postgres -q -W error -s",
    ],
    "AGENTIC_SECURITY": [
        "uv run pytest tests/test_agentic_governance.py tests/test_agent_runtime_foundation.py tests/test_agentic_qualification.py tests/test_package_boundaries.py -m 'not postgres' -q -W error",
        "uv run pytest tests/test_postgres_agentic_security.py -m postgres -q -W error -s",
    ],
    "AGENTIC_SANDBOX": [
        "uv run pytest tests/test_agentic_sandbox_qualification.py -q -W error -s",
    ],
    "AGENTIC_REAL_MODEL": [
        "uv run pytest tests/test_agentic_real_model_campaign.py -q -W error -s",
    ],
    "AGENTIC_BOUNDED_SCALE": [
        "uv run pytest tests/test_postgres_agentic_bounded_scale.py -m postgres -q -W error -s",
    ],
    "AGENTIC_ACCEPTANCE": [
        "uv run pytest tests/test_postgres_agentic_acceptance.py -m postgres -q -W error -s",
    ],
}

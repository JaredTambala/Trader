"""Unit tests for configurable console delivery of public agent events.

Subject: Agent-owned configuration, rendering, filtering, sequencing, and fan-out for console observability.
Level: Unit contract.
Collaborators: Real event builders, emitter, and in-memory text/recording sinks; no runtime or external service.
Guarantees: INFO and DEBUG settings produce ordered, correlated, parseable, and severity-safe console output.
Non-goals: Full agent execution, MCP child-process logging, durable trace export, and provider behavior.
"""

"""Contract tests for sink-neutral public agent observability events.

Subject: Agent-owned event semantics, correlation, redaction, safe projection, and in-memory sink contracts.
Level: In-process contract.
Collaborators: Real public Agent contracts and observability adapters with no runtime or external service.
Guarantees: Every emitted event has fixed semantics, bounded safe fields, explicit trust stages, and stable ordering.
Non-goals: Console presentation, complete agent trajectories, MLflow storage, and provider or MCP execution.
"""
...
from trader_agents.observability.tracing import NoOpTraceSink
...
def test_event_contract_fixes_visibility_authority_and_json_encoding() -> None:
    """An accepted semantic name fixes visibility, authority, and stable JSON output."""

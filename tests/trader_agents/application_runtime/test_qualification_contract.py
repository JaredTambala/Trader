"""Contracts for frozen Agent identity and campaign acceptance evidence.

Subject: Application-owned identity manifests, scenario-result schemas, completeness scoring, and fail-closed acceptance.
Level: In-process qualification contract.
Collaborators: Real profile/program/tool/fixture manifests and deterministic result objects; no runtime execution.
Guarantees: Only one exact, traceable, digest-pinned, complete, and safe campaign can satisfy the evidence contract.
Non-goals: Producing campaign results, exercising models or tools, persistence, and final release approval.
"""
...
def test_agentic_identity_manifest_pins_model_program_tools_fixture_and_image() -> None:
    """Build one immutable identity that pins every credential-free qualification input."""
...
def test_campaign_fails_closed_for_missing_duplicate_or_unsafe_result() -> None:
    """Reject missing, duplicate, or unsafe results before campaign acceptance is possible."""

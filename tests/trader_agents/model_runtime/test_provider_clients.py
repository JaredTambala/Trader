"""Unit contracts for provider-neutral Agent model clients.

Subject: Environment composition, request translation, static responses, and exact Ollama model identity checks.
Level: Unit adapter contract.
Collaborators: Real client adapters with a deterministic in-memory HTTP transport; no network or provider service.
Guarantees: Invalid configuration fails early and admitted requests use the required endpoint, schema, limits, and digest.
Non-goals: Structured output validation, semantic model quality, retry policy, and live provider availability.
"""
...
from trader_agents.model_runtime.client import (
...
def test_build_llm_client_from_env_requires_provider() -> None:
    """An absent provider fails before any model transport can be created."""
...
def test_runtime_configured_llm_client_respects_empty_environment() -> None:
    """Lazy runtime composition reports missing provider configuration on first use."""
...
def test_build_llm_client_from_env_rejects_unsupported_provider() -> None:
    """An unknown provider name cannot silently select a different adapter."""
...
def test_static_json_llm_client_returns_configured_json_and_records_requests() -> None:
    """The deterministic client returns queued JSON and records its public request."""
...
def test_openrouter_style_client_uses_openai_compatible_endpoint() -> None:
    """OpenRouter configuration produces the required authenticated OpenAI-compatible request."""
...
def test_ollama_client_uses_local_chat_endpoint() -> None:
    """Ollama configuration binds JSON mode and bounded generation to local chat."""
    transport = FakeTransport(
        {
            "message": {
                "content": '{"action": "finish", "reason": "complete"}',
...
def test_ollama_client_verifies_admitted_model_digest_before_chat() -> None:
    """A matching local model digest permits exactly one subsequent chat request."""
    digest = "a" * 64
    transport = FakeTransport(
        {
            "message": {
...
def test_ollama_client_rejects_model_digest_drift_before_chat() -> None:
    """Digest drift rejects the local model before any completion request occurs."""
    transport = FakeTransport(
        {
            "message": {
                "content": '{"action": "finish", "reason": "complete"}',

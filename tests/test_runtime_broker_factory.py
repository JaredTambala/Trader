"""Tests for runtime broker construction."""
...
def test_build_runtime_broker_returns_noop_for_unknown_or_noop_type() -> None:
    ...
    assert isinstance(build_runtime_broker(_config(broker_type="unknown"), NoOpEventStore()), NoOpBroker)


def test_build_runtime_broker_returns_internal_paper_broker_for_sim_aliases() -> None:
    assert isinstance(build_runtime_broker(_config(broker_type="internal"), NoOpEventStore()), InternalPaperBroker)
    assert isinstance(build_runtime_broker(_config(broker_type="paper"), NoOpEventStore()), InternalPaperBroker)
    assert isinstance(build_runtime_broker(_config(broker_type="sim"), NoOpEventStore()), InternalPaperBroker)

"""Workflow contracts for local knowledge registration, ingestion, retrieval, and extraction.

Subject: The filesystem-backed application path from bounded source registration to citeable evidence.
Level: Offline workflow contract.
Collaborators: Real services, JSON store, deterministic embeddings, temporary files, and local PDF parsing.
Guarantees: Sources validate, ingestion publishes atomically, evidence dereferences, and extraction warns safely.
Non-goals: Postgres, provider network calls, methodology interpretation, MCP transport, or agent research quality.
"""
...
from trader_research.knowledge.extractors import (
    ExtractedDocument,
    ExtractedSection,
    extract_text,
)
...
def test_source_registration_detects_duplicates_and_rejects_bad_paths(
    tmp_path: Path,
) -> None:
    """Source registration reports duplicates and rejects unsupported types, labels, and roots."""
...
def test_ingest_retrieve_and_validate_citations(tmp_path: Path) -> None:
    """Registered text can be ingested, retrieved, boundedly dereferenced, and citation-validated end to end."""
    artifact_root = tmp_path / "artifacts"
    ...
    assert (
        registered.data["knowledge_source_manifest"]["source_type"] == "primary_paper"
    )
    ...
    bad_citation = validate_citations(
        artifact_root=artifact_root,
        artifact={
            "knowledge_evidence_refs": [{"source_id": source_id, "chunk_id": "missing"}]
        },
    )
    ...
    dereferenced_chunk = dereferenced.data["evidence_chunk_dereference_report"][
        "chunks"
    ][0]
    ...
    assert (
        "simple moving average computes the arithmetic mean"
        in dereferenced_chunk["text"]
    )
...
def test_force_ingestion_replaces_chunks_without_loading_legacy_payloads(
    ...
    """Forced ingestion regenerates current evidence without first parsing an obsolete chunk payload."""
    artifact_root = tmp_path / "artifacts"
    ...
    def reject_legacy_load(_source_id: str):
        raise ValueError(
            "knowledge evidence unit must be regenerated with schema_version 2"
        )
...
def test_pdf_extraction_reports_scanned_page_warning(tmp_path: Path) -> None:
    """A blank PDF page yields no fabricated text and an explicit OCR-disabled warning."""
...
def test_extraction_replaces_invalid_unicode_surrogates(
    ...
    """Text extraction replaces invalid Unicode surrogates and reports the exact sanitization count."""
...
def test_ingestion_requires_configured_real_embedding_provider_by_default(
    tmp_path: Path,
) -> None:
    """Default ingestion fails explicitly when no runtime embedding provider has been configured."""
    artifact_root = tmp_path / "artifacts"
    registered = register_source(
        artifact_root=artifact_root,
        path=FIXTURE,
...
def test_force_ingestion_stages_embeddings_before_replacing_active_evidence(
    tmp_path: Path,
) -> None:
    """Failed replacement embeddings leave the previously active evidence generation unchanged."""
    artifact_root = tmp_path / "artifacts"
    ...
    source_path.write_text(
        "Replacement Method: entirely new source text.", encoding="utf-8"
    )

"""Adapter-integration test for the canonical Postgres knowledge store.

Subject: Source registration, atomic ingestion, hybrid retrieval, dereference, citation, and card persistence.
Level: Guarded Postgres adapter integration.
Collaborators: Real Postgres with pgvector, deterministic embeddings, and checked-in source text; no network.
Guarantees: The concrete adapter preserves the complete knowledge lifecycle and typed retrieval evidence.
Non-goals: Provider embeddings, concurrent ingestion, methodology-agent reasoning, MCP transport, or qualification.
"""
...
def test_postgres_knowledge_store_register_ingest_retrieve_validate(
    ...
    """Postgres preserves the complete source-to-retrieval lifecycle and published method-card lineage."""
    source = Path("tests/trader_research/knowledge/fixtures/sma_method.md")
    ...
    assert (
        ...
        in dereferenced.data["chunks"][0]["text"]
    )

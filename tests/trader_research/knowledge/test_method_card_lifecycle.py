"""Contracts for canonical evidence-backed method-card lifecycle behavior.

Subject: Draft creation, approval, revision lineage, retirement, search, and read-only access.
Level: Offline application workflow contract.
Collaborators: Knowledge domain values, JSON store, and in-memory canonical artifact records.
Guarantees: Cards require validated evidence, publish idempotently, and preserve immutable lineage.
Non-goals: Candidate discovery, computational implementation validation, Postgres, MCP, or agent decisions.
"""
...
def test_validated_candidate_creates_only_canonical_method_card_draft(
    tmp_path: Path,
) -> None:
    """A passed methodology candidate creates one canonical evidence-backed draft."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
    ...
    assert (
        payload["source_methodology_candidate_id"] == "methodology_candidate_pairs_card"
    )
...
def test_publish_preserves_complete_card_and_approved_reader_is_read_only(
    tmp_path: Path,
) -> None:
    """Publishing is idempotent and exposes approved cards through a read-only port."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
    ...
    assert reader.has_approved_method_card(
        ("method_card_pairs_v1",), method_id="cointegration_pairs_method"
    )
    assert not hasattr(reader, "save_method_card")


def test_method_card_lifecycle_and_revision_set_keep_complete_lineage(
    tmp_path: Path,
) -> None:
    """Successive approvals retain immutable revisions and identify the current card."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
    draft_v1 = _create_pairs_draft(
        artifact_root, store, artifact_store, validation, version=1
    )
    ...
    draft_v2 = _create_pairs_draft(
        artifact_root, store, artifact_store, validation, version=2
    )
    ...
    persisted = {
        card.method_card_id: card for card in store.list_persisted_method_cards()
    }
    ...
    assert (
        listing.data["method_card_sets"][0]["current_approved_method_card_id"]
        == "method_card_pairs_v2"
    )


def test_retired_card_is_hidden_but_remains_auditable(tmp_path: Path) -> None:
    """Retired cards leave active reads while remaining present in audit storage."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
    draft = _create_pairs_draft(artifact_root, store, artifact_store, validation)
    ...
    assert (
        stored["method_card_pairs_v1"].lineage["superseded_by_method_card_id"]
        == "method_card_pairs_v2"
    )


def test_draft_creation_fails_closed_for_invalid_lineage_and_missing_stores(
    tmp_path: Path,
) -> None:
    """Draft creation rejects blocked lineage and unavailable canonical stores."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
    blocked_validation = {
    ...
    assert (
        "research artifact store is required" in no_artifact_store.errors[0]["message"]
    )


def test_draft_creation_rejects_semantically_unsupported_overrides(
    tmp_path: Path,
) -> None:
    """Draft creation rejects identifiers and families unsupported by validated evidence."""
    artifact_root, store, artifact_store, validation = _validated_pairs_context(
        tmp_path
    )
...
def _store_with_evidence(
    artifact_root: Path,
) -> tuple[JsonKnowledgeStore, dict[str, object]]:
...
def _persist_pairs_methodology_validation(
    ...
    candidate = MethodologyCandidate(
        ...
        core_fields={
            ...
            "risk_validation": {
                "assumptions": field(
                    "spread is mean reverting after relationship validation"
                ),

"""Unit contracts for canonical knowledge and methodology domain values.

Subject: Evidence, sources, chunks, method candidates, evidence packets, cards, and validation records.
Level: In-process unit contract.
Collaborators: Real immutable knowledge values and deterministic chunking; no stores or external services.
Guarantees: Domain payloads are bounded, JSON-safe, evidence-backed, and closed to unsupported schema fields.
Non-goals: Filesystem ingestion, retrieval ranking, persistence, MCP adaptation, or agent interpretation.
"""
...
def _field(value: object) -> EvidenceBackedField:
    return EvidenceBackedField(
        value=value, evidence_refs=(_evidence_ref(),), confidence=0.8, quality="direct"
    )


def test_evidence_claim_span_round_trips_with_exact_hash() -> None:
    """A claim span round-trips exactly and rejects text whose recorded hash is stale."""
...
def test_knowledge_artifacts_round_trip_json() -> None:
    """The foundational knowledge artifact families round-trip together through one JSON-safe payload."""
    source = KnowledgeSourceManifest(
        source_id="knowledge_source_demo",
        title="Demo Source",
        source_type="method_textbook",
        path="tests/trader_research/knowledge/fixtures/sma_method.md",
        file_hash="abc123",
        file_size_bytes=42,
        topics=("indicators",),
    ...
    assert (
        KnowledgeSourceManifest.from_dict(payload["source"]).to_dict()["source_id"]
        == source.source_id
    )
    assert (
        KnowledgeChunk.from_dict(payload["chunk"]).to_dict()["chunk_id"]
        == chunk.chunk_id
    )
    assert (
        KnowledgeEmbeddingManifest.from_dict(payload["embedding"]).to_dict()[
            "dimension"
        ]
        == 32
    )
    assert (
        KnowledgeIngestionReport.from_dict(payload["report"]).to_dict()[
            "chunks_indexed"
        ]
        == 1
    )
    ...
    assert (
        MethodCardSet.from_dict(
            payload["method_card_set"]
...
def test_source_type_labels_are_closed_registry() -> None:
    """Source classification accepts only the maintained labels and retains the conservative default."""
...
def test_chunking_sanitizes_nul_bytes_before_hashing() -> None:
    """Chunk construction removes database-invalid NUL bytes before computing canonical text identity."""
    source = KnowledgeSourceManifest(
...
def test_chunking_creates_target_sized_evidence_units_with_labels_and_neighbors() -> (
    None
):
    """Chunking separates adjacent methods while retaining labels, section identity, and neighbor links."""
    source = KnowledgeSourceManifest(
...
def test_methodology_candidate_round_trips_rich_nullable_fields() -> None:
    """A methodology candidate preserves evidenced values and explicitly nullable rich-schema fields."""
    candidate = MethodologyCandidate(
        ...
        candidate_spans=(
            {"chunk_id": "knowledge_chunk_demo", "start": 20, "end": 220},
        ),
        core_fields={
            "identity": {
            ...
            "method_specification": {
                "algorithm_steps": _field(
                    ("estimate hedge ratio", "test residual stationarity")
                ),
            },
        },
        extension_fields={
...
def test_methodology_evidence_packet_round_trips_role_evidence() -> None:
    """An evidence packet retains role requirements, source locations, and candidate lineage through serialization."""
    packet = MethodologyEvidencePacket(
        ...
        status="assembled",
        candidate_ref={
            "uri": "research://postgres/methodology_candidate/methodology_candidate_pairs_demo"
        },
        source_ids=("knowledge_source_demo",),
        chunk_ids=("knowledge_chunk_demo",),
        role_evidence=(
...
def test_method_card_round_trips_and_derives_compact_summary() -> None:
    """A rich method card round-trips while deriving a deliberately smaller read-model summary."""
    card = MethodCard(
        ...
        core_fields={
            "signal_decision_logic": {
                "entry_rules": _field(
                    "enter when spread z-score exceeds the threshold"
                ),
            }
        },
        extension_fields={
...
def test_method_cards_require_explicit_set_lineage() -> None:
    """A method-card payload without stable set lineage is rejected before publication."""
    payload = {
...
def test_methodology_extraction_and_validation_reports_round_trip_json() -> None:
    """Extraction and validation reports retain evidence lineage in JSON-safe canonical representations."""
    extraction = MethodologyFieldExtractionReport(
        ...
        evidence_packet_id="methodology_evidence_packet_demo",
        candidate_ref={
            "uri": "research://postgres/methodology_candidate/methodology_candidate_demo"
        },
        source_ids=("knowledge_source_demo",),
        chunk_ids=("knowledge_chunk_demo",),
    ...
    assert (
        MethodologyFieldExtractionReport.from_dict(extraction_payload).to_dict()
        == extraction_payload
    )
    assert (
        MethodologyCandidateValidationReport.from_dict(validation_payload).to_dict()
        == validation_payload
    )
...
def test_populated_methodology_field_requires_evidence() -> None:
    """A populated methodology field cannot exist without at least one supporting evidence reference."""
...
def test_null_methodology_field_does_not_require_evidence() -> None:
    """An explicitly unknown methodology field remains null without manufacturing supporting evidence."""
...
def test_methodology_candidate_supports_domain_extension_blocks(
    ...
    """Each supported methodology family serializes only its declared extension-field vocabulary."""
    candidate = MethodologyCandidate(
...
def test_methodology_candidate_rejects_unknown_field_groups_and_names() -> None:
    """Unknown core groups and extension names fail instead of expanding the schema implicitly."""
    ...
    with pytest.raises(ValueError, match="unsupported extension_fields field"):
        MethodologyCandidate(
            ...
            families=("technical_indicators",),
            extension_fields={
                "technical_indicators": {"delta": _field("not a technical field")}
            },
        )

"""Contracts for candidate-family signal diagnostic reports.

Subject: Rank correlation, hit rate, quantiles, monotonicity, and turnover diagnostics.
Level: In-process computational contract.
Collaborators: Signal observations, forward-return labels, family manifests, and approved method evidence.
Guarantees: Diagnostics align observations deterministically and reject missing or duplicate evidence.
Non-goals: Trading recommendations, multiple-testing correction, backtests, or agent interpretation.
"""
...
def test_signal_diagnostics_evaluate_declared_signal_candidates(tmp_path: Path) -> None:
    """Diagnostics calculate bounded metrics for every declared candidate and horizon."""
...
def test_signal_diagnostics_fail_without_rank_ic_evidence(tmp_path: Path) -> None:
    """Diagnostics require an approved rank-correlation contract for each requested horizon."""
    result = math_run_signal_diagnostics(
        artifact_root=tmp_path / "artifacts",
        ...
        candidate_family_manifest={
            ...
            "candidates": [
                {"candidate_id": "action_signal", "signal_name": "Action Signal"}
            ],
...
def test_signal_diagnostics_fail_on_duplicate_observation_keys(tmp_path: Path) -> None:
    """Diagnostics reject duplicate candidate, symbol, and timestamp observation identities."""
    ...
    report = result.data["signal_diagnostic_report"]
    assert (
        "duplicate signal observation key: action_signal/AAA/2026-01-01T09:30:00+00:00"
        in report["blockers"]
    )

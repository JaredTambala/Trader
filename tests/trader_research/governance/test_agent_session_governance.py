"""Unit contracts for model-backed research-session governance.

Subject: Session identity, decision receipts, budgets, terminal states, and governed artifact reads.
Level: In-process unit contract.
Collaborators: Real governance services with the in-memory canonical artifact store; no model or database.
Guarantees: Session and decision evidence is idempotent, ordered, bounded, exact, and fail-closed.
Non-goals: Agent reasoning quality, MCP transport, Postgres projections, or end-to-end orchestration.
"""
...
def test_session_creation_is_strict_idempotent_and_conflict_safe() -> None:
    """Equivalent session creation replays safely while the same identity rejects conflicting content."""
...
def test_decision_receipts_verify_evidence_sequence_and_budget() -> None:
    """Decision recording accepts ordered evidence within budget and makes the receipt retrievable."""
    store = InMemoryResearchArtifactStore()
    session = _session()
...
def test_cancelled_decision_is_terminal_and_requires_public_reason() -> None:
    """Canonical cancellation requires a public reason and blocks every later branch transition."""
    store = InMemoryResearchArtifactStore()
    session = _session()
    create_agent_session(session.to_dict(), artifact_store=store)
...
def test_decision_receipt_rejects_unapproved_program_and_budget_overrun() -> None:
    """Decision evidence rejects programs outside the session allowlist and cumulative budget overruns."""
    store = InMemoryResearchArtifactStore()
    session = _session()
    create_agent_session(session.to_dict(), artifact_store=store)
...
def test_canonical_read_is_exact_governed_and_size_bounded() -> None:
    """Canonical reads enforce exact identity, accepted status, authorization, and response size limits."""
    store = InMemoryResearchArtifactStore()

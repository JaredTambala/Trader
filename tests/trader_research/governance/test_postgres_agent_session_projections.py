"""Adapter-integration tests for Postgres agent-session projections.

Subject: Governance-owned research sessions and decision receipts projected by the artifact store.
Level: Adapter integration with one offline schema contract.
Collaborators: Real schema declarations and guarded Postgres; no model, MCP transport, or agent runtime.
Guarantees: Required tables are declared and canonical session evidence is queryable through typed columns.
Non-goals: Coordinator behavior, process recovery, model quality, or full orchestration qualification.
"""
...
def test_research_schema_exposes_agentic_records_to_pgadmin() -> None:
    """The offline schema declares dedicated tables for sessions and decision receipts."""
...
def test_agentic_session_and_receipt_have_typed_postgres_projections(
    ...
    """Persisted session evidence is queryable through stable typed Postgres projection columns."""

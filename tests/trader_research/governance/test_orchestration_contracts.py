"""Contract tests for research orchestration plans and protocol governance.

Subject: Objectives, approved experiment protocols, workflow plans, approvals, and step results.
Level: In-process contract.
Collaborators: Real governance value objects and validators; no coordinator, tool transport, or persistence.
Guarantees: Orchestration inputs are bounded, internally consistent, JSON-safe, and explicit about readiness.
Non-goals: Executing a workflow, evaluating model choices, persisting records, or invoking research tools.
"""
...
def test_research_objective_and_approved_protocol_round_trip_json() -> None:
    """An approved objective and protocol round-trip with constraints, datasets, and approval evidence intact."""
...
def test_approved_protocol_rejects_unresolved_assumptions_and_unsealed_holdout() -> (
    None
):
    """Approved protocols reject unresolved decisions, missing holdouts, and temporally overlapping dataset roles."""
    protocol = _approved_protocol()
...
def test_protocol_rejects_invalid_or_undeclared_tunable_paths() -> None:
    """Optimization dimensions cannot target malformed or undeclared strategy and risk parameter paths."""
    ...
    payload = _approved_protocol().to_dict()
    payload["risk_managers"][0]["tunable_fields"] = ["/risk/1/parameters/max_orders"]
...
def test_workflow_plan_round_trip_validates_capabilities_slots_and_readiness() -> None:
    """A ready workflow round-trips with explicit capabilities, artifact slots, and readiness state."""
    plan = _ready_plan()
...
def test_workflow_plan_rejects_invented_capabilities_bad_bindings_and_cycles() -> None:
    """Workflow validation rejects unknown tools, incompatible bindings, and cyclic step dependencies."""
...
def test_ready_workflow_rejects_unresolved_prerequisites_and_approvals() -> None:
    """A ready plan cannot retain unresolved policy gates, approvals, or required input slots."""
    payload = _ready_plan().to_dict()
...
def test_workflow_step_result_is_bounded_json_and_requires_explicit_retry() -> None:
    """Step results serialize bounded public evidence and require explicit retry disposition on blockage."""
...
def test_approval_contract_is_canonical_and_decisions_are_explicit() -> None:
    """Approval requests remain undecided until a separate explicit decision supplies its rationale."""
    requested = Approval(
...
def test_capabilities_reject_runtime_state_and_non_research_side_effects() -> None:
    """Capability declarations reject resolved runtime state and side effects outside research authority."""

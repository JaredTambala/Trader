"""Contracts for typed Postgres projections of canonical optimisation evidence.

Subject: Optimisation schema shape and pgAdmin-visible plan, run, trial, and authority projections.
Level: Schema unit contract plus guarded Postgres adapter integration.
Collaborators: Research artifact schema and the concrete Postgres canonical artifact store.
Guarantees: Current typed tables exist and canonical optimisation records project without authority drift.
Non-goals: Running optimisation, provider storage, historical projections, or agent decisions.
"""
...
def test_research_schema_has_canonical_optimization_tables_without_retired_projections() -> (
    None
):
    """The schema declares current optimisation projections and excludes retired research tables."""
...
def test_optimization_artifacts_have_typed_pgadmin_visible_projections(
    ...
    """Canonical optimisation artifacts populate typed Postgres rows with preserved authority fields."""

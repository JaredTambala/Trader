"""Contracts for neutral maintained strategy and risk implementation templates.

Subject: Public maintained-template metadata, family filtering, and resolvable runtime entrypoints.
Level: In-process catalogue contract.
Collaborators: Experiments catalogue functions and maintained trader_standard runtime objects.
Guarantees: Templates expose real entrypoints without candidate-generation or admission coupling.
Non-goals: Source authoring, implementation validation, strategy execution, Postgres, or agents.
"""
...
def test_strategy_catalog_exposes_runtime_metadata_without_candidate_coupling() -> None:
    """Strategy templates expose maintained runtime metadata without candidate-authoring assumptions."""
...
def test_risk_catalog_contains_only_real_maintained_entrypoints() -> None:
    """Risk templates enumerate only maintained and importable runtime implementation entrypoints."""
...
def test_catalog_entrypoints_resolve_to_maintained_runtime_objects() -> None:
    """Every advertised maintained entrypoint resolves to an installed runtime object."""
    templates = [
...
def test_catalog_rejects_unknown_and_empty_filters() -> None:
    """Template catalogues reject unsupported families and explicitly empty filter sets."""

"""Unit contracts for canonical artifact projection dispatch.

Subject: The generic projection registry joining context-owned Postgres projection writers.
Level: In-process unit contract.
Collaborators: Real artifact records and registry logic with in-memory writer callables; no database connection.
Guarantees: Dispatch is type-specific, duplicate ownership fails, and the default registry covers each context.
Non-goals: SQL execution, schema correctness, durable artifact writes, or business-context behavior.
"""
...
def test_projection_registry_dispatches_only_registered_artifact_type() -> None:
    """Dispatch invokes only the writer registered for the record's artifact type."""
...
def test_projection_registry_rejects_duplicate_context_ownership() -> None:
    """Registry composition rejects two contexts claiming the same artifact projection."""
...
def test_default_projection_registry_is_partitioned_by_context() -> None:
    """The default registry includes representative artifact projections from every owning context."""

"""Tests for typed model-deployment bindings in canonical strategy specifications."""
...
class FixtureDeploymentReader:
    ...
    def __init__(self, *, semantics: str = "expected_return", model_version_id: str = "model_1") -> None:
...
def test_model_backed_strategy_specification_pins_typed_binding_and_revalidates() -> None:
    ...
    assert strategy["prediction_bindings"][0]["mapper"]["mapper_id"] == "identity_numeric:v1"
    ...
    assert validated.ok
    validation_id = validated.data["strategy_specification_validation_report"]["validation_id"]
...
def test_prediction_binding_rejects_missing_or_semantically_incompatible_deployment() -> None:
    store = InMemoryResearchArtifactStore()
    implementation_validation_id = _validated_implementation(store)
    catalog = MaintainedPredictionMapperCatalog()
    ...
    incompatible = create_strategy_specification(
        implementation_validation_ref=implementation_validation_id,
        prediction_bindings=_binding(),
        prediction_deployment_reader=FixtureDeploymentReader(semantics="class_probability"),
        prediction_mapper_catalog=catalog,
        artifact_store=store,
    )
...
def test_prediction_binding_validation_blocks_deployment_or_mapper_drift() -> None:
    ...
    drifted = validate_strategy_specification(
        strategy_specification_id=strategy["strategy_specification_id"],
        prediction_deployment_reader=FixtureDeploymentReader(model_version_id="model_2"),
        prediction_mapper_catalog=catalog,
        artifact_store=store,
    )
    ...
    tampered["prediction_bindings"][0]["mapper"]["parameters"]["target_name"] = "changed"

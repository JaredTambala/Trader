...
def test_mcp_open_world_methods_materialize_canonical_cards_and_fail_closed(tmp_path: Path) -> None:
    ...
    async def _run() -> None:
        ...
        technical_candidates = await _discover_by_family(server, source_id, "technical_indicators")
        statistical_candidates = await _discover_by_family(server, source_id, "statistical_arbitrage")
        assert {TECHNICAL_METHOD, ADJACENT_METHOD, INCOMPLETE_METHOD} <= set(technical_candidates)
...
async def _discover_by_family(server: Any, source_id: str, family: str) -> dict[str, str]:
...
async def _materialize_card(
    ...
    *,
    ...
    return {"packet": packet, "candidate": candidate, "draft": draft, "published_card": published_card}
...
def _assert_specific_technical_fields(candidate: dict[str, Any]) -> None:
    ...
    assert candidate["core_fields"]["signal_decision_logic"]["signal_definition"]["value"]
...
async def _assert_set_lineage(server: Any, published_card: dict[str, Any]) -> None:
    ...
    assert [item["method_card_set_id"] for item in listed.structuredContent["data"]["method_card_sets"]] == [
        method_card_set_id
    ]
...
async def _assert_missing_formula_role(server: Any, candidate_uri: str) -> None:
    assembled = await server.call_tool(
        KNOWLEDGE_ASSEMBLE_METHODOLOGY_EVIDENCE_TOOL,
        {"methodology_candidate_uri": candidate_uri, "readiness_goal": "implementation"},
    )
    assert assembled.isError is True
    packet = assembled.structuredContent["data"]["methodology_evidence_packet"]
    assert packet["status"] == "blocked"
    assert "formula_algorithm" in packet["missing_roles"]
    assert "missing required evidence role for implementation: formula_algorithm" in packet["blockers"]
...
async def _assert_cross_method_contamination_rejected(
    ...
    assert any("claim span" in blocker or "claim_span" in blocker for blocker in report["blockers"])

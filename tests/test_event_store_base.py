"""Tests for default EventStore lifecycle behavior."""
...
def test_default_run_session_methods_skip_session_records_for_backtests() -> None:
    ...
    store.record_run_session_finish("run_1", "backtest", timestamp, timestamp, "success", None)
...
def test_default_cycle_and_legacy_run_methods_emit_run_events() -> None:
    ...
    store.record_run_finish("legacy_run", "demo", "once", timestamp, timestamp, timestamp, "failed", "boom")

    assert [event_type for event_type, _ in store.events] == ["run_events", "run_events"]

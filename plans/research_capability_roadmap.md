...
| Capability frontier | Status | Hard dependencies | Current deliverable | Acceptance evidence |
| --- | --- | --- | --- | --- |
| Design and evaluation charter | in_progress | Current-state, dependency, and lineage audit | Review the temporary [First Agentic Implementation Slice plan](agent_designs/first_agentic_slice_implementation_plan.md), then complete the Coordinator, Data Research, and Strategy Engineering records plus only the shared authority, context, evaluation, sandbox, and pattern decisions required by the slice. Other specialist records continue later; ML is parked. | Every first-slice agent has an accepted complete record, slice-wide patterns preserve those boundaries, representative scenarios are labelled, and deferred decisions cannot leak into the slice. |
| MCP capability and trust-boundary inventory | ready | May proceed with the slice charter | First inventory Research Session, Data, implementation-catalogue, Coding Workspace, admission, and canonical-read capabilities required by the Coordinator–Data–Strategy slice; retain, redesign, split, remove, or add tools explicitly. Knowledge, RWFO, Evaluation, and MLflow inventories may follow. | First-slice inventory includes role, side effect, approval, idempotency, output/resource shape, missing capability, disposition, and qualification fixture for every exposed operation. |
| Framework and observability spike | blocked | Approved first-slice charter and representative scenario fixtures | Compare LangChain/LangGraph with a disposable PydanticAI implementation using a real model, the Coordinator plus Data Research and Strategy Engineering, MCP transport, Postgres resume, one parallel branch, one interrupt, structured outputs, and MLflow traces. | Recorded framework decision; materially different prompts produce different delegations; a blocker causes replanning; one specialist revises after tool evidence; the losing spike is removed. |
| Agent runtime foundation | blocked | Accepted framework spike | Clean `trader_agents` cutover with model profiles, agent-program identity, MCP adapters, role-aware dynamic tools, context policy, budgets, delegation/evidence-return/coordinator-decision contracts, branch lineage, loop guards, persistence, interrupts, and trace correlation. | Contract, policy, persistence, security, and fresh-process tests prove model/tool loops without direct platform access or replayed mutations. |
| Coordinator evidence-review loop | blocked | Agent runtime foundation | Every specialist return rejoins the coordinator; canonical refs are independently checked; the coordinator can advance, revise, revisit, fork, ask, conclude, or fail closed. | Poor/promising/stable/unstable scenarios produce evidence-consistent decisions; equivalent loops, exhausted budgets, contaminated evaluation, and out-of-authority actions stop before mutation. |
| First agentic implementation slice | blocked | Accepted first-slice charter, focused MCP inventory, accepted framework spike, Agent runtime foundation, BASE-DATA, BASE-IMPL | Research Coordinator, Data Research Agent, and Strategy Engineering Agent over role-scoped Data, implementation-catalogue, Coding Workspace, admission, session, and canonical-read capabilities. The provisional cutoff is an admitted strategy/risk candidate; experiment design/execution is the next extension. | Real-model natural-language tasks produce materially different agendas; multi-asset Data discovery/backfill/revalidation, implementation comparison, reuse/adapt/author choice, isolated code revision after failed admission, canonical handoff, restart, interrupt, prompt-injection resistance, bounded loops, and denied scope paths work end to end across repeated runs. |
...
| Data Research Agent architecture | in_review | Multi-asset composite scope, dynamic role-scoped MCP catalogue, data readiness/backfill mission, and pre-approved acquisition-envelope authority are accepted in the [Data Research design](agent_designs/data_research.md); the remaining standard record is pending. |
...
| Strategy Engineering Agent architecture | in_review | The [Strategy Engineering design](agent_designs/strategy_engineering.md) accepts typed build contracts, mandatory MCP comparison, exact-version trust/re-admission rules, an isolated Coding Workspace, normal outcome isolation, and a bounded evidence-bearing admission-repair loop. Search/retrieval/indexing gaps, exact schemas, and the remaining standard record stay open. |
...
| Specialist architecture records | pending | Responsibility rows are provisional until each specialist is reviewed through the complete standard record. |
| Shared agent design-pattern review | pending | Review supervisor/subagent, agent-as-tool, custom workflow, evaluator isolation, coding workspace, fan-out/join, and recovery patterns only after agent boundaries are accepted. |
...
1. Complete the Data Research and Strategy Engineering architecture records and the shared pattern/evaluation decisions
   needed by those agents and the already accepted Research Coordinator.
2. Complete the focused MCP inventory for Research Session, Data, implementation catalogue, Coding Workspace,
   admission, and canonical evidence reads; record every missing first-slice capability.
3. Define repeated real-model scenarios covering materially different agendas, multi-asset scope, permitted backfill,
   reuse/adapt/author decisions, failed-admission revision, interruption, restart, prompt injection, and fail-closed
   limits.
4. Run the disposable framework and observability spike with the slice agents and remove the losing framework.
5. Implement the minimum runtime foundation and production slice, then qualify it before extending into Experiment

...
| MCP capability and trust-boundary inventory | in_progress | Accepted slice charter | Implementation catalogue, isolated Coding Workspace, immutable Research Session/public-decision evidence, and bounded canonical-read operations are now registered. Complete the reviewed disposition and qualification-fixture inventory for these and the remaining composite Data/admission surfaces. Knowledge, RWFO, Evaluation, and MLflow inventories may follow. | First-slice inventory includes role, side effect, approval, idempotency, output/resource shape, missing capability, disposition, and qualification fixture for every exposed operation. |
...
2. Complete the remaining focused MCP inventory for Research Session, Data, admission, and canonical evidence reads;
   implementation-catalogue and Coding Workspace capabilities are now registered but not yet runtime-qualified.

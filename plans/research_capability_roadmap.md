...
| Agent runtime foundation | in_progress | Accepted framework spike | Clean `trader_agents` cutover is implemented with model profiles, agent-program identity, MCP adapters, role-aware dynamic tools, context policy, budgets, delegation/evidence-return/coordinator-decision contracts, branch lineage, semantic loop guards, bounded source-free coordinator and specialist Postgres state, interrupts, explicit owning-operator cancellation, trace correlation, runtime composition, and CLI. Focused tests prove specialist resume and coordinator decision-commit recovery through fresh PostgreSQL connections without repeating an accepted read or model decision. Provider backfill requires a costed matching dry-run plan, runtime-bound lineage, and a canonical prepared/terminal operation journal that suppresses provider replay; a non-destructive Postgres test also proves post-load recovery and exact terminal replay through new service connections. MCP result and decision-commit traces retain only allowlisted public identities and evidence/error summaries, and a local MLflow test proves that public correlation fields are queryable without source content. Production MLflow, cross-process cancellation, and operational qualification remain. | Contract, policy, persistence, security, and fresh-process tests prove model/tool loops without direct platform access or replayed mutations. |
| Coordinator evidence-review loop | in_progress | Agent runtime foundation | The implemented Coordinator rejoins every specialist return, independently reads canonical refs, checkpoints a validated decision before append-only receipt mutation, then applies the exact decision after the receipt is accepted. It can revise, revisit, fork, ask, conclude, cancel, or fail closed. Agenda policy admits disjoint Data and Strategy catalogue fan-out, enforces specialist-owned hard joins, derives mutation locks from trusted scope, and preserves unfinished delegation identity across a soft join. Semantic fingerprints ignore paraphrase and disposable IDs so an equivalent second transition stops with `low_information_loop`; revision decisions consume the coordinator revision budget. Broader behavioral and production-scale evidence remains. | Poor/promising/stable/unstable scenarios produce evidence-consistent decisions; equivalent loops, exhausted budgets, contaminated evaluation, and out-of-authority actions stop before mutation. |
| First agentic implementation slice | in_progress | Accepted first-slice charter, focused MCP inventory, accepted framework spike, Agent runtime foundation, BASE-DATA, BASE-IMPL | Research Coordinator, Data Research Agent, and Strategy Engineering Agent loops are implemented over role-scoped Data, implementation-catalogue, Coding Workspace, admission, session, and canonical-read capabilities. Multi-asset Data readiness, costed backfill plus revalidation, exact reuse, adaptation, isolated authoring/admission, parallel join, evidence verification, interrupt, cancellation, runtime, and CLI paths have focused tests. Strategy admission failure destroys the rejected workspace before one budgeted replacement attempt with a new immutable candidate identity. Coding checks require a complete digest-pinned image reference and enforce network, privilege, filesystem, process, resource, deadline, and output limits without a host fallback. Source-free checkpoints, replay-safe candidate/Data/coordinator mutations, immutable-package registration, prompt-injection denial, no-silent-substitution, repair-loop, and prohibited-trading-path tests close the covered local trust gaps. All 12 canonical scenarios map to executable scripted checks; real-container and repeated real-model qualification remain open. | Real-model natural-language tasks produce materially different agendas; multi-asset Data discovery/backfill/revalidation, implementation comparison, reuse/adapt/author choice, isolated code revision after failed admission, canonical handoff, restart, interrupt, prompt-injection resistance, bounded loops, and denied scope paths work end to end across repeated runs. |

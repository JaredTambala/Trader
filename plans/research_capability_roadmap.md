...
| First agentic implementation slice | in_progress | Accepted first-slice charter, focused MCP inventory, accepted framework spike, Agent runtime foundation, BASE-DATA, BASE-IMPL | Research Coordinator, Data Research Agent, and Strategy Engineering Agent loops are implemented over role-scoped Data, implementation-catalogue, Coding Workspace, admission, session, and canonical-read capabilities. Multi-asset Data readiness, costed backfill plus revalidation, exact reuse, adaptation, isolated authoring/admission, parallel join, evidence verification, interrupt, cancellation, runtime, and CLI paths have focused scripted tests. Strategy admission failure destroys the rejected workspace before one budgeted replacement attempt with a new immutable candidate identity. Coding checks use a real digest-pinned image and enforce network, privilege, filesystem, process, resource, deadline, output, and cleanup limits without host fallback. Source-free checkpoints, replay-safe candidate/Data/coordinator mutations, immutable-package registration, prompt-injection denial, no-silent-substitution, bounded candidate-revision, and prohibited-trading-path tests close the covered local trust gaps. The bounded contract-test register separates scripted conformance, model choice, production boundaries, and vertical behavior. The rejected Qwen profile remains historical evidence. The active LFM 2.5 8B profile selected only Data for three equivalent readiness briefs but failed the material-ambiguity brief by returning executable Data work. The combined Coordinator gate therefore remains failed, and its dependent Data model test was not run. Broader integration remains paused; no compensated pass counts as evidence. All 12 canonical scenarios still map to a no-selection 36-run campaign; four strict scale profiles and eight retained phase entry points are implemented. | Real-model natural-language tasks produce materially different specialist subsets and agendas; multi-asset Data discovery/backfill/revalidation, implementation comparison, reuse/adapt/author choice, isolated code revision after failed admission, canonical handoff, restart, interrupt, prompt-injection resistance, bounded loops, and denied scope paths work end to end across repeated runs. |
...
| Controlled agentic qualification | in_progress | Accepted end-to-end target surface | The closed profile, identity manifest, guarded Postgres controller, strict scenario/scale records, all eight pre-acceptance entry points, and canonical verifier are implemented. The `cc72ee7` candidate was invalidated by development end-to-end behavior before controlled phases began. Replacement model feasibility remains open because the active LFM profile failed its first complete Coordinator choice gate. Only after bounded behavior is green may one clean replacement freeze, exact image, isolated Postgres runtime, and unselected 36-run campaign be created. | Reviewed thresholds pass for brief interpretation, evidence review, routing, replanning, loop termination, scientific quality, grounding, independence, human authority, security, recovery, scale, cost, and latency. |
...
The selected delivery focus remains the first Coordinator–Data–Strategy capability slice, but integration follows the
[bounded contract-test implementation register](agent_designs/first_agentic_slice_implementation_plan.md#bounded-contract-test-implementation-register)
rather than treating all capabilities as one initial test. Work proceeds in this order:
...
3. Resolve the active LFM profile's failed material-ambiguity behavior through model/profile selection or a reviewed
   contract/design decision; do not add runtime transformations or coded routing.
4. Rerun the complete Coordinator choice gate, then isolate Data evidence judgment and Coordinator review with deterministic
   in-process evidence before combining them.
5. Independently prove the same Coordinator–Data handoff with scripted model outputs over stdio MCP and isolated
   Postgres, then combine the already-proven model and boundary contracts as the ready-Data vertical slice.
6. Extend that seam to negative Data evidence, approved backfill, and recovery, then separately prove
   Coordinator-to-Strategy exact reuse with zero Data activity using the same model/boundary/vertical separation.
7. Extend the Strategy seam to isolated adaptation/authorship, admission, revision, cleanup, and recovery.
8. Recompose Data and Strategy only after both single-specialist paths are stable; then prove parallel scheduling,
   joins, evidence attribution, and failure isolation.
9. Run a diverse development set, then qualify the clean production freeze across the unselected 12-scenario dataset
   with repeated real-model runs before extending into Experiment Design and coordinator-invoked deterministic

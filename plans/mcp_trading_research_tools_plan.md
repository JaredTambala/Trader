...
| 57. Reproducible Strategy, Risk, Backtest, And Optimisation Specifications | In progress | 57L's Postgres-only direct-service fixture passed controlled qualification; 57M is the next mandatory phase. | Immutable Postgres specifications, runs, trial ledgers, projections, Evaluation, and Adversarial refs compose without provider authority or filesystem identity; the slice is complete only after mandatory verification passes. |
...
| 55 | P | Mandatory graph is multi-asset and produces parameter-sensitive trades, costs, exposure, approvals, and rejections. | 57L's direct Postgres fixture proves the realistic behavior; the current public MCP graph remains one-symbol, twelve-bar, empty-strategy smoke coverage. | Partial / 57L direct Postgres fixture and 57M MCP graph |

...
| 57I. Freeze Revision And Build Acceptance Matrix | Done | `verification-57i-freeze-v3` resolves to `1dd4d3dcea85fbd17513ca1d0140f6b0bde21daf`. | The replacement freeze contains the 57N canonical ledger/lineage hardening, deterministic Postgres controls, tests, and active docs on top of the prior candidate cutover. |
| 57J. Provision Isolated Verification Runtime | Done | Controlled rerun on harness `d8420ba`: 4 Postgres guard tests passed; phase row has isolation/qualification `passed`, no blockers, and matching operator fingerprint `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`. | Requalified `trader_verification_test` and its no-migration control schema; all mutation gates remained false. |
| 57K. Static, Contract, And Regression Gate | Done | Controlled rerun on harness `d8420ba`: Ruff, compileall, mypy, 705 strict non-Postgres tests, 55 focused contract tests, and `git diff --check` passed; operator fingerprints matched. | Warnings-as-errors passed without an allowlist. Retired packages/models/readers are absent and protected by regressions. |
...
| 57L. Realistic Deterministic Evidence Fixture | Done | Controlled rerun on harness `d8420ba` passed the Postgres fixture in 9.14 seconds; the phase row records isolation/qualification `passed`, matching operator fingerprints, and no blockers. | Qualified the bounded direct-service strategy/risk/backtest/grid-selection/holdout path using only `PostgresEventStore` and `PostgresResearchArtifactStore`; 57M then composed the same fixture through public MCP, Evaluation, and Adversarial tools. |
| 57M. Postgres-Native MCP Evidence Graph | Done | Controlled stdio MCP graph passed in 16.55 seconds on harness `d8420ba`; Postgres retains 87 reconciled canonical artifacts, passed Evaluation and Adversarial reports, empty blockers, and matching operator fingerprints. | The graph uses only verification Postgres, public MCP calls, canonical `research://postgres/...` refs, exact typed-projection reconciliation, immutable selection checks, and guarded post-test evidence retention for pgAdmin inspection. |
| 57N. Determinism, Integrity, And Leakage Tests | Done | Controlled run on freeze `1dd4d3d` / harness `69f4bee`: clean A/B each retained 120 artifacts with digest `dacabc75a30a0c8b2d79d32d5dbba73fea14f2986cdfb651902f5be8e4bd9923`; all 14 mutations failed closed; operator fingerprints matched. | Grid order `2,3,4,5`, independently seeded random order `3,2,5,4`, observations/scores/selections/refs matched, selection reads ended before the database seal, and holdout reads began only after it. |
...
| TRR-12. Requalify Refactored Product | In progress | Target freeze tag `verification-57i-freeze-v4`; requires TRR-11. | Run static, package, MCP, Postgres, docs, and replacement 57I-N verification on the refactored freeze before resuming 57O. |
| 57O. Restart, Resume, And Fault-Injection Tests | Planned | Requires TRR-12 and replacement 57I-N evidence on the refactored freeze. | Interrupt after persisted phase boundaries, recreate service processes, resume without duplicate trials, exercise bounded retry/timeout/provider-loss paths, and prove partial or blocked runs remain inspectable and cannot be silently continued under changed configuration. |
...
2. `TRR-5` through `TRR-11` are complete. The no-compatibility bounded-context cutover is now the only supported
   research-layer package architecture. Its canonical record is the "`trader_research` Refactor Review And Plan"
   section in `docs/research_agents/architecture.md`.
3. `TRR-12`: create a new freeze and rerun replacement 57I-N evidence. Any `trader_research`, MCP, persistence, or agent
   change invalidates the current freeze as qualification evidence even though its historical results remain valid.
4. `57O-57S`: resume controlled core/Postgres acceptance on the refactored freeze; keep Optuna and each tracking sink
   disabled until its independent adapter profile passes.
5. `56A-56D` and `57A-57H` remain the implemented product behavior that the refactor must preserve:
...
6. `44` and `46`: generalise the delivered optimisation audit split so Adversarial plans and judges attacks while the
   Supervisor executes immutable backtest and optimisation variants.
7. `39A-39J`: add the MLflow lifecycle from point-in-time feature engineering and gated fitting through immutable model
   versions, deployment manifests, strategy integration, prediction events, and drift.
8. `58` and `59`: compose the provider-neutral optimisation protocol inside each fold and add independent
...
| TRR-12. Requalify Refactored Product | Replace revision-bound verification evidence after structural product changes. | new freeze/tag, static suites, Postgres guards/fixtures/MCP graph/determinism, acceptance inventory | In progress against target tag `verification-57i-freeze-v4`. Run Ruff, compileall, mypy, strict non-Postgres tests, package/docs/MCP contracts, Postgres projection reconciliation, and replacement 57I-N on the frozen revision. Review `git status --short` and residual paths. 57O remains blocked until this passes. |
| 57O. Restart, Resume, And Fault-Injection Tests | Verify canonical Postgres state is sufficient for recovery and that partial work is never misrepresented as complete. | Subprocess/service-recreation tests, injected executor/provider/store failures, resume assertions | Blocked until TRR-12 produces replacement 57I-N evidence on the refactored freeze. Then stop after plan persistence, after suggestions, after a child backtest, after a failed objective, and before final selection; recreate MCP/services with only canonical configuration and resume. Confirm completed trials are not duplicated, sequence numbers remain stable, retries are bounded and fully recorded, and deterministic selection matches an uninterrupted run. Exercise child timeout, invalid objective result, unavailable metric, artifact-store write failure, Optuna loss, and provider/configuration drift. Terminal evidence remains readable; unsafe resume yields explicit partial/blocked status and requires a new run rather than engine switching or state repair. |
...
The current replacement checkpoint revision is identified by annotated Git tag `verification-57i-freeze-v3` on branch
`experiment/agentic-build`. It resolves to the 57N-hardened product checkpoint after the inventory, matrix,
documentation regression, and checkpoint commit completed. Later verification records use the commit resolved by that
tag, not a moving branch name. The pre-cutover parent remains `40ade24` (`Complete semantic extraction and research
roadmap`).
...
| 53 | C | Evidence is tied to one frozen revision and downstream phases rerun after changes. | Checkpoint `1dd4d3d`, annotated `verification-57i-freeze-v3` tag, and controlled 57J-N rows all pin that revision and harness `69f4bee`. | Passed / replacement 57I-N |
...
Verification commits after `verification-57i-freeze-v3` may change only tests, active docs, and the tracker. The controlled

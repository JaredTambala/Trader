...
| 25. Strategy Candidate Schema and Template Catalog | Define the strategy candidate artifact and expose maintained templates that can consume validated signal/method packages. | `src/trader_research/strategies.py`, `src/trader_research/domain.py`, `tests/test_strategy_candidates.py` | Done. `strategy_candidate_manifest.json` records template family, method package refs, signal refs, entry/exit semantics, sizing, risk assumptions, data requirements, warnings, and blockers. Template discovery returns maintained families, parameters, defaults, constraints, and required artifact types without importing arbitrary strategy code. Direct service only; candidate creation remains 26 and MCP registration remains 27. |

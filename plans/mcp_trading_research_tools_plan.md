...
| 23C. Knowledge Text Extraction and Chunking | Done | `tests/test_knowledge_services.py` | Extract PDF/Markdown/text sources and create deterministic locator-preserving chunks without silent OCR. |
...
| 23G. Hybrid Retrieval and Citation Validation MCP Tools | Not started |  | Search approved method cards, run hybrid lexical/vector retrieval with deterministic rank fusion, return citeable evidence chunks, and validate source IDs, locators, source approval, method-card coverage, and claim support. |
...
math_validate_method_contract
  -> Postgres-backed source/chunk/embedding/index state, ingestion reports, hybrid retrieved evidence, citation validation, and method metadata
  -> declares agent_owner = Quantitative Methods Agent
  -> records source IDs, locators, embedding provider/model/index version, lexical/vector/combined retrieval scores, assumptions, fixture status, and failure modes
```

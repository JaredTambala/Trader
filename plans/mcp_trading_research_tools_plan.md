...
| 33E. Risk-Scoped Portfolio Backtest Tools | Run backtests over a multi-asset data scope with a validated strategy/risk stack. | `src/trader_research/backtests/`, `src/trader_mcp/research_tools.py`, tests | Add `research_run_portfolio_backtest` or extend backtest execution with an explicit stack-ref mode. The tool requires a passed strategy/risk stack validation report and exactly one Data Agent dataset manifest; persists stack refs, multi-symbol data scope, risk-manager decisions, exposure summaries, limit breaches, symbol-level metrics, trades, positions, curves, and provenance. |
| 33F. Portfolio And Risk Evaluation Reports | Make Evaluation reports reflect strategy plus risk stack behavior. | `src/trader_research/evaluation/performance.py`, `tests/test_performance_reports.py`, docs | Evaluation reports consume risk-scoped portfolio backtest bundles, summarize per-symbol and portfolio metrics, gross/net exposure, concentration, turnover, fees/slippage, risk-manager blockers/warnings, VaR/CVaR-style estimates when supplied, and risk-limit breach evidence. Reports block when portfolio evaluations omit required risk evidence. |

...
| 57. Reproducible Strategy, Risk, Backtest, And Optimisation Specifications | In progress | Controlled 57J-M passed on frozen product revision `767511c`; 57N implementation calibration passes and replacement frozen qualification is pending. | Immutable Postgres specifications, runs, trial ledgers, projections, Evaluation, and Adversarial refs compose through public stdio MCP. Canonical run reads now recompute complete ledger/selection integrity; restart, provider, policy, scale, and final acceptance phases 57O-S remain after controlled 57N. |
...
| 57N. Determinism, Integrity, And Leakage Tests | In progress | Calibration: repeated 120-artifact stdio graphs produced one normalized evidence digest; 14 source/config/snapshot/lineage mutations failed closed; audited selection reads ended before holdout. | Product hardening, Postgres controls, test harness, and active docs are implemented. Create replacement freeze, rerun 57J-N under controlled phase records, and retain final 57N evidence before marking done. |
...
| 57N. Determinism, Integrity, And Leakage Tests | Prove that reproducibility and holdout claims survive independent repetition and hostile state changes. | `tests/test_postgres_optimization_determinism_integrity.py`, `tests/support/postgres_57n.py`, canonical plan/run loaders | Implemented; controlled qualification pending. Two clean 120-artifact stdio graphs compare stable refs and normalized payload digests, excluding only wall-clock backtest finish/duration telemetry. Seeded-random order is independently recomputed; forced ties and persisted failed/retry attempts are covered directly. Fourteen mutations cover implementation source/hash, validation payload, strategy parameters, dataset/quality snapshot, costs, plan seed, provider digest, trial order/objective/child lineage, selected trial/child, and holdout lineage. Results, tracking, Evaluation, variants, and Adversarial consumers use the revalidating run loader. Audited SQL reads prove selection execution/objectives stop before holdout and holdout Evaluation begins after the database selection seal. |
...
| 53 | C | Evidence is tied to one frozen revision and downstream phases rerun after changes. | Checkpoint `1dd4d3d`, annotated `verification-57i-freeze-v3` tag; controlled 57J-N must be rerun and pinned to that revision. | In progress / replacement 57I-N |
| 54 | P | Tests require explicit `*_test` DB and leave operator runtime/research/knowledge unchanged. | Server/marker/role/locale checks before store construction and every truncate; controlled 57J-M phase rows have matching before/after digest `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`. | Passed / controlled 57J-M |
...
- `TRADER_VERIFICATION_RETAIN_PHASE=57M` left the evidence in `trader_verification_test` for pgAdmin inspection.
  This is not final release acceptance: 57N must still repeat clean graphs and attack determinism, lineage integrity,
  source/config drift, and holdout-access leakage before restart, provider, policy, scale, and final record phases.

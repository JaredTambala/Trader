...
| 53A. Research Documentation Reader Journey | Done | `docs/research_agents/README.md`, present-state architecture, implementation-to-evidence walkthrough, source-path and documentation regression checks. | Adds an explicit reading order and topic paths, separates current bounded-context architecture from historical TRR delivery detail, traces one supplied implementation through canonical Postgres evidence, and points ownership documentation at the current Governance modules. |
...
| 57. Reproducible Strategy, Risk, Backtest, And Optimisation Specifications | Done | Controlled Core and Trader Postgres acceptance passed on `verification-57i-freeze-v6`; `verification_control.acceptance_records` records the release decision. | Immutable Postgres specifications, runs, trial ledgers, projections, Evaluation, and Adversarial refs compose through public stdio MCP. Built-in grid/random are qualified; Optuna and MLflow tracking remain independently not qualified and gated off. |
...
| 57I. Freeze Revision And Build Acceptance Matrix | Done | `verification-57i-freeze-v6` resolves to `3cd5928533bb678cd387955a0efd7dd19f6d046a`. | Pins strict idempotent backtest recovery plus the recovery/security/deadline/scale/acceptance additions; no path under `src/trader` changed. |
| 57J. Provision Isolated Verification Runtime | Done | Controlled v6 runtime guards passed with empty blockers and unchanged operator fingerprint. | Recreated `trader_verification_test` and its no-migration control schema with all mutation gates false. |
| 57K. Static, Contract, And Regression Gate | Done | Controlled v6 Ruff, compileall, mypy, strict non-Postgres, focused MCP/docs/domain/package, and freeze-diff checks passed. | The frozen source surface is internally consistent; no path under `src/trader` changed. |
| 57K-R. Candidate Retirement And Warning Remediation | Done | Commit/tag `767511c` / `verification-57i-freeze-v2`; replacement 57J-K phase rows both passed with empty blockers. | Removed 11,070 lines of candidate-era code/contracts and retained only neutral read-only catalog discovery over real maintained entrypoints. |
| 57L. Realistic Deterministic Evidence Fixture | Done | Controlled v6 Postgres-only direct-service fixture passed with empty blockers. | Three-symbol strategy/risk/objective/specification/grid/holdout fixture is the locked qualification fixture. |
| 57M. Postgres-Native MCP Evidence Graph | Done | Controlled v6 public stdio graph passed and produced stable Evaluation/Adversarial refs. | Reproduced meaningful multi-asset selection/holdout, risk, cost, exposure, and immutable-selection evidence. |
| 57N. Determinism, Integrity, And Leakage Tests | Done | Controlled v6 clean repeats produced 120 artifacts and digest `702702a4...ab521`; all attacks and access audits passed. | Selection reads end before the database seal and holdout reads begin afterward over the disjoint region. |
| TRR-1. Inventory Current `trader_research` | Done | Read-only source/package/LOC/public-surface inventory recorded in `docs/research_agents/architecture.md`. | 77 Python files and 27,420 lines classified by capability; oversized modules, root workflows, facades, MCP consumers, and persistence surfaces identified. No product code changed. |
| TRR-2. Map Dependencies And Authority | Done | AST import review plus storage, MCP, agent, CLI, and documentation consumer review. | Identified the citation/card source cycle, facade-induced knowledge/method cycles, central root and Postgres hubs, deep MCP imports, filesystem leftovers, and provider boundaries. No product code changed. |
| TRR-3. Diagnose Comprehensibility Failures | Done | Findings recorded in the canonical architecture document. | Mixed lifecycle stages, duplicate method-card meanings, transport leakage, superseded workflows, terminology drift, central dispatch, and missing executable context boundaries are explicitly classified. No product code changed. |
| TRR-4. Approve Target Bounded-Context Design | Done | Five bounded domain contexts, a Foundation/Governance kernel, outer Infrastructure, one-way dependencies, vocabulary, no-compatibility policy, migration order, and acceptance rules documented. | This is the architecture gate. Production refactoring starts at TRR-5 only. |
| TRR-5. Retire Superseded Root Workflows | Done | Six research workflow modules, five root CLIs, four obsolete test modules, mypy entries, and active CLI guidance removed; package/docs boundary regressions pass. | Canonical MCP/Postgres research remains; generic `trader` event-store APIs and schema are unchanged. No aliases or replacement wrappers were added. |
| TRR-6. Extract Foundation, Governance, And Persistence | Done | Foundation/Governance packages, MCP-owned transport contracts, explicit artifact ownership, and registered Postgres projection writers; targeted boundary/contract suites and mypy pass. | Deleted all former root hubs without aliases; canonical JSONB and typed projections remain transactional; `src/trader` is unchanged. |
| TRR-7. Encapsulate Knowledge And Methodology Engineering | Done | Canonical card/domain/service splits, consolidated `methodology` package, approved-card read port, hard-cutover boundary tests, focused Quant Methods/MCP/docs suites. | Knowledge owns one evidence-backed method-card lifecycle; compact summaries are derived and non-writable. Methodology Engineering no longer depends on knowledge storage internals, maintained computational contracts are immutable catalog data, and generic implementation admission remains provenance-neutral. No `src/trader` code changed. |
| TRR-8. Decompose Data | Done | Split modules, public-facade/provider boundary tests, direct Data/MCP/agent suites, Ruff, compileall, and mypy. | Removed the 1,676-line `data/services.py` module without an alias; separated domain, catalog, inventory, quality, and loading responsibilities; moved the read-only Alpaca catalog adapter to `infrastructure.providers`; retained Data request/result payloads, deterministic IDs, MCP tool names, and core platform API usage. No `src/trader` code changed. |
| TRR-9. Consolidate Experimentation | Done | Hard-cutover import tests, provider-neutral optimization/MCP/projection suites, targeted mypy over 31 experiment/provider modules, Ruff, and compileall. | Moved implementation admission, specifications, backtests, optimization, and tracking under one `trader_research.experiments` facade; split the optimization service monolith into planning, orchestration, ledger, query, and shared-validation modules; moved Optuna and MLflow adapters to Infrastructure; preserved MCP names, payloads, IDs, Postgres authority, built-in grid/random behavior, and provider-independent reads. No `src/trader` code changed. |
| TRR-10. Isolate Independent Review | Done | Immutable-read-port tests, Review mutation AST guards, optimization Evaluation/Adversarial behavior suite, Ruff, compileall, and targeted mypy. | Moved Evaluation and Adversarial under one `trader_research.review` context without aliases; added `ExperimentEvidenceReader` and a store-backed adapter inside Experiments; removed Review imports of the broad Experiment facade and direct backtest/trial reads; and prohibited Review writes of implementation, specification, backtest, optimization, or tracking artifacts. No `src/trader` code changed. |
| TRR-11. Cut Over Composition Roots | Done | Public-facade and acyclic-context AST tests, clean-process Optuna/MLflow-absent MCP startup, complete MCP/agent ownership suites, Ruff, compileall, and mypy. | MCP now composes Data, Knowledge, Methodology, Experiments, and Review only through public facades; agents import public Foundation/Governance contracts only; optional and Postgres adapters remain outer infrastructure; old/deep context imports and lazy facade cycle workarounds are absent. Alpaca remains an unchanged core dependency. No `src/trader` code changed. |
| TRR-12. Requalify Refactored Product | Done | Freeze `verification-57i-freeze-v4` / `6f3efb4`; replacement 57J-N phase rows all passed with empty blockers and matching operator fingerprints. | Static, package, MCP, Postgres, docs, realistic fixture, public evidence graph, determinism, integrity, leakage, and retained-evidence checks qualify the refactored product. `src/trader` remained unchanged. |
| 57O. Restart, Resume, And Fault-Injection Tests | Done | Three controlled v6 Postgres cases passed: exact fresh-process recovery, injected write boundaries/retries/blocked reads, and enforced process deadlines. | Persisted child evidence is strictly revalidated and reused without row update, duplicate trial, or event replay. |
| 57P. Provider Independence And Adapter Qualification | Done | Mandatory fresh-process profile passed with Optuna and MLflow imports forcibly unavailable. | Built-in grid/random and canonical reads are qualified; optional adapters remain independently not qualified and gated off. |
| 57Q. Policy, Security, And Resource-Boundary Tests | Done | All 63 controlled admission, gate, closed-input, limit, deadline, bounded-error, and package-boundary cases passed. | Independent gates, forbidden ambient capabilities, immutable boundaries, budgets/deadlines, and bounded failures are enforced. |
| 57R. Projection, Operator, And Bounded-Scale Checks | Done | Controlled 64-grid, 100-random, and 3x1,000-bar profiles passed with full projection reconciliation and retained final MCP evidence. | Local timings and storage/query evidence are recorded in `verification_control.bounded_scale_results`; operator fingerprint was unchanged. |
| 57S. Acceptance Record And Release Decision | Done | `verification_control.acceptance_records` records `status="passed"` for freeze `3cd5928`. | Built-ins are qualified; Optuna/MLflow are `not_qualified`; residual security and local-scale limitations are explicit. |
...
1. `TRR-1` through `TRR-4` are complete as a read-only architecture gate. They inventory the current
   `trader_research` package, map dependencies and authority, diagnose the comprehensibility failures, and define the
   accepted target without changing production code.
2. `TRR-5` through `TRR-12` are complete. The no-compatibility bounded-context cutover is now the only supported
   research-layer package architecture. Its canonical record is the "`trader_research` Refactor Review And Plan"
   section in `docs/research_agents/architecture.md`. Freeze `verification-57i-freeze-v4` and retained replacement
   57J-N Postgres evidence are the historical qualification baseline.
3. `57I-57S` are complete on `verification-57i-freeze-v6`. Keep Optuna and each tracking sink disabled until its
   independent adapter profile passes.
4. `56A-56D` and `57A-57H` remain the implemented product behavior that the refactor preserves:
   implementation/specification intake, canonical backtests, provider-neutral optimization, independent review, and
   disposable tracking projection.
...
9. `41` and `42`: deepen attribution and skeptical Evaluation once the new baseline/variant evidence is available.
...
| `knowledge_validate_methodology_candidate` | Quantitative Methods Agent | `local_mutating` | Validate field-level citations, source suitability, unsupported-claim blockers, and family-specific minimum evidence before canonical method-card materialization; write DB-backed validation reports. |
| `knowledge_create_method_card_draft` | Quantitative Methods Agent | `local_mutating` | Materialize the one canonical evidence-backed method-card draft from a passed methodology-candidate validation report. Free-form and summary-only card writes are not supported. |
...
| 53A. Research Documentation Reader Journey | Make the current research product understandable without requiring contributors to reconstruct its refactor history. | `docs/research_agents/README.md`, `architecture.md`, `workflows.md`, ownership references, tracker, docs tests | Done. The README is the Start Here guide, architecture describes the current bounded contexts and links historical TRR evidence instead of embedding the migration, workflows contain a concrete implementation-to-evidence graph with proof limits, and active docs reference `governance/ownership.py` and `governance/artifacts.py` as the executable sources of truth. |
...
| 57I. Freeze Revision And Build Acceptance Matrix | Establish exactly what is being tested and prevent a moving worktree from invalidating conclusions. | Git checkpoint, `git status --short`, `git diff --stat`, registered-tool/config/artifact/projection inventories, invariant-to-test matrix | Done. `verification-57i-freeze-v6` resolves to `3cd5928533bb678cd387955a0efd7dd19f6d046a`; v4 and failed v5 evidence is historical, and all mandatory v6 phases passed. |
| 57J. Provision Isolated Verification Runtime | Make destructive and external integration tests safe, repeatable, and visibly separate from operator state. | `tests/conftest.py`, dedicated Trader verification DB, optional dedicated Optuna schema/role, disposable tracking experiment, environment manifest | Done on v6. Fresh provisioning and four runtime guards passed against `trader_verification_test`, role `trader_verification_runner`, UTC, and `en_US.utf8`; no `PG_DB` fallback occurred. Every 57J-S phase retained identical before/after operator digest `73c3e970...a5bf`. |
| 57K. Static, Contract, And Regression Gate | Establish that the frozen revision is internally consistent before spending time on integration evidence. | Ruff, compileall, mypy, non-Postgres pytest, MCP/docs/domain/package/SQL boundary suites, `git diff --check` | Done on v6. Ruff over all source/test paths, compileall over five source packages, mypy over 144 files, the full warnings-as-errors non-Postgres suite, focused MCP/docs/domain/package tests, and the frozen diff check passed. No product path changed after the tag. |
| 57L. Realistic Deterministic Evidence Fixture | Replace the toy empty-strategy smoke graph as the acceptance fixture while keeping it bounded and reproducible. | `tests/support/realistic_optimization_fixture.py`, `tests/test_postgres_realistic_optimization_fixture.py`, `PostgresEventStore`, `PostgresResearchArtifactStore` | Done on v6. Three symbols have 48 selection and 32 holdout hourly bars each, separated by 25 hours. The Postgres-only fixture admitted handwritten strategy/risk/objective code, produced meaningful trades/costs/exposure/risk decisions, executed four parameter-sensitive grid trials, selected immutably, ran untouched holdout, and reconciled canonical/typed rows. |
| 57M. Postgres-Native MCP Evidence Graph | Prove the public tool boundary produces complete, visible, realistic product evidence. | `tests/test_postgres_optimization_evidence_graph.py`, `tests/support/mcp_postgres_optimization_server.py`, `PostgresResearchArtifactStore`, typed projections | Done on v6. A real stdio MCP client traversed Data, implementation, specification, backtest, optimisation, Evaluation, and Adversarial tools. Stable root refs include optimisation `parameter_optimization_run_57ad1c26eeeee594`, selected trial `parameter_optimization_trial_9b30d89e706da749`, holdout run `backtest_run_ccfe5ca55bcb72ef`, Evaluation `parameter_optimization_evaluation_b92688e4d77bbe9d`, and robustness report `parameter_optimization_robustness_f425273a64435e47`. Final retained evidence contains 87 canonical artifacts across 15 artifact types, all typed rows reconcile, and canonical responses/rows contain no filesystem authority. |
| 57N. Determinism, Integrity, And Leakage Tests | Prove that reproducibility and holdout claims survive independent repetition and hostile state changes. | `tests/test_postgres_optimization_determinism_integrity.py`, `tests/support/postgres_57n.py`, canonical plan/run loaders | Done on v6. Clean A/B each produced 120 artifacts and digest `702702a4b499325586b94f9f3bbd3e88f07becd20774cf8898dcdb5b3a8ab521`. All 14 hostile mutations failed closed. Selection issued 588 audited reads through `2026-03-03T23:00:00Z`, the database seal was written at `2026-07-21T22:41:29.542280Z`, and 99 holdout reads covered only the later `2026-03-05` region after the seal. |
| TRR-1. Inventory Current `trader_research` | Establish a factual baseline before proposing moves. | Package/file/LOC inventory, public facades, MCP/agent/CLI consumers, large-module symbol inventory | Done without product edits. The architecture record identifies 77 Python files, 27,420 lines, area sizes, seven files over 900 lines, root surfaces, public APIs, and deep consumers. |
| TRR-2. Map Dependencies And Authority | Show which modules actually depend on each other and which stores/providers are authoritative. | AST import map, strongly connected components, store/projection review, MCP composition review | Done without product edits. The review distinguishes one implementation cycle from facade-induced cycles and records central artifact/domain hubs, knowledge/method coupling, DB authority, filesystem leftovers, and optional adapters. |
| TRR-3. Diagnose Comprehensibility Failures | Convert inventory observations into actionable causes rather than treating file size as the design problem. | Canonical architecture findings and current-to-target classification | Done without product edits. Findings cover mixed lifecycle ownership, deep imports, transport leakage, duplicate card semantics, superseded workflows, terminology drift, central projection dispatch, and insufficient boundary enforcement. |
| TRR-4. Approve Target Bounded-Context Design | Establish the target and review gate before any code is moved. | `docs/research_agents/architecture.md`, tracker, docs regression | Done without product edits. Defines Foundation/Governance, Data, Knowledge, Methodology Engineering, Experiments, Review, and outer Infrastructure; pins one-way imports, product-contract handling, vocabulary, hard-cutover policy, phases, and acceptance rules. |
| TRR-5. Retire Superseded Root Workflows | Reduce the change surface before reorganizing current capabilities. | `artifacts.py`, `discovery.py`, `promotion.py`, `recommendations.py`, `research.py`, `suites.py`, root scripts/tests/docs | Done. Deleted the old filesystem workflows, all research-layer callers of core event-store experiment APIs, five command wrappers, obsolete root export, obsolete direct/CLI tests, and active guidance. Generic `trader` event-store APIs/schema remain unchanged. Negative package/docs tests prohibit every retired path; no wrappers, aliases, or readers exist. |
| TRR-6. Extract Foundation, Governance, And Persistence | Replace central dependency hubs with narrow ports and outer adapters. | `foundation`, `governance`, `trader_mcp/contracts.py`, `infrastructure/postgres`, projection-registry and boundary tests | Done. Deleted the old root `agents`, `domain`, `contracts`, artifact-store, and Postgres-store modules without aliases. Research services now return `ApplicationResult`; MCP owns `ToolEnvelope`, side effects, ownership presentation, and JSON. Artifact writes supply explicit context ownership. The generic Postgres store writes canonical JSONB then dispatches registered Methodology, Experiment, or Review projections in the same transaction; it no longer imports every artifact type or contains central `if/elif` dispatch. Foundation-only, retired-hub, SQL-adapter, projection-registry, MCP-contract, handoff, docs, package, Ruff, compileall, mypy, and diff checks pass. Core `trader` was not modified. |
| TRR-7. Encapsulate Knowledge And Methodology Engineering | Make the pinned knowledge subsystem understandable and prevent it coupling generic execution. | split knowledge domain/services/storage, methodology package, method-card cutover, boundary tests | Done. Split the Knowledge domain and methodology extraction/validation/card lifecycle modules; removed the citation/card cycle and KnowledgeStore/MethodRegistry coupling; replaced the former `methods` and `method_implementations` packages with one explicit `methodology` context; removed writable summary cards, seeded card dependencies, `RichMethodCard`, `card_format`, and the second draft tool; exposed only a narrow approved-card read port to Methodology Engineering; and kept maintained contracts and generic implementation admission provenance-neutral. Historical 23/33 task rows describe their original delivery revisions and do not reinstate retired APIs. The core `trader` package was not modified. |
| TRR-8. Decompose Data | Give Data Agent operations cohesive modules and isolate provider infrastructure. | data domain/inventory/quality/loading/catalog modules, Alpaca adapter, facade tests | Done. Removed `data/services.py` without a compatibility module; split request/policy values, provider resolution and discovery, manifest construction, quality reporting, and loading orchestration; retained `trader_research.data` as the only caller facade; moved the Alpaca SDK adapter to `infrastructure.providers.alpaca`; and added import rules that prohibit SDKs, MCP, agents, Knowledge, Methodology, Experiments, and Review from Data. Existing payloads, deterministic IDs, policy failures, MCP names, and platform calls are unchanged. The core `trader` package was not modified. |
| TRR-9. Consolidate Experimentation | Make the implementation-to-run pipeline navigable without weakening 56/57 boundaries. | experiments subpackages/facades, optimization ports, tracking projections, optional provider adapters | Done. Deleted the five former top-level experiment packages without aliases and exposed their application operations through `trader_research.experiments`; retained cohesive internal implementation/specification/backtest/optimization/tracking packages; split optimization planning, execution, ledger validation, queries, and constraints; moved Optuna and MLflow implementations to `infrastructure.providers`; and added import rules proving the provider-neutral context does not import optional SDKs, Infrastructure, Knowledge, Methodology, Review, MCP, or agents. Canonical 56/57 artifacts, MCP names, IDs, Postgres projections, grid/random execution, and provider-independent result reads remain unchanged. The core `trader` package was not modified. |
| TRR-10. Isolate Independent Review | Make skeptical interpretation structurally independent of experiment mutation. | review/evaluation and review/adversarial facades, immutable read ports, mutation-boundary tests | Done. Deleted the former top-level Evaluation and Adversarial packages without aliases; grouped both specialist facades under `trader_research.review`; added the narrow `experiments.reads.ExperimentEvidenceReader` contract and store-backed adapter; routed canonical backtest/plan/run/trial-ledger reads through it; removed direct Experiment artifact dereferencing from Review; and added AST guards proving Review cannot import mutation services or persist Experiment-owned artifact types. Existing Evaluation and Adversarial MCP names, payloads, IDs, ownership, and Postgres projections remain unchanged. The core `trader` package was not modified. |
| TRR-11. Cut Over Composition Roots | Make the new boundaries the only import paths. | MCP dependency injection/adapters, agent ownership imports, package exports, active docs, boundary tests | Done. Expanded explicit Knowledge, Methodology, Foundation, and Governance facades; switched MCP and agents off every deep bounded-context import; retained explicit outer Postgres/Alpaca/Optuna/MLflow adapter imports only at composition; added a declared acyclic context-graph check; and proved a clean interpreter can construct the complete MCP server while all Optuna and MLflow imports fail. Tool names, owners, side effects, policy gates, and direct behavior remain unchanged. Alpaca is still required by the unchanged core `trader.timeframes` package and was not reclassified by this research-layer task. |
| TRR-12. Requalify Refactored Product | Replace revision-bound verification evidence after structural product changes. | new freeze/tag, static suites, Postgres guards/fixtures/MCP graph/determinism, acceptance inventory | Done on `verification-57i-freeze-v4` / `6f3efb4`. Ruff, compileall, expanded mypy, strict non-Postgres tests, package/docs/MCP contracts, Postgres projection reconciliation, and replacement 57J-N all passed with empty blockers and unchanged operator fingerprints. Residual-path and `src/trader` audits passed. |
| 57O. Restart, Resume, And Fault-Injection Tests | Verify canonical Postgres state is sufficient for recovery and that partial work is never misrepresented as complete. | `tests/test_postgres_optimization_recovery.py`, fresh-process worker, injected stores/executors, deadline adapter | Done on v6. All three Postgres cases passed. Uninterrupted and resumed canonical payloads match; pre/post persistence faults do not duplicate trials; an orphan child run is reused without payload or `updated_at` change; retries, terminal blocked reads, and enforced child-process deadlines pass. |
| 57P. Provider Independence And Adapter Qualification | Separate proof of the core product from proof of optional provider adapters. | `tests/test_postgres_optimization_provider_independence.py`, package-blocking stdio server | Done for the mandatory v6 no-provider profile. Fresh MCP processes ran/resumed grid and random and reloaded canonical ledgers while Optuna/MLflow imports failed. Optional Optuna and MLflow profiles remain `not_qualified` and gated off. |
| 57Q. Policy, Security, And Resource-Boundary Tests | Challenge the admission and execution boundaries directly rather than inferring safety from happy paths. | MCP gate matrix, malicious objective/implementation fixtures, limit/deadline tests, SQL/package-boundary tests | Done on v6. All 63 cases passed with warnings as errors, covering forbidden ambient capabilities, independent gates, closed observations, finite scores, immutable scope/cost/holdout boundaries, bounded trial/retry/deadline behavior, process termination, bounded errors, and package direction. Admission remains explicitly not an OS sandbox. |
| 57R. Projection, Operator, And Bounded-Scale Checks | Prove that operators can inspect and operate the feature without relying on test-only internals. | `tests/test_postgres_optimization_bounded_scale.py`, `verification_control.bounded_scale_results`, retained 57M graph | Done on v6. Grid 64 completed in 96.64s with a 0.84s result query; seeded random 100 in 342.57s with a 1.31s query; the three-symbol 1,000-bar backtest in 104.16s. The measured database was 46,428,643 bytes with 1,331 scale artifacts. Required indexes/query plans and every typed projection reconciled; the final public graph was retained. |
| 57S. Acceptance Record And Release Decision | Make the outcome auditable and prevent a partial green run from being described as system verification. | `tests/test_postgres_optimization_acceptance.py`, `verification_control.acceptance_records`, tracker evidence | Done on v6. The acceptance row has `status="passed"`, pins dependency lock `de6a7bc8...edfa`, all passed 57J-R digests/timestamps, final 87-artifact inventory and root refs, qualified built-in grid/random, `not_qualified` Optuna/MLflow with writes disabled, and four explicit residual risks. |
...
The current qualification candidate is identified by annotated Git tag `verification-57i-freeze-v6` on branch
`experiment/agentic-build`. It resolves to `3cd5928533bb678cd387955a0efd7dd19f6d046a` and includes the no-compatibility
bounded-context refactor plus strict idempotent child-backtest recovery and the 57O-S provider-independence,
security/deadline, bounded-scale, and acceptance-record additions. Later verification records use the commit resolved
by that tag, not a moving branch name. The prior v4 and failed v5 checkpoints remain historical evidence for their exact
product revisions.
...
| 38 | C | Candidate IDs/cards/packages are rejected as canonical strategy/risk/backtest inputs. | `test_mcp_exposes_decoupled_tools_and_independent_write_gates`, canonical package-boundary test | Passed / controlled 57K and 57M |
| 39 | C,P | Candidate MCP schemas, aliases, compatibility readers, and dual writes are absent. | Hard-absence package/domain regressions, MCP registration assertions, retired-table checks, and active-doc contract tests. | Passed / replacement 57K |
| 40 | P | Retired development projections are absent and no legacy rows are translated. | `test_research_schema_has_canonical_optimization_tables_without_retired_projections` | Passed / controlled 57J and 57R |
| 41 | P | Backtests, comparisons, sidecars, and Evaluation are complete Postgres artifacts with no durable filesystem authority. | Controlled 57M stdio graph retained complete backtest bundles and Evaluation evidence in canonical JSONB and rejected filesystem authority. | Passed / controlled 57M and bounded 57R |
| 42 | C,P | Methodology provenance status cannot mutate completed implementations/specifications/runs. | 57N canonical loaders and source/validation/strategy/backtest lineage attacks fail closed before run/result consumption. | Passed for canonical execution lineage / controlled 57N |
| 43 | C | Core imports provider-neutral contracts; MCP/grid/random work with Optuna and MLflow unavailable. | Package-boundary tests, built-in engine tests, and clean-process package-blocking MCP profile. | Passed / controlled 57P |
| 44 | C,P | Runs pin engine identity/version/configuration/capabilities/seed/executor and never switch engines. | Unit identity checks plus controlled deterministic and fresh-process resume evidence. | Passed / controlled 57N-O |
| 45 | C | Objectives are content-addressed, closed-input, finite scalar implementations with no ambient capabilities. | Closed-observation and hostile implementation admission tests. | Passed / controlled 57Q |
| 46 | C,P | Plans seal selection/holdout/quality/objective/search/constraint/seed/budget/resource inputs. | Controlled 57N plan/source/snapshot/cost/provider mutation matrix plus clean-repeat graph. | Passed / controlled 57N |
| 47 | C,P | Every suggestion, attempt, failure, child ref, observation, score, and tie-break is canonical evidence. | Controlled 57N retained 12 grid/random/variant trials and revalidated child observations; 57O passed retry/exception/write-fault recovery. | Passed / controlled 57N-O |
...
| 50 | C,P | Holdout Evaluation and Adversarial audit are separately owned and cannot rewrite selection. | Controlled 57M stdio graph produced separately owned passed reports and asserted the baseline run and selected specification were unchanged. | Passed / controlled 57M |
| 51 | C,O,T | Backtest, optimisation, external, Optuna, and tracking gates are independent and reported by config. | Full MCP policy-gate matrix. | Passed / controlled 57Q; optional writes remained false |
| 52 | C,P | Canonical loaders recompute identities and fail closed on source/config/snapshot/lineage drift. | Controlled 57N recorded 14 passed integrity attacks across implementation, validation, specifications, plan, provider, trials, selection, and holdout. | Passed / controlled 57N |
| 53 | C | Evidence is tied to one frozen revision and downstream phases rerun after changes. | Checkpoint `3cd5928` and annotated `verification-57i-freeze-v6` pin the product; all 57J-S rows reference it and v4/failed-v5 evidence is historical. | Passed / controlled 57J-S |
| 54 | P | Tests require explicit `*_test` DB and leave operator runtime/research/knowledge unchanged. | Server/marker/role/locale checks before store construction and every truncate; all controlled v6 phases have matching before/after digest `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`. | Passed / controlled 57J-S |
| 55 | P | Mandatory graph is multi-asset and produces parameter-sensitive trades, costs, exposure, approvals, and rejections. | The controlled 57M stdio MCP graph proves the realistic behavior over the locked 57L fixture, including four distinct trial outcomes and risk-limit breaches. | Passed / controlled 57M |
| 56 | P | Selection and holdout are disjoint by time/hash and holdout is unread before selection. | Controlled 57N SQL audit: selection maximum `2026-03-03T23:00:00Z`; selection reads completed before seal; holdout reads began after seal over the `2026-03-05` region. | Passed / controlled 57N |
| 57 | C,P | Two clean runs reproduce IDs/order/observations/scores/tie-break/selection/reports. | Controlled clean A/B each produced 120 artifacts, identical root refs, and digest `702702a4...ab521`; grid/random orders were independently asserted. | Passed / controlled 57N |
| 58 | P,O | Process restart/faults preserve partial evidence, avoid duplicates, and reject changed-config resume. | Exact baseline/restart, pre/post-write faults, unchanged orphan row, retry, blocked-read, and process-deadline evidence. | Passed for built-ins / controlled 57O; Optuna not qualified |
| 59 | C,O,T | Built-ins/startup/reads survive missing optional packages; each adapter is separately qualified. | Package-blocking stdio MCP test ran/resumed grid/random and canonical reads with Optuna/MLflow unavailable. | Passed for built-ins / controlled 57P; optional profiles not qualified |
| 60 | P | Every acceptance artifact reconciles between canonical JSONB and its typed projection. | Controlled 57M reconciled the public graph; 57R reconciled all 1,331 bounded-scale artifacts and recorded the principal query plan. | Passed / controlled 57R |
| 61 | C,P,O,T | Final record separates pass/fail/not-run/not-qualified and never promotes disposable evidence. | `verification_control.acceptance_records` records the passed mandatory profile and independent optional-provider decisions. | Passed / controlled 57S |

#### 57J Isolated Runtime Contract

Verification commits after `verification-57i-freeze-v6` may change only tests, active docs, and the tracker. The controlled
...
surface is the neutral read-only maintained implementation catalog exposed by `trader_research.experiments`; catalog
...
- The replacement product checkpoint is commit `767511c4af43ad276f0955b296442474b37a7552`, tagged by annotated tag
  `verification-57i-freeze-v2` on `experiment/agentic-build`. The tag resolves to that exact commit, and 57J/57K both
  record it as the executed and verdict revision.
- 57J recreated `trader_verification_test` and the incompatible first control schema from scratch. Postgres 16.11 ran
  in UTC with `LC_COLLATE=en_US.utf8` and `LC_CTYPE=en_US.utf8`; the Trader and Optuna writers remained distinct
  non-superuser roles. All 4 marked isolation tests passed.
- The credential-free configuration digest for both phases was
  `4c86f73fd46e6c6908bfaa2a1c40f515abe4ab788f4f5e688c3fb5095354a72e`. All eight mutation gates were false.
- 57K passed `uv run ruff check src tests`, compileall over all five source packages, `uv run mypy`, and
  `git diff --check`. Mypy reported no issues in its configured 22-source-file surface.
- `uv run pytest -o addopts= -m "not postgres" -q -ra -W error` passed 700 tests in 66.72 seconds with 18 explicitly
  deselected Postgres tests and no warning failure or skip. The focused MCP/optimisation/evidence-graph/docs/domain/
  identity/package/SQL/catalog command passed 58 tests in 7.41 seconds under `-W error`.
- Postgres records both 57J and 57K with `isolation_status="passed"`, `qualification_status="passed"`, empty blockers,
  matching executed/verdict revisions, and identical before/after operator fingerprint
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`. 57J ran from
  2026-07-18 15:34:27-15:34:30 UTC and 57K from 15:35:34-15:36:54 UTC.

#### 57L Postgres-Only Execution Evidence

- The fixture/test/docs harness is commit `34a91c331be41ed1c123d7bcfeca9326cb5f5488`. The controlled harness confirmed
  product paths remain byte-identical to freeze `767511c4af43ad276f0955b296442474b37a7552`.
- The fixture locks selection content hash `e883fa92e7eea43292bcb2f78e1d5ec48b7bf7aec5fe19dee2169646d150d0b0`
  and holdout content hash `96a896db53cfcbfc76b8bc9115d46b2d6bcf8fcc538905410c81ceb1f88debbf`.
  Their Data Agent dataset IDs are `dataset_e02d3f18f9ee9abc` and `dataset_c2866c31b1730cc2`; manifest and quality
  payload hashes are also locked in the test.
- `uv run pytest tests/test_postgres_realistic_optimization_fixture.py -m postgres -q -W error` passed the one
  Postgres qualification test in the controlled phase. The pure fixture assertion, focused docs checks, Ruff,
  compileall, and mypy passed; the strict non-Postgres regression passed 702 tests with 19 Postgres tests deselected in
  62.98 seconds.
- Postgres records 57L from 2026-07-18 17:02:43 to 17:03:13 UTC with
  `isolation_status="passed"`, `qualification_status="passed"`, empty blockers, and matching executed/verdict harness
  revision `34a91c3`. Before/after operator fingerprint is
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`; the credential-free configuration digest is
  `319daea4f2bf3e45c077cdb251584c048cd30a2c5fcc019f96f12fdd3a7b9faa`.
- 57L proves the direct canonical Postgres capability only. Public MCP registration/gates/envelopes, holdout Evaluation,
  Adversarial variants/reporting, and full graph projection reconciliation remain acceptance work for 57M and later
  phases.

#### 57M Controlled Stdio MCP Execution Evidence

- Controlled 57J-M ran against frozen product revision `767511c4af43ad276f0955b296442474b37a7552` with committed
  harness `d8420baf5828c7bf37e7d6bc1264408e56dc8ca2`. Product paths remained byte-identical to the freeze. Phase
  configuration digests were `b09668611d3464dcd6dbbfacc75c65b4f9b5090c5d058e93e81c7968478c9827` (57J),
  `ef80c4446f98b1c81be713043bb081de3b8e6e977ec2352563ded85187be6f8b` (57K),
  `870b0e3b6ca6c04b046c930a9c42e06db4bdca0f11b3c43c86329ad9065b51be` (57L), and
  `8cfaacec5b1662f5d016a7044b83c8920d099c5ce2a803095d9b01c2ff338412` (57M).
- 57J passed 4 Postgres runtime guards. 57K passed Ruff over `src` and `tests`, compileall over all five source
  packages, mypy over its configured 22-file surface, 55 focused contract tests, 705 strict non-Postgres tests with
  20 marked Postgres tests deselected, and `git diff --check`. The 57L Postgres fixture passed in 9.14 seconds. The
  57M stdio MCP graph passed in 16.55 seconds with warnings treated as errors.
- Postgres records all four phases as `isolation_status="passed"` and `qualification_status="passed"` with empty
  blockers. They ran from 2026-07-19 13:10:14 through 13:16:07 UTC. Every phase has matching before/after operator
  fingerprint `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`.
- The retained graph contains 87 canonical artifacts across 15 artifact types: 10 backtest runs and each associated
  specification/validation, 3 implementation versions and validations, 9 strategy and 9 risk-stack specifications
  with validations, 2 optimisation plans, 2 runs, 8 trials, and one Evaluation report, audit plan, and robustness
  report. The test reconciled every canonical payload exactly to its typed projection and found no canonical
  filesystem authority.
- Baseline optimisation run
  `research://postgres/parameter_optimization_run/parameter_optimization_run_44aea4df93a65159` evaluated lookbacks
  2, 3, 4, and 5 in deterministic order. All four trials passed with one successful attempt each and distinct
  objective/return evidence. It uniquely selected lookback 2 via trial
  `research://postgres/parameter_optimization_trial/parameter_optimization_trial_ad49966904208c09`; the selected
  child backtest is `research://postgres/backtest_run/backtest_run_91e2ee04fa8b5d75`.
- Untouched holdout run `research://postgres/backtest_run/backtest_run_1cb626bb8e137a53` used a distinct dataset hash,
  produced 19 trades, nonzero fees/slippage/exposure, and 18 recorded `GAMMA` quantity-limit breaches. Evaluation
  `research://postgres/parameter_optimization_evaluation_report/parameter_optimization_evaluation_4470d3b5833758ce`
  passed with no blockers.
- Adversarial audit plan
  `research://postgres/parameter_optimization_audit_plan/parameter_optimization_audit_plan_e744fffa969c95db`
  covered seed sensitivity and multiple-testing evidence. Supervisor variant run
  `research://postgres/parameter_optimization_run/parameter_optimization_run_12a0d5295a33f67f` completed, and
  Adversarial report
  `research://postgres/parameter_optimization_robustness_report/parameter_optimization_robustness_13670d585c010a5d`
  passed with no blockers. Evaluation and Adversarial execution did not mutate the baseline run, selected trial, or
  selected specification.
- `TRADER_VERIFICATION_RETAIN_PHASE=57M` originally left this evidence in `trader_verification_test` for pgAdmin
  inspection. The later controlled 57N run replaced those disposable product rows after repeating and extending the
  graph; the historical 57M result remains in its phase record.

#### 57N Controlled Determinism And Integrity Evidence

- Product checkpoint `1dd4d3dcea85fbd17513ca1d0140f6b0bde21daf` is annotated as
  `verification-57i-freeze-v3`; controlled harness `69f4bee44726fc3f5a9586033d9ab10b16541877` changed only tests,
  active docs, and the tracker. Freshly provisioned 57J-N phase rows all pin those revisions, have passed isolation and
  qualification statuses, empty blockers, and matching before/after operator fingerprint
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`.
- Controlled configuration digests were `7a85b942e20bf9c0cc3e79ed2019a16298cfb82fdd66d23688197bb0a884b89e`
  (57J), `b6c71eef9759295f99d5f874d6b38be8ab56931fe14c343a1e475c100a5bf301` (57K),
  `cf7328bfac59688da0a74e8de472cc3f36105881aa5946dc5312f8345f7c4bd1` (57L),
  `b9ae7194872e6a958110bd081a7d478df3feaa3c36639b9704badbc34380743a` (57M), and
  `f04eaa3922c6d2185bea22b2dde73968e67c2a7a8064e1fd4aabb7725fdd86bf` (57N).
- 57K passed Ruff over `src` and `tests`, compileall over all five source packages, mypy over its configured 22-file
  surface, 56 focused optimisation/MCP/docs/domain/identity/package tests, 710 strict non-Postgres tests with 21 marked
  Postgres tests deselected, and `git diff --check`. The controlled 57L fixture and 57M public stdio MCP graph each
  passed before 57N.
- Clean executions A and B each produced 120 canonical artifacts across 15 artifact types with identical root refs and
  normalized graph digest `dacabc75a30a0c8b2d79d32d5dbba73fea14f2986cdfb651902f5be8e4bd9923`.
  Normalization excludes only wall-clock backtest `finished_at` and `duration_seconds`; all research payload evidence is
  hashed. The retained graph has 14 backtest runs/specifications/validations, 3 implementation versions/validations,
  3 optimisation runs, 12 trials, 2 plans, 13 strategy and 13 risk specifications with validations, and one Evaluation,
  audit plan, and robustness report.
- Baseline grid order was lookbacks `2,3,4,5`; independently seeded random order was `3,2,5,4`. Both selected lookback
  2 from the same four objective values. Unit qualification also passed canonical forced-tie selection, retry/exception
  persistence, finite-space random exhaustion, and complete result reload.
- All 14 hostile mutations were rejected through public MCP consumers and remain visible in
  `verification_control.integrity_checks`: objective source, objective validation, strategy parameters, selection
  dataset, selection quality, fixed costs, plan seed, engine profile, trial objective, trial sequence, trial child
  lineage, selected trial, selected child, and holdout selection origin.
- Selection optimisation performed 588 audited bar reads with maximum parameter timestamp
  `2026-03-03T23:00:00Z`; its last audit row was recorded at 14:46:20.042708 UTC. The database selection seal was
  written at 14:46:20.303207 UTC. Holdout Evaluation reads began at 14:46:23.273627 UTC over the distinct
  `2026-03-05T00:00:00Z` to `2026-03-06T07:00:00Z` region. Plan setup read both regions only to seal manifests before
  optimisation. No holdout backtest existed before selection, and every trial child/observation retained selection
  lineage.
- `TRADER_VERIFICATION_RETAIN_PHASE=57N` retains the final clean graph and all four 57N control tables in
  `trader_verification_test` for pgAdmin inspection. It replaces the earlier disposable retained 57M product rows.
  Tasks 57O-S remain required before final release acceptance.

#### TRR-12 Replacement 57I-57N Requalification Evidence

- Annotated tag `verification-57i-freeze-v4` resolves to product checkpoint
  `6f3efb49d41b1e7bc3d7a753920bcf2444d23dfa`. The controlled 57J-N rows executed that same clean harness revision.
  No path under `src/trader` changed. Every phase records `isolation_status="passed"`,
  `qualification_status="passed"`, empty blockers, and identical before/after operator fingerprint
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`.
- Configuration digests are `e84fb3a2477e35b1e47b1f534c0e7d3a70d444bb44e1be808737fc706ff4baaa` (57J),
  `0785a910248dffb379c09c836d4ef9c4b9c31d6dec58742bd61a813b2997635c` (57K),
  `92b1a753fb12699656d9366a87fbcf20b8491003e5bf0039e2f92f71017f01f7` (57L),
  `49970cc9f131139e757c0180ee1e8a8587e18fb980fff0d9509e4e7871ec4623` (57M), and
  `0e89f8f812449d5b64c5c7154c179e53d0b4db360613d3af81e8a0d785e7600b` (57N).
- 57J recreated `trader_verification_test` and passed all four role/database/locale/control-schema isolation tests.
  57K passed Ruff, compileall over all five source packages, mypy over 142 source files, 712 strict non-Postgres tests
  with 20 marked Postgres tests deselected, 80 focused MCP/docs/domain/identity/package/SQL contract tests, and
  `git diff --check`, all with warnings treated as errors.
- 57L passed the direct Postgres realistic fixture in 6.78 seconds. 57M passed the public stdio MCP graph in 13.56
  seconds, including complete canonical/typed-projection reconciliation, meaningful multi-asset trades, costs,
  exposure, risk approvals/rejections, untouched-holdout Evaluation, and independent Adversarial evidence.
- 57N clean executions A and B each produced 120 canonical artifacts across 15 artifact types with identical root refs
  and graph digest `702702a4b499325586b94f9f3bbd3e88f07becd20774cf8898dcdb5b3a8ab521`. Grid order was
  `2,3,4,5`; independently seeded random order was `3,2,5,4`; both selected lookback 2 from the same four objective
  values. The retained holdout run has 19 trades, nonzero fees/slippage/exposure, 37 risk decisions, and 18 quantity
  limit breaches. Evaluation and the two-attack Adversarial report passed with empty blockers.
- All 14 hostile implementation/validation/parameter/snapshot/cost/plan/provider/trial/selection/holdout mutations
  failed closed. Selection optimisation issued 588 audited bar reads ending at `2026-03-03T23:00:00Z`; its last audit
  row preceded the database seal at `2026-07-19T17:53:27.990924Z`. Holdout reads began afterward over the distinct
  `2026-03-05T00:00:00Z` through `2026-03-06T07:00:00Z` region. The final clean graph and all 57N control tables remain
  retained in verification Postgres for pgAdmin inspection.

#### Failed V5 Qualification And V6 Recovery Boundary

- Annotated tag `verification-57i-freeze-v5` resolves to product checkpoint
  `1616f313607a3b9b9c05c2d16742b253045c2187`; harness revision
  `903ec8ab15e1621fb9379d46b6084a6f0e8aeeed` changed only tests, active docs, and the tracker. Controlled 57J-N passed
  with empty blockers and unchanged operator fingerprint
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`.
- V5 configuration digests were `087b538ea74ad9333a6d443d15f424f112c72507b13ee818704527324bb0bf08` (57J),
  `ab1ef95236b60da0aeae0381743079a9346947c18702db5541f192d725731c92` (57K),
  `3848dfc3a787143719a3bd638710c9e7ecdcf820b6ccc6053b47832c59983b36` (57L),
  `31765c31b64ea22306c96709dbd9897f3a4de292c7998340abf711c4877cf34c` (57M),
  `4fc07d26345eac3efd8b958456d5d908aa5a1ef28612d11da8caa59820a96837` (57N), and
  `f482dc4c942bfbad86c6624a06567a6244c9d3478182999371b0e928a4828672` at the failed 57O boundary.
- The controlled v5 57O command produced one pass and two failures, then stopped before recording a phase verdict.
  A pre-trial-write fault left a complete child `backtest_run` but no trial artifact; restart replayed the same
  deterministic run against an already-mutated event store, overwrote its canonical row, and selected a different
  objective than the uninterrupted baseline. The deadline fixture also declared one trial against a four-value grid.
  V5 is therefore failed historical evidence and cannot support any 57O-S acceptance claim.
- Annotated tag `verification-57i-freeze-v6` resolves to
  `3cd5928533bb678cd387955a0efd7dd19f6d046a`. Backtest execution now resolves an existing deterministic run ID,
  revalidates record ownership/status, complete lineage, dataset/source hashes, result identity, sidecars, and
  provenance, then returns the unchanged row without replay. Drift fails closed. The deadline fixture now has a
  one-value grid. The exact 57O Postgres suite passed in a pre-qualification repair check and then in the controlled v6
  sequence below.

#### V6 Controlled 57J-S Acceptance Evidence

- Product freeze `verification-57i-freeze-v6` resolves to
  `3cd5928533bb678cd387955a0efd7dd19f6d046a`; controlled harness
  `8732407d756b2a096326cb0c011c31126e9009c5` changed only tests, active docs, and the tracker. All ten 57J-S phase rows
  passed isolation and qualification with empty blockers from `2026-07-21T22:34:44Z` through
  `2026-07-21T22:55:16Z`. Every phase has matching before/after operator fingerprint
  `73c3e970eca95f589c71ac477a2e1f3db9b48ede0c0b3c16c2ee161add498190`.
- Configuration digests are `83c7552bb6c72da269ab52096bfd4c74d5eb8fa824b717cd760ae24954f770b5` (57J),
  `5859b446c3172c1b0e69f0728d3e317b5c2822c8ce966bcdf9509dfc1a05b8dc` (57K),
  `9188f95a52e55704fd6d42ced9ca186a8d111f7ea8d8bb1e2f315c0f07fde367` (57L),
  `252138aeaadb5e4e0dde8147ce7a8914dd6b3221f5eed88aa2f8999779b805fa` (57M),
  `7191c23fe8fd956bc88a0e7731b8654b12506f24cd90e0d355caeb52a6f8cbc3` (57N),
  `050072a1d083038363752386957321efb44ce7eab5a372b1ebd32abacb619e08` (57O),
  `ef45a45ec37553072a020ab5939982ba04397e99c1905cf3fbb517d67c09e947` (57P),
  `6afcc5206d2d40d0b9a87e45934f2189cdc9df7848fe07ac1821dccd6f4a127f` (57Q),
  `40a0bf57ea3d84f2c24edfe7d33df3cedfb42db036e0247ab869fc4d1aef7178` (57R), and
  `08945e431b2be0d4f6abfd384668f72dbe1c92277de2c43b2e49e649e29e5dfa` (57S).
- 57J passed four live Postgres guards. 57K passed Ruff, five-package compileall, mypy over 144 source files, the full
  strict non-Postgres suite, focused MCP/docs/domain/package tests, and the frozen diff check. 57L passed the direct
  Postgres fixture; 57M passed the public stdio graph with stable Evaluation and Adversarial refs; 57N repeated the
  120-artifact graph at digest `702702a4b499325586b94f9f3bbd3e88f07becd20774cf8898dcdb5b3a8ab521`
  and passed all integrity/leakage assertions.
- 57O passed all three recovery/fault/deadline cases and proved that persisted child evidence is not updated or replayed.
  57P passed fresh-process grid/random execution and canonical reads with Optuna and MLflow imports forcibly absent. 57Q
  passed all 63 policy, admission, resource, and package-boundary cases with warnings treated as errors.
- 57R recorded three passed profiles in `verification_control.bounded_scale_results`: 64 grid trials in 96.64 seconds
  with a 0.84-second result query; 100 seeded-random trials in 342.57 seconds with a 1.31-second query; and one
  three-symbol, 1,000-bar-per-symbol portfolio backtest in 104.16 seconds. The measured database was 46,428,643 bytes
  with 1,331 canonical scale artifacts. These are local bounded measurements, not service-level guarantees.
- 57S wrote a passed `verification_control.acceptance_records` row pinned to dependency lock
  `de6a7bc82a5537e4693fc87069b8f76ad78e40f69688d414e29bf400602aedfa`. The final retained product graph has 87
  canonical artifacts, including optimisation runs `parameter_optimization_run_57ad1c26eeeee594` and
  `parameter_optimization_run_c72bfa854be152df`, Evaluation
  `parameter_optimization_evaluation_b92688e4d77bbe9d`, and robustness report
  `parameter_optimization_robustness_f425273a64435e47`.
- Built-in grid and seeded-random are `qualified` with no external-provider writes. Optuna TPE and MLflow tracking are
  `not_qualified`, their write gates remain false, and no optional-provider claim is inferred from package presence.
  Residual risks record that source admission is not an OS sandbox and the scale measurements are not universal

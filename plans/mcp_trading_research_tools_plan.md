...
| --- | --- | --- |
| Quant Research Supervisor Agent | Supervisor state, handoff ledger, strategy planning, risk-stack planning, validation, backtest orchestration, recommendations | strategy candidates, risk-manager candidates, strategy/risk stacks, experiment plans, suites, comparison reports, recommendation reports |
...
| 33H. Rich Methodology Candidate Schema | Not started |  | Add a nullable `methodology_candidate` / rich method-card schema with field-level evidence refs. Define common core fields plus nullable domain extension blocks for technical indicators, statistical arbitrage, options/derivatives, fundamental valuation, sentiment/alternative data, portfolio construction, risk models, and execution methods. The schema must support examples such as options straddles, RSI-style indicators, and commodity sentiment indicators without forcing irrelevant fields. |
| 33I. Methodology Candidate Discovery MCP Tools | Not started |  | Add Quantitative Methods tools to discover methodology candidates from ingested sources by combining retrieval, neighboring chunks, source metadata, and deterministic clustering. Discovery should return candidate spans and blockers, not approved methods, and must avoid treating ingestion as approval. |
| 33J. Evidence-Grounded Field Extraction And Validation | Not started |  | Add extraction services that populate nullable methodology fields and domain extension blocks only when source/chunk evidence supports them. Add validation that every populated field carries valid field-level refs, unsupported fields remain null, domain-specific minimum fields are enforced for the claimed family, internal-note-only provenance fails for textbook-derived methods, and high-risk method families require enough evidence before draft-card creation. |
| 33K. Rich Method Card Draft And Approval Tools | Not started |  | Add `knowledge_create_rich_method_card_draft` and rich-card publishing/validation paths that preserve backward-compatible method-card search while exposing richer cards for implementation generation. Drafts remain non-executable until explicitly approved and validated against their field-level evidence. |
| 33L. Strategy Generation From Rich Method Cards | Not started |  | Extend method packaging and strategy-candidate generation so approved rich method cards can drive implementable strategy/risk templates. The generator should consume algorithm steps, equations, parameters, signal rules, entry/exit rules, sizing, validation requirements, and execution assumptions rather than relying on shallow assumptions/input/output text. |
| 33M. Rich Methodology Documentation And Operator Guide | Not started |  | Update `docs/research_agents/architecture.md`, `docs/research_agents/mcp_tools.md`, `docs/research_agents/tool_contracts.md`, `docs/research_agents/workflows.md`, `docs/research_agents/operations.md`, and agent ownership docs to explain source registration versus ingestion, methodology discovery, rich field extraction, field-level citations, draft/approval behavior, and how rich cards flow into strategy generation. |
| 33N. Rich Methodology End-To-End Evidence | Not started |  | Add deterministic MCP evidence for book PDF -> full-document ingestion -> methodology candidate discovery -> rich field extraction -> rich method-card draft -> approved rich method card -> generated strategy/risk candidate -> portfolio backtest -> Evaluation report, using pairs/cointegration/statistical-arbitrage methods as the representative chain. |
...
| `knowledge_discover_methodology_candidates` | Quantitative Methods Agent | `read_only` | Planned: discover candidate methodology spans from ingested sources using retrieval, neighboring chunks, source metadata, and deterministic grouping without creating approved methods. |
| `knowledge_extract_methodology_fields` | Quantitative Methods Agent | `local_mutating` | Planned: populate nullable rich methodology fields from candidate spans, preserving nulls for unsupported fields and attaching field-level evidence refs to every populated claim. |
| `knowledge_validate_methodology_candidate` | Quantitative Methods Agent | `read_only` | Planned: validate field-level citations, source suitability, unsupported-claim blockers, and family-specific minimum evidence before rich method-card draft creation. |
...
| `knowledge_create_rich_method_card_draft` | Quantitative Methods Agent | `local_mutating` | Planned: create a non-approved rich method-card draft from a validated methodology candidate with nullable field groups and field-level evidence refs. |
...
| 33H. Rich Methodology Candidate Schema | Add rich, nullable methodology artifacts before attempting automated extraction. | `src/trader_research/knowledge/domain.py`, `src/trader_research/domain.py`, `tests/test_knowledge_domain.py`, docs | Define `methodology_candidate` and rich method-card field schemas with common core fields plus nullable domain extension blocks. Core fields cover identity, scope, data requirements, method specification, signal/decision logic, portfolio/execution, risk/validation, and implementation notes. Extension blocks cover technical indicators, statistical arbitrage, options/derivatives, fundamental valuation, sentiment/alternative data, portfolio construction, risk models, and execution methods. Each populated field carries value, evidence refs, confidence/quality metadata, warnings, and blockers. The schema must support options straddles, RSI-style indicators, commodity sentiment indicators, fundamentals, quantitative/statistical methods, portfolio methods, and risk methodologies without requiring irrelevant fields. |
| 33I. Methodology Candidate Discovery MCP Tools | Discover methodology candidates from ingested source chunks without approving them. | `src/trader_research/knowledge/methodology_candidates.py`, `src/trader_mcp/knowledge_tools.py`, `tests/test_methodology_candidates.py`, `tests/test_mcp_quant_methods_tools.py` | Add `knowledge_discover_methodology_candidates` over explicit source IDs, retrieval queries, or method families. The service combines hybrid retrieval, neighboring chunk expansion, source metadata, locator grouping, and deterministic de-duplication to produce candidate spans. It writes/read returns candidate refs, warnings, and blockers; it must not create method cards, implementations, strategies, or approvals. |
| 33J. Evidence-Grounded Field Extraction And Validation | Populate rich methodology fields only when evidence supports them. | `src/trader_research/knowledge/methodology_extraction.py`, `src/trader_research/knowledge/citation_validation.py`, tests | Add `knowledge_extract_methodology_fields` and `knowledge_validate_methodology_candidate`. Extraction can use deterministic rules and a bounded optional LLM adapter, but every non-null core or extension-block field must attach field-level evidence refs. Unsupported or missing fields remain null. Validation fails closed for invalid chunks/locators, unsupported claims, unapproved or unsuitable sources where policy requires approval, missing domain-specific minimum fields for the claimed method family, internal-note-only evidence for textbook-derived methods, excessive direct quotation, and insufficient family-specific evidence for high-risk methods. |
| 33K. Rich Method Card Draft And Approval Tools | Promote validated methodology candidates into rich method-card drafts. | `src/trader_research/knowledge/method_cards.py`, `src/trader_mcp/knowledge_tools.py`, tests | Add `knowledge_create_rich_method_card_draft` and rich-card publish/validation paths while keeping existing shallow cards readable. Rich drafts preserve nullable field groups, field-level evidence refs, candidate provenance, extraction/validation reports, source hashes, and approval lineage. Drafts are still non-executable until explicitly published and accepted by method-contract/implementation validation. |
| 33L. Strategy Generation From Rich Method Cards | Generate implementable strategy/risk candidates from approved rich method cards. | `src/trader_research/methods/`, `src/trader_research/strategy_candidates/`, `src/trader_research/risk_managers/`, tests | Extend method packaging and strategy/risk candidate generation so approved rich cards can drive implementations from algorithm steps, equations, parameters, signal rules, entry/exit rules, sizing, risk controls, validation requirements, and execution assumptions. Generators must reject rich cards missing the fields required by the selected template and preserve symbols/timeframes/date windows as Data Agent scope, not method-card state. |
| 33M. Rich Methodology Documentation And Operator Guide | Document the upgraded source-to-method-to-strategy behavior. | `docs/research_agents/architecture.md`, `docs/research_agents/agents.md`, `docs/research_agents/mcp_tools.md`, `docs/research_agents/tool_contracts.md`, `docs/research_agents/workflows.md`, `docs/research_agents/operations.md`, `tests/test_research_agent_docs.py` | Explain source registration versus full-document ingestion, indexed retrieval, candidate discovery, core fields plus domain extension blocks, field extraction, field-level citation semantics, draft versus approved rich cards, validation blockers, source suitability policy, and how rich method cards feed method packages, strategy/risk generation, portfolio backtests, and Evaluation reports. Include operator examples for pairs/cointegration, an options straddle, an RSI-style technical indicator, and a commodity sentiment/alternative-data indicator. |
| 33N. Rich Methodology End-To-End Evidence | Prove rich methodology extraction can produce strategy evidence. | `tests/test_mcp_rich_methodology_toolchain.py`, docs | Add deterministic MCP evidence for `Algorithmic Trading and Quantitative Strategies` style source ingestion -> methodology candidate discovery -> rich field extraction -> validated rich method-card draft -> approved rich method card -> generated strategy/risk candidate -> risk-scoped portfolio backtest -> Evaluation report. Include fail-closed assertions for thin cards, missing field evidence, internal-note-only evidence, unapproved drafts, and strategy generation from unsupported rich-card fields. |
...
Implement chunk 34 after the deterministic baseline and portfolio/risk toolchains exist. The supervisor should consume
and preserve method, rich methodology, strategy, risk-manager, strategy/risk stack, backtest, and performance artifacts;
it should not become the mechanism that compensates for missing tools.
...
5. Rich methodology candidates can be discovered from ingested sources and can populate nullable method fields only when each populated field has source/chunk evidence.
6. Rich method cards can represent technical, statistical, options/derivatives, portfolio, risk, sentiment/alternative-data, and fundamental methodologies with field-level citations, validation blockers, and explicit draft/approval lineage.
7. Rich methodology schemas use common core fields plus nullable domain extension blocks for technical indicators, statistical arbitrage, options/derivatives, fundamental valuation, sentiment/alternative data, portfolio construction, risk models, and execution methods.
8. Strategy and risk candidate generation can consume approved rich method cards when required fields such as algorithm steps, parameters, signal rules, entry/exit rules, sizing, risk controls, and validation requirements are present.
9. Documentation explains the upgraded source registration, full-document ingestion, retrieval, methodology discovery, rich field extraction, rich method-card approval, domain extension blocks, and strategy-generation flow with operator examples.
...
11. Strategy candidates include an importable Python strategy source artifact implementing `trader.strategies.Strategy`
   and are validated before any backtest run.
12. Multi-asset strategy candidates can be generated and validated without baking symbols, timeframe, or date windows into the strategy source; the Data Agent dataset manifest remains the data-scope authority.
13. Risk-manager candidates include importable Python source artifacts implementing the platform risk-manager contract and are validated before any risk-scoped portfolio backtest.
14. Strategy/risk stack artifacts can compose one validated strategy with one or more validated risk managers and preserve their ordering, source hashes, validation reports, and no-live-trading constraints.
15. Baseline and risk-scoped portfolio backtest artifacts include reproducible config/provenance, dataset manifest references, data-quality report references when available, strategy/risk refs, result summaries, equity curves, symbol-level metrics, exposure/risk summaries, and trade exports when available.
...
17. One end-to-end MCP test proves: source-backed method package -> strategy candidate -> validated strategy -> baseline backtest -> performance report.
18. A follow-on end-to-end MCP test proves: source-backed method package -> multi-asset strategy candidate -> risk-manager candidate(s) -> validated strategy/risk stack -> risk-scoped portfolio backtest -> portfolio/risk evaluation report.
19. A rich-methodology MCP test proves: full-document book ingestion -> candidate discovery -> field-level extraction -> rich card -> generated strategy/risk candidate -> risk-scoped portfolio backtest -> Evaluation report.
20. The Quant Research Supervisor can consume method package, rich methodology, strategy candidate, risk-manager candidate, strategy/risk stack, backtest, and performance-report refs without rewriting or forging specialist artifacts.
...
- Generated strategy and risk-manager code: strategy source generation exists for maintained templates. The next
  boundary is source-backed risk-manager generation and strategy/risk stack validation for backtesting only; do not
  expose arbitrary prompt-authored runtime code or live risk controls.

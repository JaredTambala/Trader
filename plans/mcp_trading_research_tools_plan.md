...
| 57. Reproducible Strategy, Risk, Backtest, And Optimisation Specifications | In progress | TRR-12 replacement 57J-N passed on refactored freeze/harness `6f3efb4`; 57N evidence remains retained in verification Postgres. | Immutable Postgres specifications, runs, trial ledgers, projections, Evaluation, and Adversarial refs compose through public stdio MCP. Canonical run reads recompute complete ledger/selection integrity; restart, provider, policy, scale, and final acceptance phases 57O-S remain. |
...
| 57I. Freeze Revision And Build Acceptance Matrix | Done | `verification-57i-freeze-v4` resolves to `6f3efb49d41b1e7bc3d7a753920bcf2444d23dfa`. | The TRR-12 freeze contains the no-compatibility bounded-context refactor, expanded type gate, deterministic Postgres controls, tests, and active docs without modifying `src/trader`. |
...
| 57O. Restart, Resume, And Fault-Injection Tests | Planned | Ready after passing TRR-12 replacement 57I-N evidence on the refactored freeze. | Interrupt after persisted phase boundaries, recreate service processes, resume without duplicate trials, exercise bounded retry/timeout/provider-loss paths, and prove partial or blocked runs remain inspectable and cannot be silently continued under changed configuration. |
...
2. `TRR-5` through `TRR-12` are complete. The no-compatibility bounded-context cutover is now the only supported
   ...
   57J-N Postgres evidence qualify that architecture.
3. `57O-57S`: resume controlled core/Postgres acceptance on the refactored freeze; keep Optuna and each tracking sink
...
The current replacement checkpoint revision is identified by annotated Git tag `verification-57i-freeze-v4` on branch
`experiment/agentic-build`. It resolves to `6f3efb49d41b1e7bc3d7a753920bcf2444d23dfa`, the no-compatibility bounded-context
refactor after the inventory, matrix, documentation regression, and checkpoint commit completed. Later verification
records use the commit resolved by that tag, not a moving branch name. The prior v3 checkpoint remains historical
evidence for its exact pre-refactor product revision.
...
| 60 | P | Every acceptance artifact reconciles between canonical JSONB and its typed projection. | Controlled 57M reconciled all 87 graph artifacts across 15 artifact types exactly against their typed projections. | Passed for the 57M graph / 57R repeats at bounded scale |
...
Verification commits after `verification-57i-freeze-v4` may change only tests, active docs, and the tracker. The controlled

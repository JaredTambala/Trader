...
| 33A. Multi-Asset And Risk Artifact Schemas | Done | `uv run ruff check src/trader_research/risk_managers.py src/trader_research/domain.py src/trader_research/agents.py src/trader_mcp/constants.py src/trader_mcp/research_tools.py src/trader_mcp/server.py tests/test_risk_manager_candidates.py tests/test_mcp_risk_manager_tools.py tests/test_agent_identities.py tests/test_research_domain.py tests/test_mcp_server.py`; `python -m compileall -q src/trader_research src/trader_mcp src/trader_agents`; `uv run pytest tests/test_risk_manager_candidates.py tests/test_mcp_risk_manager_tools.py tests/test_agent_identities.py tests/test_research_domain.py tests/test_mcp_server.py -q` | Defines `risk_manager_candidate_manifest.json`, `risk_manager_implementation.py`, `risk_manager_candidate_validation_report.json`, `strategy_risk_stack_manifest.json`, `strategy_risk_stack_validation_report.json`, and `portfolio_backtest_run_ref.json` artifact contracts. Backtest refs now carry optional strategy/risk stack IDs, symbol metrics, exposure summaries, and risk-measure summaries. Strategy and risk candidates remain data-free; symbols, timeframe, and date windows still come from Data Agent dataset manifests. |
| 33B. Risk Manager Template Catalog And Builder | Done | `uv run ruff check src/trader_research/risk_managers.py src/trader_research/domain.py src/trader_research/agents.py src/trader_mcp/constants.py src/trader_mcp/research_tools.py src/trader_mcp/server.py tests/test_risk_manager_candidates.py tests/test_mcp_risk_manager_tools.py tests/test_agent_identities.py tests/test_research_domain.py tests/test_mcp_server.py`; `python -m compileall -q src/trader_research src/trader_mcp src/trader_agents`; `uv run pytest tests/test_risk_manager_candidates.py tests/test_mcp_risk_manager_tools.py tests/test_agent_identities.py tests/test_research_domain.py tests/test_mcp_server.py -q` | Adds supervisor-owned `research_list_risk_manager_templates` and `research_create_risk_manager_candidate`. The catalog covers gross exposure caps, per-symbol exposure caps, concentration caps, drawdown guards, and VaR/CVaR-style filters as generation targets. Candidate creation writes importable backtest-only `trader.risk.RiskManager` source plus `risk_manager_candidate_manifest.json`, records source hashes, bounded scalar parameters, optional validated method-package refs for risk measures, and no-live-trading execution assumptions. Risk-manager validation, stack composition, portfolio backtests, and portfolio/risk reports remain 33D-33F. |
| 33C. Multi-Asset Strategy Candidate Generation | Not started |  | Extend strategy templates and generated source metadata for multi-asset portfolio construction: per-symbol independent strategies, cross-sectional ranking/allocation, rebalance cadence, allocation bounds, and portfolio-state requirements. Generated strategy source must accept the symbol universe from the backtest data scope and may emit orders for multiple symbols; it must not bake symbols or dates into the strategy candidate. |
| 33D. Strategy/Risk Stack Builder And Validation | Not started |  | Add supervisor-owned tools to compose one validated strategy candidate with one or more validated risk-manager candidates into `strategy_risk_stack_manifest.json`, then validate the stack with deterministic multi-asset fixture smoke tests. Validation must check ordering, runtime contracts, allowed side effects, exposure/risk telemetry hooks, and that every risk manager is validated before any portfolio backtest. |
| 33E. Risk-Scoped Portfolio Backtest Tools | Not started |  | Add a portfolio backtest path that consumes a passed strategy/risk stack validation report and exactly one Data Agent dataset manifest with one or more symbols. It should run through existing `BacktestRunner`/risk-manager interfaces, persist the strategy/risk stack refs in provenance, export symbol-level returns, positions, trades, exposure summaries, risk-manager decisions, and risk-limit breach evidence, and remain gated like other backtest execution tools. |
| 33F. Portfolio And Risk Evaluation Reports | Not started |  | Extend Evaluation-owned reports so meaningful evaluations can be based on a strategy plus one or more risk managers over one or more assets. Reports should summarize total and per-symbol performance, allocation/concentration, gross/net exposure, turnover, fees/slippage, risk-limit breaches, VaR/CVaR-style estimates when supplied by validated method packages, and blockers when portfolio backtests omit required risk evidence. |
| 33G. Multi-Asset Risk Toolchain Evidence | Not started |  | Add an MCP evidence test proving method package -> multi-asset strategy candidate -> risk-manager candidate(s) -> strategy/risk stack -> risk-scoped portfolio backtest -> Evaluation report. This becomes the meaningful portfolio-construction checkpoint before expanding supervisor autonomy. |
...
| `research_validate_risk_manager_candidate` | Quant Research Supervisor Agent | `local_mutating` | Planned: validate generated risk-manager source against the `trader.risk` runtime contract before any portfolio backtest. |
| `research_create_strategy_risk_stack` | Quant Research Supervisor Agent | `local_mutating` | Planned: compose one validated strategy candidate with one or more validated risk-manager candidates into a strategy/risk stack manifest. |
| `research_validate_strategy_risk_stack` | Quant Research Supervisor Agent | `local_mutating` | Planned: smoke-test a strategy/risk stack over a deterministic multi-asset fixture and write `strategy_risk_stack_validation_report.json`. |
...
| 33B. Risk Manager Template Catalog And Builder | Add the supervisor-owned risk-manager generation surface. | `src/trader_research/risk_managers.py`, `src/trader_mcp/research_tools.py`, tests | Done. MCP lists bounded risk-manager templates and creates source-backed `risk_manager_candidate_manifest.json` artifacts from approved templates, scalar parameters, and optional method-package refs. Templates cover exposure caps, per-symbol caps, concentration caps, drawdown controls, and VaR/CVaR-style risk filters as generation targets. Generated candidates implement `trader.risk.RiskManager`, record source hashes and no-live-trading assumptions, and remain validation-deferred/backtest-only. |
| 33C. Multi-Asset Strategy Candidate Generation | Extend source-backed strategy generation for portfolio construction. | `src/trader_research/strategies.py`, tests | Strategy templates declare whether they are single-symbol, per-symbol multi-asset, or cross-sectional portfolio templates; generated source accepts symbols from the backtest data scope; parameters include allocation/rebalance semantics where needed; validation smoke tests use multiple symbols; candidates remain data-free. |
| 33D. Strategy/Risk Stack Builder And Validation | Compose validated strategies with one or more risk managers before backtesting. | `src/trader_research/portfolio_stacks.py`, `src/trader_mcp/research_tools.py`, tests | Add `research_create_strategy_risk_stack` and `research_validate_strategy_risk_stack`. The stack builder consumes passed strategy and risk-manager validation reports, records risk-manager order/priority, writes `strategy_risk_stack_manifest.json`, and validates the combined runtime against deterministic multi-asset fixture bars before any portfolio backtest. |
| 33E. Risk-Scoped Portfolio Backtest Tools | Run backtests over a multi-asset data scope with a validated strategy/risk stack. | `src/trader_research/backtests.py`, `src/trader_mcp/research_tools.py`, tests | Add `research_run_portfolio_backtest` or extend backtest execution with an explicit stack-ref mode. The tool requires a passed strategy/risk stack validation report and exactly one Data Agent dataset manifest; persists stack refs, multi-symbol data scope, risk-manager decisions, exposure summaries, limit breaches, symbol-level metrics, trades, positions, curves, and provenance. |
| 33F. Portfolio And Risk Evaluation Reports | Make Evaluation reports reflect strategy plus risk stack behavior. | `src/trader_research/evaluation.py`, `tests/test_performance_reports.py`, docs | Evaluation reports consume risk-scoped portfolio backtest bundles, summarize per-symbol and portfolio metrics, gross/net exposure, concentration, turnover, fees/slippage, risk-manager blockers/warnings, VaR/CVaR-style estimates when supplied, and risk-limit breach evidence. Reports block when portfolio evaluations omit required risk evidence. |
| 33G. Multi-Asset Risk Toolchain Evidence | Prove the portfolio-construction MCP chain before expanding supervisor autonomy. | `tests/test_mcp_portfolio_risk_toolchain.py`, docs | One deterministic MCP flow builds a source-backed multi-asset strategy, one or more source-backed risk managers, a validated strategy/risk stack, a risk-scoped portfolio backtest, and an Evaluation report. The test asserts artifact ownership, side effects, multi-symbol scope, risk refs, risk telemetry, and fail-closed behavior for unvalidated risk managers or missing stack validation. |
...
and preserve method, strategy, risk-manager, strategy/risk stack, backtest, and performance artifacts; it should not
become the mechanism that compensates for missing tools.
...
1. The MCP server exposes data inventory, data quality, bounded data-loading, source-backed method, method-package, strategy-candidate, risk-manager-candidate, strategy/risk stack, backtest, result-lookup, and performance-report tools through shared envelopes.
...
7. Multi-asset strategy candidates can be generated and validated without baking symbols, timeframe, or date windows into the strategy source; the Data Agent dataset manifest remains the data-scope authority.
8. Risk-manager candidates include importable Python source artifacts implementing the platform risk-manager contract and are validated before any risk-scoped portfolio backtest.
9. Strategy/risk stack artifacts can compose one validated strategy with one or more validated risk managers and preserve their ordering, source hashes, validation reports, and no-live-trading constraints.
10. Baseline and risk-scoped portfolio backtest artifacts include reproducible config/provenance, dataset manifest references, data-quality report references when available, strategy/risk refs, result summaries, equity curves, symbol-level metrics, exposure/risk summaries, and trade exports when available.
11. Performance reports include practical metrics, assumptions, warnings, blockers, caveats, multi-asset attribution summaries where available, exposure/concentration evidence, and risk-manager evidence linked to the backtest and data-quality artifacts.
12. One end-to-end MCP test proves: source-backed method package -> strategy candidate -> validated strategy -> baseline backtest -> performance report.
13. A follow-on end-to-end MCP test proves: source-backed method package -> multi-asset strategy candidate -> risk-manager candidate(s) -> validated strategy/risk stack -> risk-scoped portfolio backtest -> portfolio/risk evaluation report.
14. The Quant Research Supervisor can consume method package, strategy candidate, risk-manager candidate, strategy/risk stack, backtest, and performance-report refs without rewriting or forging specialist artifacts.
15. LangGraph agent identities and supervisor LLM control are follow-on orchestration layers over the deterministic toolchain, not prerequisites for the first meaningful suite.
16. Every tool returns the shared JSON envelope and declares its side-effect class.
17. Every tool declares the owning agent and returns/links the artifact owned by that agent.
18. Missing/incomplete data fails closed or produces Data Agent warnings and downstream Evaluation blockers.
19. `src/trader/` contains no research experiment, agent-tool, MCP schema/definition, or LangGraph agent modules.
20. No MCP tool or LangGraph agent can place live orders, mutate broker state, run raw SQL, bypass existing platform validation, or execute arbitrary strategy code from prompts.
